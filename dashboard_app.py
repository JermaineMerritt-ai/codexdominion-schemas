...
class CodexDashboard:
    ...
    def render_header(self):
        ...
        with col1:
            ...
            st.caption(
                f"v{self.app_config.get('app', {}).get('version', '1.0.0')} | Command Center"
            )
    ...
    def render_empire_overview(self):
        ...
        with col1:
            st.metric(
                "🎬 Video Studio",
                "OMEGA",
                help="Video Studio Omega - GenSpark Obliterator",
            )
        ...
        with col2:
            st.metric(
                "🚀 Web Development",
                "DESTROYER",
                help="Lovable Destroyer - Web Dev Revolution",
            )
        ...
        with col3:
            st.metric(
                "⚡ Automation",
                "FLOW ENGINE",
                help="Codex Flow Engine - N8N Annihilator",
            )
        ...
        with col5:
            active_systems = len(
                [k for k, v in self.app_config.get("features", {}).items() if v]
            )
    ...
    def render_dashboard_capsules(self):
        ...
        if self.capsules_data:
            ...
            for idx, (capsule_name, capsule_data) in enumerate(
                self.capsules_data.items()
            ):
                ...
                with col:
                    with st.container():
                        st.write(
                            f"**{capsule_data.get('capsule', {}).get('name', capsule_name.title())}**"
                        )

                        capsule_type = capsule_data.get("capsule", {}).get(
                            "type", "unknown"
                        )
                        version = capsule_data.get("capsule", {}).get(
                            "version", "1.0.0"
                        )

                        st.caption(f"Type: {capsule_type} | v{version}")

                        description = capsule_data.get("capsule", {}).get(
    ...
    def render_system_cycles(self):
        ...
        if self.cycles_data:
            ...
            if current_cycle:
                st.info(
                    f"🌟 Current Cycle: **{current_cycle['cycle']}** ({current_cycle['season']})"
                )
        else:
    ...
    def render_sidebar(self):
        ...
        st.sidebar.caption(
            f"Version: {self.app_config.get('app', {}).get('version', '1.0.0')}"
        )
        st.sidebar.caption(
            f"Environment: {self.app_config.get('app', {}).get('environment', 'production')}"
        )
    ...
    def render_dashboard(self):
        ...
        with col2:
            st.caption(
                "👑 **Codex Dominion Digital Empire** - Supreme Automation Platform"
            )
