...
class SecureCodexMCPServer {
  ...
  setupRoutes() {
    ...
    this.app.get('/capabilities', (req, res) => {
      res.json({
        ...
        tools: ['read_file', 'list_directory', 'system_status', 'workspace_info'],
        endpoints: {
          health: '/health',
  ...
  async getSystemStatus() {
    ...
    const criticalFiles = ['package.json', 'app.py', '.env', 'data/codex_empire.db'];
  ...
  async getWorkspaceInfo() {
    try {
      ...
      return {
        ...
        structure: {
          hasBackend: await fs
            .access(path.join(__dirname, 'app.py'))
            .then(() => true)
            .catch(() => false),
          hasFrontend: await fs
            .access(path.join(__dirname, 'frontend'))
            .then(() => true)
            .catch(() => false),
          hasDatabase: await fs
            .access(path.join(__dirname, 'data'))
            .then(() => true)
            .catch(() => false),
          hasConfig: await fs
            .access(path.join(__dirname, 'config'))
            .then(() => true)
            .catch(() => false),
        },
      };
