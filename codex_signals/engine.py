...
class SignalsEngine:
    def __init__(self, exposure_cap: float = 0.10, vol_alpha: float = 0.25, trend_alpha: float = 0.35):
    ...
    def classify_tier(self, m: MarketSnapshot) -> str:
        if m.trend_20d >= self.trend_alpha and m.vol_30d <= self.vol_alpha and m.liquidity_rank <= 50:
            return "Alpha"
    ...
    def target_weight(self, tier: str, m: MarketSnapshot, current_weight: float, allowed_max: float) -> float:
        base = {
    ...
    def rationale(self, tier: str, m: MarketSnapshot) -> str:
        parts = []
        parts.append(f"20d trend={m.trend_20d:.2f}, 30d vol={m.vol_30d:.2f}, liquidity_rank={m.liquidity_rank}")
        if tier == "Alpha":
            parts.append("Strong positive trend with controlled volatility and deep liquidity.")
        elif tier == "Beta":
            parts.append("Neutral/stable regime; measured exposure and diversification preferred.")
        elif tier == "Gamma":
    ...
    def risk_factors(self, m: MarketSnapshot) -> List[str]:
        factors = []
        if m.vol_30d > 0.35: factors.append("Volatility above comfort threshold")
        if abs(m.trend_20d) < 0.15: factors.append("Weak/indecisive trend")
        if m.liquidity_rank > 100: factors.append("Lower liquidity")
        return factors

    def generate(self, market: List[MarketSnapshot], positions: Dict[str, Position]) -> Dict[str, Any]:
        picks: List[Pick] = []
