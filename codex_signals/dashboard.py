...
class SignalsDashboard:
    ...
    def render_sidebar(self):
        ...
        data_source = st.sidebar.selectbox(
            "Data Source", ["Mock Data", "Live Feed (Demo)"], index=0
        )
    ...
    def render_market_overview(self, snapshots: List[MarketSnapshot]):
        ...
        with col2:
            ...
            fig_scatter.add_hline(
                y=self.engine.vol_alpha,
                line_dash="dash",
                annotation_text="Vol Alpha Threshold",
            )
            fig_scatter.add_vline(
                x=self.engine.trend_alpha,
                line_dash="dash",
    ...
    def render_signals_analysis(self, snapshot: Dict):
        ...
        with col2:
            st.metric(
                "Beta Tier", tier_counts["Beta"], help="Neutral/stable conditions"
            )
        with col3:
            st.metric(
                "Gamma Tier",
        ...
        picks_display["Target Weight"] = picks_display["target_weight"].apply(
            lambda x: f"{x*100:.1f}%"
        )
        picks_display["Risk Factors Count"] = picks_display["risk_factors"].apply(len)

        # Color-code tiers in picks
        styled_picks = picks_display[
            ["symbol", "tier", "Target Weight", "rationale", "Risk Factors Count"]
        ].style.applymap(
            lambda val: (
                ...
                if val == "Alpha"
                    ...
                    if val == "Beta"
                        ...
                        if val == "Gamma"
                        else "background-color: #FFB6C1" if val == "Delta" else ""
                    )
                )
            ),
            subset=["tier"],
    ...
    def render_risk_analysis(self, snapshot: Dict):
        ...
        for pick in picks:
            for risk in pick["risk_factors"]:
                all_risks.append(
                    {"symbol": pick["symbol"], "risk": risk, "tier": pick["tier"]}
                )
        ...
        if all_risks:
            ...
            with col1:
                ...
                risk_df = pd.DataFrame(
                    list(risk_counts.items()), columns=["Risk Factor", "Count"]
                )
                fig_risk = px.bar(
                    risk_df,
                    x="Count",
            ...
            with col2:
                ...
                if not risk_symbols_df.empty:
                    risk_summary = (
                        risk_symbols_df.groupby("symbol")
                        .agg({"risk": "count", "tier": "first"})
                        .reset_index()
                    )
            ...
            st.success(
                "✅ No significant risk factors detected across portfolio signals"
            )
