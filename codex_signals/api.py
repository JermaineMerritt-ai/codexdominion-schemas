...
from codex_signals.engine import SignalsEngine, MarketSnapshot, Position, Pick
...
class MarketSnapshotModel(BaseModel):
    ...
    vol_30d: float = Field(..., ge=0, le=2, description="30-day realized volatility [0.0-2.0]")
    trend_20d: float = Field(..., ge=-1, le=1, description="20-day trend score [-1.0, 1.0]")
    liquidity_rank: int = Field(..., ge=1, description="Liquidity rank (1=highest)")

class PositionModel(BaseModel):
    symbol: str = Field(..., description="Asset symbol")
    weight: float = Field(..., ge=0, le=1, description="Current portfolio weight [0.0-1.0]")
    allowed_max: float = Field(..., ge=0, le=1, description="Maximum allowed weight [0.0-1.0]")

class EngineConfigModel(BaseModel):
    exposure_cap: float = Field(0.10, ge=0.01, le=0.50, description="Maximum weight per position")
    vol_alpha: float = Field(0.25, ge=0.10, le=0.60, description="Volatility threshold for Alpha tier")
    trend_alpha: float = Field(0.35, ge=0.10, le=0.80, description="Trend threshold for Alpha tier")

class DailySignalsRequest(BaseModel):
    market: List[MarketSnapshotModel] = Field(..., description="List of market snapshots")
    positions: List[PositionModel] = Field(default=[], description="Current portfolio positions")
    engine_config: Optional[EngineConfigModel] = Field(None, description="Engine configuration override")
...
async def daily_signals(request: DailySignalsRequest):
    ...
    try:
        ...
        market_summary = {
            ...
            "avg_volatility": sum(m.vol_30d for m in market) / len(market) if market else 0,
            "avg_trend": sum(m.trend_20d for m in market) / len(market) if market else 0,
        ...
        logger.info(f"Generated signals for {len(market)} symbols in {execution_time:.2f}ms")
...
async def classify_tier(snapshot: MarketSnapshotModel):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Tier classification failed: {str(e)}")
...
async def portfolio_analysis(request: DailySignalsRequest):
    ...
    try:
        ...
        market = [
            MarketSnapshot(
                symbol=m.symbol, price=m.price, vol_30d=m.vol_30d,
                trend_20d=m.trend_20d, liquidity_rank=m.liquidity_rank
            )
            for m in request.market
        ]
        
        positions = {
            p.symbol: Position(symbol=p.symbol, weight=p.weight, allowed_max=p.allowed_max)
            for p in request.positions
        }
        
        snapshot = engine.generate(market, positions)
        rebalance_analysis = integration.update_positions_from_signals(snapshot, auto_rebalance=False)
...
async def get_metrics():
    ...
    try:
        ...
        if os.path.exists(history_file):
            ...
            today_signals = [
                h for h in history 
                if datetime.fromisoformat(h.get('timestamp', '2000-01-01')).date() == today
            ]
