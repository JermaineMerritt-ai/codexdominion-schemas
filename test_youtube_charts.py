...
def test_youtube_system():
    ...
    try:
        ...
        if config_file.exists():
            ...
            print(f"   API Key configured: {'Yes' if config.get('youtube', {}).get('api_key') not in [None, 'YOUR_YOUTUBE_API_KEY_HERE'] else 'No'}")
            print(f"   Channel ID configured: {'Yes' if config.get('youtube', {}).get('channel_id') not in [None, 'YOUR_CHANNEL_ID_HERE'] else 'No'}")
            print(f"   Auto-archive enabled: {config.get('chart_settings', {}).get('auto_archive', False)}")
    ...
    try:
        ...
        test_data = {"test": "compatibility_check", "timestamp": datetime.datetime.utcnow().isoformat()}
