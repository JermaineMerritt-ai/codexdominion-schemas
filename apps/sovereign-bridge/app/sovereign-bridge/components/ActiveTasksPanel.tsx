...
import { DashboardTile, StatusBadge, ProgressTile } from '@/components'
...
export default function ActiveTasksPanel() {
  ...
  return (
    <DashboardTile title="Active Tasks & Executions" icon="⚙️">
        ...
        {tasks.map((task) => (
          <div key={task.id} className="codex-panel hover:bg-codex-navy/90 transition-all">
            <div className="flex items-start justify-between mb-3">
              ...
              <StatusBadge
                status={task.status === 'completed' ? 'success' : task.status === 'failed' ? 'error' : task.status === 'running' ? 'active' : 'pending'}
