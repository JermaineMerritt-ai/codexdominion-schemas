...
class CodexPinterestCapsule:
    ...
    def pinterest_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            start_date = end_date - datetime.timedelta(days=self.config.get('analytics_days', 30))
            ...
            if analytics_response.status_code != 200:
                logger.warning(f"Pinterest analytics API error: {analytics_response.status_code}")
                return self._get_demo_metrics()
            ...
            boards_data = boards_response.json() if boards_response.status_code == 200 else {"items": []}
    ...
    def _process_pinterest_analytics(self, analytics_data: Dict, boards_data: Dict) -> Dict[str, Any]:
        ...
        # Calculate totals
        total_impressions = sum(day.get('data_status', {}).get('IMPRESSION', 0) for day in daily_metrics)
        total_clicks = sum(day.get('data_status', {}).get('OUTBOUND_CLICK', 0) for day in daily_metrics)
        total_pin_clicks = sum(day.get('data_status', {}).get('PIN_CLICK', 0) for day in daily_metrics)
        total_saves = sum(day.get('data_status', {}).get('SAVE', 0) for day in daily_metrics)
        ...
        affiliate_revenue = total_clicks * estimated_affiliate_rate * estimated_commission
        
        # Board analysis
        total_boards = len(boards_data.get('items', []))
        active_boards = sum(1 for board in boards_data.get('items', []) if board.get('pin_count', 0) > 0)
        ...
        if len(daily_metrics) >= 7:
            ...
            recent_impressions = sum(day.get('data_status', {}).get('IMPRESSION', 0) for day in recent_week)
            previous_impressions = sum(day.get('data_status', {}).get('IMPRESSION', 0) for day in previous_week)
            
            growth_rate = ((recent_impressions - previous_impressions) / previous_impressions * 100) if previous_impressions > 0 else 0
        else:
            growth_rate = 0
        ...
        return {
            ...
            'board_utilization_rate': round((active_boards / total_boards * 100) if total_boards > 0 else 0, 1),
            ...
            # Revenue analysis
            'revenue_per_click': round(affiliate_revenue / total_clicks if total_clicks > 0 else 0, 2),
            'revenue_per_impression': round(affiliate_revenue / total_impressions * 1000 if total_impressions > 0 else 0, 2),  # RPM
            'estimated_monthly_revenue': round(affiliate_revenue * 30 / self.config.get('analytics_days', 30), 2),
            
            # Quality metrics
            'content_quality': self._assess_content_quality(click_through_rate, save_rate),
            'audience_engagement': self._assess_audience_engagement(engagement_rate),
            'monetization_efficiency': self._assess_monetization_efficiency(affiliate_revenue, total_clicks),
            
            # Trend analysis
            'performance_trend': self._determine_performance_trend(growth_rate, performance_score),
            'revenue_trend': self._determine_revenue_trend(affiliate_revenue, total_clicks),
    ...
    def _calculate_pinterest_performance_score(self, ctr: float, save_rate: float, 
                                            engagement_rate: float, impressions: int) -> float:
        ...
        engagement_score = min(engagement_rate / 2.5 * 100, 100)  # 2.5% is good engagement
    ...
    def _determine_performance_trend(self, growth_rate: float, performance_score: float) -> str:
    ...
    def _get_demo_metrics(self) -> Dict[str, Any]:
        ...
        base_clicks = random.randint(int(base_impressions * 0.008), int(base_impressions * 0.025))
        base_saves = random.randint(int(base_impressions * 0.015), int(base_impressions * 0.040))
        ...
        return {
            ...
            # Revenue analysis
            'revenue_per_click': round(affiliate_revenue / base_clicks if base_clicks > 0 else 0, 2),
            'revenue_per_impression': round(affiliate_revenue / base_impressions * 1000 if base_impressions > 0 else 0, 2),
    ...
    def pinterest_pin(self, title: str, link: str, image_url: str, 
                     board_id: Optional[str] = None, description: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            if not board_id:
                return {
                    'success': False,
                    'error': "No board ID provided or configured"
                }
...
def pinterest_pin(title: str, link: str, image_url: str, 
                 board_id: Optional[str] = None, description: str = "") -> Dict[str, Any]:
