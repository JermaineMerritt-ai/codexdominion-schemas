...
class MCPAutoStartupTester {
  ...
  async testWebhookAvailability() {
    await this.runTest('Auto-Startup Webhook', async () => {
      const response = await axios.get(`${this.webhookUrl}/mcp-status`, {
        timeout: 5000,
      });
  ...
  async testChatActivityTrigger() {
    await this.runTest('Chat Activity Trigger', async () => {
      ...
      const response = await axios.post(`${this.webhookUrl}/chat-activity`, chatActivity, {
        timeout: 5000,
      ...
      const statusResponse = await axios.get(`${this.mcpServerUrl}/health`, {
        timeout: 3000,
      });
  ...
  async testAutoServerStart() {
    await this.runTest('Auto Server Start Simulation', async () => {
      ...
      const finalCheck = await axios.get(`${this.mcpServerUrl}/health`, {
        timeout: 3000,
      });
  ...
  async testHealthMonitoring() {
    await this.runTest('Health Monitoring System', async () => {
      ...
      const statusResponse = await axios.get(`${this.webhookUrl}/mcp-status`, {
        timeout: 5000,
      });
  ...
  async simulateChatScenarios() {
    ...
    const scenarios = [
        ...
        activities: [
          {
            ...
            details: 'Explain this function',
            type: 'code_explanation',
          },
          {
            source: 'vscode_copilot',
            details: 'Generate unit tests',
      ...
      },
      {
        ...
        activities: [
          {
            source: 'code_review_chat',
            details: 'Review pull request #123',
            type: 'code_review',
          },
          {
            source: 'code_review_chat',
    ...
    // Final verification
    try {
      const finalStatus = await axios.get(`${this.mcpServerUrl}/health`, {
        timeout: 3000,
      });
