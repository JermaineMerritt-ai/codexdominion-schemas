...
from codex_ceremonial_inscriber import CodexCeremonialInscriber, inscribe_ceremony
...
Examples:
  ...
  %(prog)s --list-kind blessing
    ...
    parser.add_argument("kind", nargs="?", choices=['proclamation', 'silence', 'blessing'],
                       help="Type of ceremonial inscription")
    parser.add_argument("message", nargs="?", help="Sacred message to inscribe")
    
    # Information commands
    parser.add_argument("--list-recent", type=int, metavar="N", help="List N most recent ceremonial inscriptions")
    parser.add_argument("--list-kind", choices=['proclamation', 'silence', 'blessing'],
                       help="List inscriptions of specific kind")
    parser.add_argument("--show-status", action="store_true", help="Show ceremonial system status")
    ...
    try:
        ...
        if args.kind and args.message:
            ...
            print(f"📚 Retrieving {args.list_recent} most recent ceremonial inscriptions...")
            ...
            if ceremonial_file.exists():
                ...
                if recent_cycles:
                    ...
                    for cycle in recent_cycles:
                        kind_icon = {"proclamation": "📢", "silence": "🤫", "blessing": "🙏"}.get(cycle.get('kind', ''), '🕯️')
                        
                        print(f"{kind_icon} {cycle.get('cycle_id', 'unknown')}: {cycle.get('kind', 'unknown').upper()}")
            ...
            if ceremonial_file.exists():
                ...
                if kind_cycles:
                    kind_icon = {"proclamation": "📢", "silence": "🤫", "blessing": "🙏"}.get(args.list_kind, '🕯️')
                    print(f"\n{kind_icon} Found {len(kind_cycles)} {args.list_kind} inscriptions:")
                    print("=" * 60)
                    
                    for cycle in kind_cycles:
                        print(f"   {cycle.get('cycle_id', 'unknown')}: {cycle.get('timestamp', 'unknown')[:19]}")
                        print(f"   Message: {cycle.get('message', 'unknown')}")
                        print("-" * 30)
                else:
                ...
                try:
                    ...
                    if data.get('cycles'):
                        ...
                        for kind, count in kinds.items():
                            kind_icon = {"proclamation": "📢", "silence": "🤫", "blessing": "🙏"}.get(kind, '🕯️')
