...
def main():
    ...
    with col1:
        ...
        if st.button("🌟 Generate Ultimate Intelligence Report") and query:
            with st.spinner(
                f"Analyzing '{query}' across all 24 elite knowledge domains..."
            ):
                report = asyncio.run(
                    st.session_state.ultimate_intelligence.generate_ultimate_intelligence_report(
                        query
                    )
                )
                ...
                for insight in report["comprehensive_analysis"][
                    "advanced_technology_insights"
                ]:
                    st.markdown(
                        f"""
    ...
    with col2:
        ...
        for (
            cluster,
            domains,
        ) in st.session_state.ultimate_intelligence.all_domains.items():
