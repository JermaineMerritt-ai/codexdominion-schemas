...
st.set_page_config(page_title="📓 Codex Notebook Editor", page_icon="📓", layout="wide")
...
with col1:
    if st.button("📝 Add Text Cell"):
        cells.append(
            {"type": "text", "content": "", "created_at": datetime.now().isoformat()}
        )
...
with col2:
    if st.button("💻 Add Code Cell"):
        cells.append(
            {"type": "code", "content": "", "created_at": datetime.now().isoformat()}
        )
        st.session_state["cells"] = cells
        st.rerun()
...
with col3:
    if st.button("🤖 Add Prompt Cell"):
        cells.append(
            {"type": "prompt", "content": "", "created_at": datetime.now().isoformat()}
        )
        st.session_state["cells"] = cells
        st.rerun()
...
with st.sidebar:
    ...
    notebook_name = st.text_input(
        "Notebook Name", value=st.session_state.get("notebook_name", "codex_notebook")
    )
    ...
    if st.button("📄 Export to JSON", use_container_width=True):
        export_data = {"metadata": notebook_metadata, "cells": cells}
...
st.markdown("---")
st.markdown(
    "🔥 **Codex Dominion Suite - Notebook Editor** | *Cycle 1 - Interactive Notebook Management*"
)
