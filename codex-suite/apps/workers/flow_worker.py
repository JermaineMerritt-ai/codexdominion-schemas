...
def run_publish_invocation(cfg):
    append_entry("invocations.json", "invocations", {"role": cfg.get("role","Custodian"), "text": cfg.get("text","")})

def run_publish_ledger(cfg):
    append_entry("ledger.json", "entries", {"role": "Dispatch", "proclamation": cfg.get("proclamation","")})

def run_email(cfg):
    # Stub: write to ledger instead of sending email
    append_entry("ledger.json", "entries", {"role": "Email", "proclamation": f"To {cfg['to']}: {cfg['subject']}"})
