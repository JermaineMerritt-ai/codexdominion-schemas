...
try:
    from core.ledger import append_entry, load_json, save_json
except ImportError:
...
def avatar(role="Custodian"):
    ...
    if st.sidebar.button("🔥 Begin Setup", type="primary"):
        ...
        if role == "Custodian":
            st.sidebar.info(
                "💡 Tip: Start with System Status in the sidebar to verify all components are operational."
            )
        elif role == "Heir":
            st.sidebar.info(
                "💡 Tip: Visit the Council Access tab to submit your first proclamation."
            )
        elif role == "Council":
            st.sidebar.info("💡 Tip: Check the Council Access tab for pending reviews.")
        elif role == "Developer":
            st.sidebar.info(
                "💡 Tip: Check the terminal for any dependency installation needs."
            )
        else:
            st.sidebar.info(
                "💡 Tip: Explore each tab to discover all Codex capabilities."
            )
    ...
    try:
        ...
        if role == "Custodian":
            st.sidebar.metric(
                "System Health", "Optimal" if ledger_data else "Needs Setup"
            )
    ...
    current_wisdom = wisdom_quotes.get(
        role, "The flame eternal burns in every heart that seeks digital sovereignty."
    )
...
def main():
    ...
    """
    st.set_page_config(page_title="Codex Avatar System", page_icon="✨", layout="wide")
    ...
    with col1:
        ...
        st.write(
            "The Codex Avatar provides personalized guidance based on your role within the digital sovereignty ecosystem."
        )
        ...
        st.markdown(
            "*Select a role in the sidebar to see personalized avatar guidance.*"
        )
