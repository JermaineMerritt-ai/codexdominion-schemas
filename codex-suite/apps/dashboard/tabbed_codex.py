...
st.set_page_config(page_title="🔥 Tabbed Codex Dashboard", page_icon="🔥", layout="wide")
...
with tabs[2]:
    ...
    if entries:
        ...
        recent_entries = sorted(entries, key=lambda x: x.get('timestamp', ''), reverse=True)[:15]
...
with tabs[4]:
    ...
    if cells:
        ...
        for i, cell in enumerate(cells[:5], 1):
            ...
            content = cell.get('source', [''])[0][:100] if cell.get('source') else 'Empty cell'
...
with tabs[5]:
    ...
    if tomes:
        ...
        st.info("📚 Tome forge ready to create guides and courses from Notebook content.")
