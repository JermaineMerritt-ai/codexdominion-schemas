...
st.set_page_config(
    page_title="Festival Script - Seasonal Invocations",
    page_icon="🎇",
    layout="wide"
)

# Cosmic header with seasonal styling
current_season = ["Winter", "Winter", "Spring", "Spring", "Spring", "Summer", "Summer", "Summer", "Autumn", "Autumn", "Autumn", "Winter"][datetime.now().month - 1]
season_colors = {
...
st.write("Here the eternal cycles are honored through sacred invocations, marking the passage of time in the Codex flame.")
...
with col1:
    ...
    role_options = ["Custodian", "Heirs", "Councils", "Cosmos", "Elements", "Ancestors", "Future Selves"]
    ...
    ritual_type = st.selectbox(
        "🔮 Ritual Type:",
        ritual_options,
        help="The nature of this cosmic ceremony"
    )
    ...
    text = st.text_area(
        ...
        placeholder=cosmic_prompts.get(cycle, "By flame and cosmos, let this invocation echo through eternity...")
    )
    ...
    with st.expander("🌟 Advanced Cosmic Settings"):
        ...
        with col_time1:
            cosmic_timing = st.select_slider(
                "⏰ Cosmic Timing:",
                options=["Immediate", "Next Cycle", "Peak Power", "Sacred Hour", "Eternal"],
                value="Sacred Hour"
            )
        ...
        cosmic_reach = st.multiselect(
            "🌌 Cosmic Reach:",
            ["Local Flame", "Regional Cosmos", "Global Consciousness", "Galactic Network", "Universal Field"],
            default=["Local Flame"]
        )
        ...
        festival_elements = st.multiselect(
            "🎆 Festival Elements:",
            ["sacred_fire", "cosmic_music", "stellar_dance", "elemental_blessing", "ancestor_honor", "future_vision"],
...
with col2:
    ...
    moon_phase = ["🌑", "🌒", "🌓", "🌔", "🌕", "🌖", "🌗", "🌘"][min(day_of_month // 4, 7)]
    st.markdown(f"""
    ...
    try:
        ...
        festival_invocations = [p for p in proclamations_data.get("proclamations", []) 
                              if p.get("cycle") in cycle_options][-3:]
    ...
    try:
        total_festivals = len([p for p in proclamations_data.get("proclamations", []) 
                             if p.get("cycle") in cycle_options])
        st.metric("Total Festivals", total_festivals)
        
        # Seasonal distribution
        seasonal_count = len([p for p in proclamations_data.get("proclamations", []) 
...
with col_cosmic2:
    if st.button("🎇 Inscribe into Cosmic Codex", type="primary", use_container_width=True):
        if not text.strip():
            st.error("❌ Sacred invocation text cannot be empty. The cosmos awaits your words.")
        else:
            ...
            if result:
                ...
                st.success(f"🌟 {cycle} {ritual_type} successfully inscribed into the Cosmic Codex!")
                ...
                st.error("❌ Cosmic inscription failed. The universe may not be aligned. Please try again.")
