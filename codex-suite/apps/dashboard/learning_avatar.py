...
def create_interactive_tutorial(panel, cosmic_data):
    ...
    if steps:
        step_choice = st.selectbox(
            ...
            ["Select step..."] + [f"{i+1}. {step[0]}" for i, step in enumerate(steps)],
        )
        ...
        if step_choice != "Select step...":
            ...
            if panel == "Heartbeat" and step_index == 0:
                ...
                proclamation_type = st.selectbox(
                    "Choose type:", ["Blessing", "Silence", "Affirmation"]
                )
...
def get_jermaine_response(user_message, context="general"):
    ...
    if any(word in user_lower for word in ["hello", "hi", "greetings", "hey"]):
        ...
        word in user_lower
        for word in ["proclamation", "blessing", "silence", "proclaim"]
    ):
        return random.choice(responses["proclamations"])
    elif any(
        word in user_lower for word in ["ledger", "chronicle", "record", "history"]
    ):
        ...
        word in user_lower for word in ["help", "assist", "guide", "stuck", "confused"]
    ):
        return random.choice(responses["help"])
    elif any(
        word in user_lower
        for word in ["good", "great", "awesome", "amazing", "progress"]
    ):
...
def learning_avatar_sidebar(cosmic_data):
    ...
    if 5 <= hour < 12:
        ...
        time_wisdom = (
            "Dawn energy is perfect for new proclamations and fresh initiatives."
        )
    elif 12 <= hour < 17:
        greeting = "☀️ Good afternoon"
        time_wisdom = (
            "Midday power is ideal for executing commands and managing systems."
        )
    elif 17 <= hour < 21:
        ...
        greeting = "🌙 Good evening"
        time_wisdom = (
            "Night contemplation - excellent for deep learning and system planning."
        )
    ...
    **🌟 Today's Wisdom:** {time_wisdom}
    ...
    st.sidebar.caption(
        f"Progress: {int(progress_percent)}% • {len(panels_learned)}/{len(all_panels)} panels"
    )
    ...
    if "chat_history" not in st.session_state:
        st.session_state.chat_history = [
            {
                "role": "jermaine",
                "message": "🌟 Welcome! I'm Jermaine, your cosmic guide. Ask me anything about digital sovereignty!",
            }
        ]

    # Chat input
    user_message = st.sidebar.text_input(
        "💭 Ask Jermaine:", placeholder="Type your question or greeting..."
    )
    ...
    if st.sidebar.button("💬 Send") and user_message:
        ...
        st.session_state.chat_history.append(
            {"role": "jermaine", "message": jermaine_response}
        )
...
def learning_avatar_interface():
    ...
    st.set_page_config(page_title="✨ Learning Avatar", page_icon="✨", layout="wide")
    ...
    main_tab1, main_tab2 = st.tabs(
        ["💬 Chat with Jermaine", f"📚 Learn About {selected_panel}"]
    )
    ...
    with main_tab1:
        ...
        with chat_col1:
            ...
            # Main chat input
            st.markdown("---")
            main_user_message = st.text_area(
                "💭 Continue your conversation with Jermaine:",
            ...
            with col1:
                if (
                    ...
                    st.session_state.chat_history.append(
                        {"role": "user", "message": main_user_message}
                    )

                    # Get Jermaine's response
                    jermaine_response = get_jermaine_response(
                        main_user_message, selected_panel.lower()
                    )
                    st.session_state.chat_history.append(
                        {"role": "jermaine", "message": jermaine_response}
                    )
            ...
            with col2:
                if st.button("🔄 Clear Chat"):
                    st.session_state.chat_history = [
                        {
                            "role": "jermaine",
                            "message": "🌟 Fresh start! What would you like to explore in digital sovereignty?",
                        }
                    ]
                    st.rerun()
        ...
        with chat_col2:
            ...
            for topic in topics:
                if st.button(topic, key=f"topic_{topic}"):
                    ...
                    st.session_state.chat_history.append(
                        {"role": "user", "message": topic_message}
                    )

                    jermaine_response = get_jermaine_response(
                        topic_message, selected_panel.lower()
                    )
                    st.session_state.chat_history.append(
                        {"role": "jermaine", "message": jermaine_response}
                    )
    ...
    with main_tab2:
        ...
        if content:
            ...
            tab1, tab2, tab3, tab4 = st.tabs(
                ["🧠 Concepts", "💡 Practical Tips", "🚀 Advanced", "🎯 Tutorial"]
            )
    ...
    if st.session_state.get("show_assessment"):
        ...
        question = assessment_questions.get(
            selected_panel, "What have you learned about this panel?"
        )
        ...
        if st.button("✅ Submit Assessment"):
            if user_answer:
                ...
                st.success(
                    "🎯 Assessment completed! Your learning progress has been recorded."
                )
    ...
    if st.session_state.get("show_summary"):
        ...
        if learning_progress:
            ...
            for panel, entries in panels_studied.items():
                with st.expander(
                    f"📖 {panel} Learning Summary ({len(entries)} activities)"
                ):
