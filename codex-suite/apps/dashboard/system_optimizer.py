...
sys.path.append('..')
sys.path.append('../..')
from core.memory_optimizer import optimize_memory, monitor_memory_usage, optimize_streamlit_memory
...
with col1:
    ...
    if st.button("📊 Memory Report"):
        ...
        for proc in memory_stats['top_processes'][:8]:
            st.write(f"**{proc['name']}**: {proc['memory_mb']} MB ({proc['memory_percent']}%)")
