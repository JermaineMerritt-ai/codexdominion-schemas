# apps/dashboard/treasury_dashboard.py
import streamlit as st
from core.ledger import load_json, append_entry

st.title("💎 Treasury Dashboard Blueprint")

tabs = st.tabs(["Stores", "Social Media", "Websites", "Council Oversight", "Custodian Approval"])

with tabs[0]:
    st.header("🏬 Stores")
    stores = load_json("stores.json", {"transactions":[]})["transactions"]
    for s in stores:
        st.write(f"{s['timestamp']} — {s['item']} → ${s['amount']}")

with tabs[1]:
    st.header("📱 Social Media")
    social = load_json("social.json", {"transactions":[]})["transactions"]
    for sm in social:
        st.write(f"{sm['timestamp']} — {sm['platform']} → ${sm['amount']}")

with tabs[2]:
    st.header("🌐 Websites")
    sites = load_json("websites.json", {"transactions":[]})["transactions"]
    for w in sites:
        st.write(f"{w['timestamp']} — {w['source']} → ${w['amount']}")

with tabs[3]:
    st.header("👥 Council Oversight")
    concord = st.text_area("Council Concord")
    if st.button("Add Concord"):
        append_entry("ledger.json", "entries", {
            "role": "Council",
            "proclamation": concord
        })
        st.success("Council concord inscribed.")

with tabs[4]:
    st.header("👑 Custodian Approval")
    total = sum([s['amount'] for s in stores] +
                [sm['amount'] for sm in social] +
                [w['amount'] for w in sites])
    if st.button("Crown Balance"):
        append_entry("ledger.json", "entries", {
            "role": "Custodian",
            "proclamation": f"Sovereign Balance Crowned: ${total}"
        })
        st.success(f"Sovereign Balance Crowned: ${total}")