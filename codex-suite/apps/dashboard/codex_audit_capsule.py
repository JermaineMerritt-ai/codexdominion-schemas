...
def create_audit_capsule():
    ...
    st.set_page_config(
        page_title="🧩 Codex Audit Capsule",
        page_icon="🧩",
        layout="wide"
    )
...
def display_company_mapping():
    ...
    st.markdown("*Begin the ceremonial audit sequence by mapping the sovereign domain.*")
    ...
    with col1:
        ...
        with st.form("company_profile"):
            company_name = st.text_input("🏰 Company Name/Alias:", placeholder="Enter sovereign domain name")
            industry = st.selectbox("🏭 Industry/Sector:", [
                "Retail & E-commerce", "Manufacturing", "SaaS/Technology", "Logistics & Supply Chain",
                "Healthcare", "Financial Services", "Real Estate", "Hospitality", "Consulting",
                "Education", "Energy", "Agriculture", "Construction", "Other"
            ])
            
            revenue_streams = st.multiselect("💰 Primary Revenue Streams:", [
                "Product Sales", "Service Fees", "Subscriptions", "Licensing", "Consulting",
                "Advertising", "Affiliate", "Real Estate", "Investment Returns", "Grants"
            ])
            
            pain_points = st.text_area("⚠️ Known Pain Points/Loss Areas:", 
                placeholder="Describe suspected inefficiencies, losses, or operational challenges...")
            
            annual_revenue = st.number_input("📈 Annual Revenue Estimate ($):", min_value=0, step=1000)
            employee_count = st.number_input("👥 Employee Count:", min_value=1, step=1)
            
            submitted = st.form_submit_button("🔥 **Forge Profile**", use_container_width=True)
            ...
            if submitted and company_name:
                ...
                st.success(f"🎉 Profile forged for **{company_name}**! Ready for audit sequence.")
                st.rerun()
...
def display_loss_matrix():
    ...
    if not profiles:
        st.warning("⚠️ No company profiles found. Create a profile first in the Company Mapping tab.")
        return
    
    # Profile selection
    profile_options = {f"{p['company_name']} ({p['industry']})": p for p in profiles}
    selected_profile_name = st.selectbox("🏰 Select Company Profile:", list(profile_options.keys()))
    ...
    if selected_profile_name:
        ...
        with col1:
            ...
            loss_categories = {
                "Operational Inefficiency": st.slider("⚙️ Operational Inefficiency (%)", 0, 50, 15),
                ...
                "Marketing Misallocation": st.slider("📢 Marketing Waste (%)", 0, 30, 9)
            }
            
            # Calculate total loss
            total_loss_percent = sum(loss_categories.values())
            annual_loss = (selected_profile['annual_revenue'] * total_loss_percent) / 100
        ...
        with col2:
            ...
            # Create pie chart
            loss_df = pd.DataFrame(list(loss_categories.items()), columns=['Category', 'Percentage'])
            ...
            for category, percentage in loss_categories.items():
                if percentage > 0:
                    category_loss = (selected_profile['annual_revenue'] * percentage) / 100
...
def display_root_cause_scroll():
    ...
    # Profile selection
    profile_options = {f"{p['company_name']} ({p['industry']})": p for p in profiles}
    selected_profile_name = st.selectbox("🏰 Select Company Profile:", list(profile_options.keys()), key="root_cause_profile")
    ...
    if selected_profile_name:
        ...
        with col1:
            ...
            # Analysis categories
            categories = {
                "Leadership & Strategy": st.checkbox("👑 Leadership & Strategic Direction Issues"),
                "Process & Operations": st.checkbox("⚙️ Process Inefficiencies & Operational Gaps"),
                "Technology & Systems": st.checkbox("💻 Technology Deficiencies & System Failures"),
                "Human Resources": st.checkbox("👥 Human Resource Management Issues"),
                "Financial Management": st.checkbox("💰 Financial Controls & Cash Flow Problems"),
                "Market Position": st.checkbox("📈 Market Positioning & Competitive Disadvantages"),
                "Customer Relations": st.checkbox("🤝 Customer Retention & Service Issues"),
                "Vendor/Supply Chain": st.checkbox("🚚 Vendor Relations & Supply Chain Disruptions")
            }
            
            if st.button("🔍 **Generate Root Cause Analysis**", use_container_width=True):
                selected_categories = [cat for cat, selected in categories.items() if selected]
                
                if selected_categories:
                    st.markdown("#### **🧩 Diagnostic Scroll**")
                    
                    analysis_text = generate_root_cause_analysis(selected_profile, selected_categories)
                    ...
                    if st.button("📜 **Inscribe Analysis to Codex**"):
                        append_entry("audit_analyses.json", "analyses", {
...
def display_solution_engine():
    ...
    profile_options = {f"{p['company_name']} ({p['industry']})": p for p in profiles}
    selected_profile_name = st.selectbox("🏰 Select Company Profile:", list(profile_options.keys()), key="solution_profile")
    ...
    if selected_profile_name:
        ...
        with col1:
            ...
            immediate = st.multiselect("Quick Wins:", [
                "Cash Flow Analysis", "Cost Reduction Audit", "Process Mapping",
                "Technology Assessment", "Staff Productivity Review", "Vendor Renegotiation"
            ], key="immediate")
        
        with col2:
            st.markdown("##### **Strategic Initiatives (1-6 months)**")
            strategic = st.multiselect("Medium-term Solutions:", [
                "System Implementation", "Process Reengineering", "Team Restructuring",
                "Market Repositioning", "Product Development", "Partnership Development"
            ], key="strategic")
        
        with col3:
            st.markdown("##### **Transformation Programs (6+ months)**")
            transformation = st.multiselect("Long-term Programs:", [
                "Digital Transformation", "Cultural Change", "Market Expansion",
                "Acquisition Strategy", "Innovation Program", "Sustainability Initiative"
            ], key="transformation")
        ...
        if st.button("🔥 **Generate Solution Blueprint**", use_container_width=True):
            ...
            if st.button("⚙️ **Forge Solution to Codex**"):
                append_entry("audit_solutions.json", "solutions", {
                    "company_name": selected_profile['company_name'],
...
def generate_solution_blueprint(profile, immediate, strategic, transformation):
    ...
    estimated_recovery = min(50, len(immediate) * 5 + len(strategic) * 10 + len(transformation) * 15)
...
def display_replay_mode():
    ...
    st.markdown("*Blueprint for future councils to reuse and adapt audit methodologies.*")
    ...
    with col1:
        ...
        with st.form("replay_template"):
            template_name = st.text_input("🎯 Template Name:", placeholder="e.g., SaaS Turnaround Protocol")
            template_description = st.text_area("📝 Description:", placeholder="Describe when and how to use this template...")
            
            industry_focus = st.multiselect("🏭 Industry Applicability:", [
                "Retail & E-commerce", "Manufacturing", "SaaS/Technology", "Logistics",
                "Healthcare", "Financial Services", "Real Estate", "Hospitality",
                "Consulting", "Education", "Energy", "Agriculture", "Construction"
            ])
            
            template_steps = st.text_area("🔄 Replay Steps:", 
            ...
            if submitted and template_name:
                replay_template = {
                    "id": len(load_json("replay_templates.json", {"templates": []})["templates"]) + 1,
                    "name": template_name,
...
def display_capsule_forge():
    ...
    with col1:
        ...
        profile_options = {f"{p['company_name']} ({p['industry']})": p for p in profiles}
        selected_profile_name = st.selectbox("🏰 Select Audit to Immortalize:", list(profile_options.keys()))
        
        if selected_profile_name:
            selected_profile = profile_options[selected_profile_name]
            
            # Capsule customization
            capsule_title = st.text_input("👑 Capsule Title:", 
                value=f"{selected_profile['company_name']} Turnaround Capsule")
            
            capsule_crest = st.text_input("🏛️ Capsule Crest/Symbol:", 
                placeholder="e.g., ⚡🔥💎")
            
            capsule_motto = st.text_input("📜 Capsule Motto:", 
                placeholder="e.g., 'From Chaos to Codex'")
            
            # Enhancement options
            st.markdown("#### **📋 Capsule Enhancements**")
            
            include_contributors = st.checkbox("👥 Include Contributor Recognition Scroll")
            include_timeline = st.checkbox("📅 Include Implementation Timeline")
            include_roi_projection = st.checkbox("💰 Include ROI Projections")
            format_for_funding = st.checkbox("🏦 Format for Investor/Funder Presentation")
            
            if st.button("🔥 **Forge Immortal Capsule**", use_container_width=True):
                capsule_content = generate_legacy_capsule(
                    selected_profile, analyses, solutions, capsule_title, 
                    capsule_crest, capsule_motto, include_contributors, 
                    include_timeline, include_roi_projection, format_for_funding
                )
...
def generate_legacy_capsule(profile, analyses, solutions, title, crest, motto, 
                          include_contributors, include_timeline, include_roi, format_funding):
    ...
    company_analyses = [a for a in analyses if a['company_name'] == profile['company_name']]
    if company_analyses:
        capsule += """
        <div class="loss-matrix">
    ...
    company_solutions = [s for s in solutions if s['company_name'] == profile['company_name']]
    if company_solutions:
        latest_solution = company_solutions[-1]
        total_actions = (len(latest_solution.get('immediate_actions', [])) + 
    ...
    if include_roi:
        estimated_recovery = min(profile['annual_revenue'] * 0.3, 500000)  # Cap at 500k or 30%
        capsule += f"""
        <div class="replay-capsule">
...
def display_audit_archive():
    ...
    # Archive tables
    tab1, tab2, tab3 = st.tabs(["📋 All Audits", "📈 Performance Metrics", "🏛️ Legacy Capsules"])
    
    with tab1:
        if profiles:
            df_profiles = pd.DataFrame(profiles)
            df_profiles['created_date'] = pd.to_datetime(df_profiles['created_date']).dt.strftime('%Y-%m-%d')
            st.dataframe(df_profiles[['company_name', 'industry', 'annual_revenue', 'employee_count', 'created_date']], 
