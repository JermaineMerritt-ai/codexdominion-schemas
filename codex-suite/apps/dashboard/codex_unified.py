...
try:
    ...
    if (root_dir / "codex_models.py").exists():
        from codex_models import (CodexDataManager, Constellation,
                                  ConstellationStar, LedgerEntry, Proclamation,
                                  Status, Stream, Transaction)
...
st.set_page_config(
    page_title="🔥 Codex Dominion Unified Dashboard", page_icon="🔥", layout="wide"
)
        ...
        with col2:
            ...
            if MODELS_AVAILABLE:
                ...
                with col2_1:
                    st.metric(
                        "💰 Total Revenue",
                        f"${revenue_summary.get('grand_total', 0):.2f}",
                    )
                    st.metric(
                        "📊 Transactions", revenue_summary.get("total_transactions", 0)
                    )

                with col2_2:
                    st.metric(
                        "⭐ Constellations",
                        f"${revenue_summary.get('constellation_total', 0):.2f}",
                    )
                    stream_totals = revenue_summary.get("stream_totals", {})
                    st.metric(
                        "🏪 Store Revenue", f"${stream_totals.get('store', 0):.2f}"
                    )
                ...
                with st.expander("💳 Add Revenue Transaction"):
                    ...
                    with trans_col1:
                        source = st.selectbox(
                            "Revenue Stream", ["store", "social", "website"]
                        )
                    ...
                    if st.button("💰 Add Transaction", type="primary"):
                        try:
                            ...
                            if data_manager.save_transaction(transaction):
                                st.success(
                                    f"✅ Transaction added: ${amount:.2f} from {source}"
                                )
                                st.balloons()
                                st.rerun()
                            else:
                ...
                constellation_data = load_data_safe(
                    "constellations.json", {"constellations": []}
                )

                st.info(f"Ledger Entries: {len(ledger_data.get('entries', []))}")
                st.success(
                    f"Constellations: {len(constellation_data.get('constellations', []))}"
                )
            ...
            with st.expander("📝 Enhanced Quick Entry"):
                ...
                entry_type = st.selectbox(
                    "Entry Type", ["manual_entry", "spark", "insight", "note", "idea"]
                )
                    ...
                    if entry_text.strip():
                        if UTILS_AVAILABLE:
                            ...
                            if append_entry(
                                ...
                                st.success(
                                    "✅ Entry added to ledger using enhanced utilities!"
                                )
                                st.balloons()
                                st.rerun()
                            else:
    ...
    with tabs[3]:
        ...
        with col1:
            ...
            if st.button("� Launch Full AI Studio", type="primary"):
                ...
                st.info(
                    "💡 **Tip**: The full AI Development Studio provides advanced features like:\n"
                    "- Live code preview\n"
                    "- AI coding assistant\n"
                    ...
                    try:
                        if MODELS_AVAILABLE:
                            ...
                            if data_manager.save_proclamation(proclamation):
                                st.success(
                                    f"🔥 {ritual_type} INSCRIBED INTO THE ETERNAL FLAME! 🔥"
                                )
                                st.balloons()
                                ...
                                *By the eternal flame, this proclamation is forever inscribed in the Codex.*
                                ...
                                st.error(
                                    "❌ Failed to inscribe ritual. The flame flickers..."
                                )
                        else:
                            # Fallback to basic functionality
                            proc_data = load_data_safe(
                            ...
                            proc_data.setdefault("proclamations", []).append(
                                new_proclamation
                            )
                            ...
                            st.success(
                                f"✨ {ritual_type} inscribed into the Codex flame!"
                            )
                            st.balloons()
