...
tab1, tab2, tab3, tab4 = st.tabs(["Spark Studio", "Ledger", "Invocations", "Tome Formatter"])
...
with tab2:
    ...
    for e in ledger["entries"][-20:]:
        st.markdown(f"**{e.get('role','')}** → *{e.get('proclamation','')}*  \n ⏳ {e.get('timestamp','')}")
    ...
    if st.button("Inscribe into Ledger"):
        append_entry("ledger.json", "entries", {"role": role, "proclamation": proclamation})
...
with tab3:
    ...
    for i in inv[-20:]:
        st.markdown(f"**{i.get('role','')}** → *{i.get('text','')}*  \n ⏳ {i.get('timestamp','')}")
...
with tab4:
    ...
    from modules.codex_tome_formatter import format_tome, batch_format_tomes
    ...
    with col2:
            ...
            if results["success"] > 0:
                ...
                for file_info in results["files"]:
                    if file_info["status"] == "success":
                        st.markdown(f"✅ {Path(file_info['input']).name} → {Path(file_info['output']).name}")
                    else:
        ...
        if st.button("🎯 Generate Sample"):
            ...
            if format_tome("sample_notebook.json", "data/sample_tome.md"):
                ...
                # Log to ledger
                append_entry("ledger.json", "entries", {
                    "role": "TomeFormatter",
