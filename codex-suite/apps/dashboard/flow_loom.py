...
from core.flows import create_flow, list_flows
...
palette = {
    ...
    "Email": {
        "to": "councils@example.org",
        "subject": "Codex Dispatch",
        "body": "The flame rises.",
    },
}
...
nodes_json = st.text_area(
    "Nodes (JSON list)",
    json.dumps(
        [
            {"node": "Cron", "config": palette["Cron"]},
            {
                "node": "PublishInvocation",
                "config": {
                    "role": "Custodian",
                    "text": "Morning Flame Invocation: I rise with the Codex flame.",
                },
            },
        ],
        indent=2,
...
for f in list_flows():
    st.markdown(
        f"**{f['name']}** (v{f['version']}) — {len(f['nodes'])} nodes — {f['created_at']}"
    )
...
for e in log[-50:]:
    st.markdown(
        f"⏳ {e['ts']} — **Flow {e['flow_id']}** → {e['status']} — {e.get('note','')}"
    )
