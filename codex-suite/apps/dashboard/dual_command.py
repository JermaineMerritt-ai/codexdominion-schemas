    ...
    if role == "Custodian":
        ...
        if st.sidebar.button("📚 Study Tomes"):
            st.session_state["quick_command"] = (
                "study:tome digital sovereignty foundations"
            )
        ...
        if st.sidebar.button("🤝 Review Decisions"):
            st.session_state["quick_command"] = (
                "review:decisions recent custodian actions"
            )
        if st.sidebar.button("💡 Advise Custodian"):
            st.session_state["quick_command"] = (
                "advise:custodian system optimization recommendations"
            )
        if st.sidebar.button("👁️ Oversee Operations"):
            st.session_state["quick_command"] = (
                "oversee:operations daily system functions"
            )
...
def dual_command_interface():
    ...
    st.set_page_config(
        page_title="⚔️ Dual Command Console", page_icon="⚔️", layout="wide"
    )
    ...
    with col1:
        if st.button(
            "👑 **CUSTODIAN**",
            type="primary",
            help="Supreme sovereignty with full system control",
        ):
    ...
    with col2:
        if st.button(
            "🌱 **HEIR**",
            type="secondary",
            help="Learning authority with guided responsibilities",
        ):
    ...
    with col3:
        if st.button(
            "⚖️ **COUNCIL**",
            type="secondary",
            help="Collective wisdom with oversight authority",
        ):
    ...
    with col2:
        ...
        role_commands = [
            cmd for cmd in command_history if cmd.get("role") == selected_role
        ]
        ...
        role_entries = [
            entry for entry in ledger_entries if entry.get("role") == selected_role
        ]
    ...
    if st.button(f"⚔️ **EXECUTE AS {selected_role.upper()}**", type="primary"):
        if command:
            ...
            if success:
                st.balloons()
                st.success(
                    f"🌟 Command successfully inscribed into Codex flame as {selected_role}"
                )
    ...
    role_entries = [
        entry
        for entry in cosmic_data["ledger"].get("entries", [])
        if entry.get("role") == selected_role
    ]

    if role_entries:
        with st.expander(
            f"📊 Last {min(5, len(role_entries))} {selected_role} Actions"
        ):
            for entry in reversed(role_entries[-5:]):
                ...
                with col2:
                    proclamation = entry.get(
                        "proclamation", entry.get("command_content", "")
                    )
        ...
        st.info(
            f"📭 No {selected_role} commands executed yet. Begin with your first command!"
        )
