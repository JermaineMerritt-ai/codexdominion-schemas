...
def save_accounts(accounts_data):
    ...
    hierarchy_data["metadata"] = {
        "total_members": len(accounts_data["custodians"]) + len(accounts_data["heirs"]) + len(accounts_data["customers"]),
...
# Set up page config
st.set_page_config(
    page_title="🌟 Custodian–Heirs Concord Panel",
    page_icon="🏛️",
    layout="wide"
)
...
with col4:
    total_members = len(accounts["custodians"]) + len(accounts["heirs"]) + len(accounts["customers"])
    ...
    with col2:
        if st.button("🔥 Induct Heir", type="primary"):
            if new_heir.strip():
                ...
                st.success(f"⚡ Heir {new_heir} successfully inducted into the sacred hierarchy!")
                st.balloons()
                st.markdown(f"""
...
with tab3:
    ...
    with col1:
        new_customer = st.text_input("👤 New Customer Name", placeholder="Enter customer name...")
        
    with col2:
        if st.button("🌍 Welcome Customer", type="primary"):
            if new_customer.strip():
                # Create complete customer profile
                customer_profile = create_customer_profile(new_customer, len(accounts["customers"]))
                accounts["customers"].append(customer_profile)
                save_accounts(accounts)
                
                st.success(f"🌟 Customer {new_customer} warmly welcomed to the Codex Dominion!")
                st.balloons()
                st.markdown(f"""
...
with st.sidebar:
    ...
    total = len(accounts["custodians"]) + len(accounts["heirs"]) + len(accounts["customers"])
...
# Footer
st.markdown("---")
st.markdown("**🌟 Custodian–Heirs Concord Panel** - *Sacred Hierarchy Management for Digital Sovereignty*")