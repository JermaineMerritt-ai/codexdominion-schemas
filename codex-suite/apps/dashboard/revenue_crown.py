...
def create_revenue_metrics(stores, social, websites):
    ...
    def calculate_period_revenue(transactions, start_date, end_date):
        ...
        for t in transactions:
            try:
                t_date = datetime.fromisoformat(t.get('timestamp', '').replace('Z', '+00:00'))
    ...
    previous_websites = calculate_period_revenue(websites, previous_30_days, last_30_days)
    ...
    total_growth = safe_growth_rate(recent_stores + recent_social + recent_websites,
...
def create_revenue_visualizations(stores, social, websites):
    ...
    fig_timeline.update_layout(
        height=600,
        title_text="💎 Revenue Crown Analytics Dashboard",
        showlegend=True
    )
...
def display_stores_tab(stores):
    ...
    if not stores:
        ...
        with st.expander("➕ Add New Store Transaction"):
            ...
            amount = st.number_input("Sale Amount ($):", min_value=0.0, step=0.01, key="store_amount_1")
    ...
    if not stores_df.empty:
        stores_df['formatted_timestamp'] = pd.to_datetime(stores_df['timestamp']).dt.strftime('%Y-%m-%d %H:%M')
    ...
    with st.expander("➕ Add New Store Transaction"):
        ...
        amount = st.number_input("Sale Amount ($):", min_value=0.0, step=0.01, key="store_amount_2")
...
def display_social_media_tab(social):
    ...
    if not social:
        ...
        with st.expander("➕ Add New Social Media Revenue"):
            platform = st.selectbox("Platform:", ["Instagram", "TikTok", "YouTube", "Twitter/X", "Facebook", "LinkedIn", "Other"], key="social_platform_1")
            revenue_type = st.selectbox("Revenue Type:", ["Sponsored Post", "Affiliate Commission", "Brand Partnership", "Content Licensing", "Subscription", "Tip/Donation"], key="social_type_1")
            amount = st.number_input("Revenue Amount ($):", min_value=0.0, step=0.01, key="social_amount_1")
            ...
            if st.button("💰 Record Social Revenue", key="social_button_1"):
                if amount > 0:
                    ...
                    if append_entry("social.json", "transactions", new_transaction):
                        st.success(f"🎉 Social media revenue recorded: ${amount} from {platform}")
                        st.rerun()
        return
    ...
    top_platform = max(platforms.items(), key=lambda x: x[1]) if platforms else ("None", 0)
    ...
    if platforms:
        platform_df = pd.DataFrame(list(platforms.items()), columns=['Platform', 'Revenue'])
        fig_social = px.bar(
            platform_df,
            x='Platform',
            y='Revenue',
    ...
    if not social_df.empty:
        social_df['formatted_timestamp'] = pd.to_datetime(social_df['timestamp']).dt.strftime('%Y-%m-%d %H:%M')
    ...
    # Add new transaction form
    with st.expander("➕ Add New Social Media Revenue"):
        platform = st.selectbox("Platform:", ["Instagram", "TikTok", "YouTube", "Twitter/X", "Facebook", "LinkedIn", "Other"], key="social_platform_2")
        revenue_type = st.selectbox("Revenue Type:", ["Sponsored Post", "Affiliate Commission", "Brand Partnership", "Content Licensing", "Subscription", "Tip/Donation"], key="social_type_2")
        amount = st.number_input("Revenue Amount ($):", min_value=0.0, step=0.01, key="social_amount_2")
...
def display_websites_tab(websites):
    ...
    if not websites:
        ...
        with st.expander("➕ Add New Website Revenue"):
            ...
            revenue_type = st.selectbox("Revenue Type:", ["Product Sales", "Service Fees", "Subscription", "Advertising", "Affiliate", "Licensing", "Donations"], key="website_type_empty_1")
            amount = st.number_input("Revenue Amount ($):", min_value=0.0, step=0.01, key="website_amount_empty_1")
            transaction_id = st.text_input("Transaction ID (optional):", key="website_id_empty_1")
    ...
    if revenue_types:
        ...
        with col1:
            # Sources chart
            sources_df = pd.DataFrame(list(sources.items()), columns=['Source', 'Revenue'])
            fig_sources = px.pie(
                sources_df,
                values='Revenue',
                names='Source',
        ...
        with col2:
            # Revenue types chart
            types_df = pd.DataFrame(list(revenue_types.items()), columns=['Type', 'Revenue'])
            fig_types = px.bar(
                types_df,
                x='Type',
                y='Revenue',
    ...
    if not websites_df.empty:
        websites_df['formatted_timestamp'] = pd.to_datetime(websites_df['timestamp']).dt.strftime('%Y-%m-%d %H:%M')
    ...
    with st.expander("➕ Add New Website Revenue"):
        ...
        revenue_type = st.selectbox("Revenue Type:", ["Product Sales", "Service Fees", "Subscription", "Advertising", "Affiliate", "Licensing", "Donations"], key="website_type_main_1")
        amount = st.number_input("Revenue Amount ($):", min_value=0.0, step=0.01, key="website_amount_main_1")
        transaction_id = st.text_input("Transaction ID (optional):", key="website_id_main_1")
...
def revenue_crown_interface():
    ...
    st.set_page_config(
        page_title="💰 Codex Revenue Crown",
        page_icon="💰",
        layout="wide"
    )
    ...
    with tab5:
        ...
        with col1:
            ...
            breakdown_data = [
                ...
                ["━━━━━━━━━━━━━━━", "━━━━━━━━━━━━━"],
                ["👑 **TOTAL SOVEREIGNTY**", f"**${metrics['totals']['grand_total']:,.2f}**"]
            ]
        ...
        with col2:
            ...
            for item, value in growth_data:
                growth_color = "#32CD32" if "+" in value else "#FF6347" if "-" in value else "#FFF"
                st.markdown(f"{item}: <span style='color: {growth_color}'>{value}</span>", unsafe_allow_html=True)
        ...
        for transaction in all_recent[:10]:  # Show top 10
            try:
                formatted_time = datetime.fromisoformat(transaction['timestamp']).strftime('%Y-%m-%d %H:%M')
            except:
