    ...
    with col1:
        ...
        with st.form("meeting_setup"):
            ...
            meeting_type = st.selectbox("🏢 Meeting Type:", [
                "Team Standup", "Client Call", "Board Meeting", "Project Review",
                "One-on-One", "Interview", "Training Session", "Sales Call", "Other"
            ])
            
            participants = st.text_area("👥 Participants:", 
                placeholder="Enter participant names, one per line")
            
            meeting_agenda = st.text_area("📝 Agenda/Topics:", 
            ...
            audio_source = st.radio("Audio Source:", ["Microphone", "System Audio", "Upload File"])
            
            if audio_source == "Upload File":
                uploaded_file = st.file_uploader("Choose audio file", 
                    type=['mp3', 'wav', 'm4a', 'ogg'])
            
            start_recording = st.form_submit_button("🔴 **Start Recording**", use_container_width=True)
            
            if start_recording and meeting_title:
                # Simulate starting a recording session
                meeting_data = {
                    "id": len(load_json("meetings.json", {"meetings": []})["meetings"]) + 1,
                    "title": meeting_title,
                    "type": meeting_type,
...
def display_transcript_library():
    ...
    with col1:
        search_term = st.text_input("🔍 Search transcripts:", placeholder="Search by title, participant, or content...")
    ...
    if completed_meetings:
        ...
        for meeting in completed_meetings[::-1]:  # Reverse chronological
            if should_show_meeting(meeting, search_term, filter_type, date_range):
                with st.expander(f"📋 {meeting['title']} - {meeting.get('start_time', '')[:10]}"):
                    
                    col1, col2 = st.columns([2, 1])
                    
                    with col1:
                        st.markdown(f"**Type:** {meeting.get('type', 'Unknown')}")
                        st.markdown(f"**Duration:** {meeting.get('duration', 'Unknown')}")
        ...
        st.info("No completed meetings found. Start recording to build your transcript library!")
...
def display_ai_analysis():
    ...
    if not completed_meetings:
        st.warning("No completed meetings available for analysis. Record some meetings first!")
        return
    
    # Select meeting for analysis
    meeting_options = {f"{m['title']} - {m.get('start_time', '')[:10]}": m for m in completed_meetings}
    selected_meeting_name = st.selectbox("📋 Select meeting for analysis:", list(meeting_options.keys()))
    ...
    if selected_meeting_name:
        ...
        with col1:
            ...
            analysis_types = st.multiselect("Select analysis types:", [
                "Key Topics Extraction", "Sentiment Analysis", "Speaking Time Distribution",
                "Decision Points", "Follow-up Items", "Risk Assessment",
                "Meeting Effectiveness", "Participant Engagement"
            ])
            
            if st.button("🧠 **Generate AI Analysis**", use_container_width=True):
                if analysis_types:
                    analysis_results = generate_ai_analysis(selected_meeting, analysis_types)
                    ...
                    if st.button("💾 **Save Analysis**"):
                        append_entry("meeting_analyses.json", "analyses", {
        ...
        with col2:
            ...
            st.metric("📝 Word Count", estimate_word_count(selected_meeting.get('transcript', '')))
            
            # Recent analyses
            analyses = load_json("meeting_analyses.json", {"analyses": []})["analyses"]
            meeting_analyses = [a for a in analyses if a["meeting_id"] == selected_meeting["id"]]
            
            if meeting_analyses:
                st.markdown("**Previous Analyses:**")
                for analysis in meeting_analyses[-3:]:
                    st.markdown(f"📊 {', '.join(analysis['analysis_types'])}")
                    st.markdown(f"<small>{analysis['timestamp'][:10]}</small>", unsafe_allow_html=True)
...
def display_action_items():
    ...
    with col1:
        ...
        if all_action_items:
            ...
            filter_assignee = st.selectbox("Filter by assignee:", 
                ["All"] + list(set(item.get("assignee", "") for item in all_action_items)))
            ...
            for item in all_action_items:
                if should_show_action_item(item, filter_status, filter_assignee):
                    ...
                    if status == "completed":
                        status_color = "#28A745"
                        status_icon = "✅"
                    elif due_date and datetime.now() > datetime.fromisoformat(due_date + "T00:00:00"):
                        status_color = "#DC3545"
                        status_icon = "⚠️"
                        status = "overdue"
                    else:
                    ...
                    with col_a:
                        if st.button(f"✅ Complete", key=f"complete_{hash(item.get('task', ''))}"):
                            mark_action_complete(item)
                            st.rerun()
                    with col_b:
                        if st.button(f"📅 Reschedule", key=f"reschedule_{hash(item.get('task', ''))}"):
                            st.info("Reschedule feature coming soon!")
                    with col_c:
                        if st.button(f"💬 Add Note", key=f"note_{hash(item.get('task', ''))}"):
                            st.info("Notes feature coming soon!")
        else:
            st.info("No action items found. Record meetings with action items to see them here!")
    ...
    with col2:
        ...
        if all_action_items:
            ...
            completed_items = len([item for item in all_action_items if item.get("status") == "completed"])
            overdue_items = len([item for item in all_action_items 
                               if item.get("due_date") and datetime.now() > datetime.fromisoformat(item["due_date"] + "T00:00:00")])
            
            st.metric("📋 Total Items", total_items)
            st.metric("✅ Completed", completed_items, f"{(completed_items/total_items*100):.0f}%")
            st.metric("⚠️ Overdue", overdue_items)
            
            # Top assignees
            assignees = [item.get("assignee", "") for item in all_action_items if item.get("assignee")]
            if assignees:
...
def display_meeting_analytics():
    ...
    if not completed_meetings:
        st.warning("No meeting data available for analytics. Record some meetings first!")
        return
    
    # Time period selector
    time_period = st.selectbox("📅 Analytics Period:", [
    ...
    with col1:
        ...
        meeting_dates = [datetime.fromisoformat(m["start_time"]).date() for m in filtered_meetings]
    ...
    with col2:
        ...
        total_participants = sum(len(m.get("participants", [])) for m in filtered_meetings)
        ...
        if all_action_items:
            completed_actions = len([item for item in all_action_items if item.get("status") == "completed"])
...
def filter_meetings_by_period(meetings, period):
    ...
    return [m for m in meetings 
            if datetime.fromisoformat(m["start_time"]) > cutoff_date]
...
def display_settings():
    ...
    with col1:
        ...
        ai_analysis_level = st.selectbox("AI Analysis Level:", ["Basic", "Standard", "Advanced"])
    ...
    with col2:
        ...
        export_format = st.multiselect("Default Export Formats:", 
            ["PDF", "DOCX", "TXT", "JSON"], default=["PDF"])
        
        include_ai_summary = st.checkbox("Include AI Summary in Exports", value=True)
        include_action_items = st.checkbox("Include Action Items in Exports", value=True)
...
def display_knowledge_base():
    ...
    with col1:
        ...
        search_query = st.text_input("Search meetings and insights:", 
            placeholder="Search across all meetings, transcripts, and analyses...")
        
        search_type = st.selectbox("Search in:", [
            "All Content", "Meeting Titles", "Transcripts Only", 
            "AI Analyses", "Action Items", "Participants"
        ])
        
        if search_query:
            search_results = perform_knowledge_search(meetings, analyses, search_query, search_type)
    ...
    with col2:
        ...
        if meetings:
            ...
            topics = ["Project Planning", "Budget Review", "Team Updates", "Client Discussions", "Strategy"]
...
def perform_knowledge_search(meetings, analyses, query, search_type):
    ...
    # Search in meetings
    for meeting in meetings:
        if search_type in ["All Content", "Meeting Titles"] and query_lower in meeting.get("title", "").lower():
            results.append({
                "type": "Meeting",
        ...
        if search_type in ["All Content", "Transcripts Only"] and meeting.get("transcript"):
