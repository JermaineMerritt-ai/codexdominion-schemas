...
st.set_page_config(page_title="Council Access Crown", page_icon="👑", layout="wide")
    ...
    if user_role == "heir":
        ...
        with st.expander("📊 Ledger Overview", expanded=True):
            ...
            if recent_transactions:
                ...
                for i, tx in enumerate(reversed(recent_transactions)):
                    st.text(
                        f"{i+1}. {tx.get('description', 'N/A')} - {tx.get('amount', 0)}"
                    )
            else:
        ...
        with st.expander("📜 Add Proclamation"):
            proclamation_type = st.selectbox(
                "Type of Proclamation:", ["blessing", "silence", "general"]
            )
        ...
        with st.expander("📋 Recent Proclamations"):
            ...
            if recent_procs:
                for proc in reversed(recent_procs):
                    ...
                    st.text(
                        f"{status_icon} {proc.get('type', '').title()}: {proc.get('content', '')[:50]}..."
                    )
            else:
...
with col2:
    ...
    if user_role == "council":
        ...
        with st.expander("🌀 Flow Loom Dispatch Cycles", expanded=True):
            ...
            if recent_events:
                ...
                for i, event in enumerate(reversed(recent_events)):
                    ...
                    st.text(
                        f"{status_color} {event.get('action', 'Unknown')} - {event.get('timestamp', '')[:19]}"
                    )
            else:
        ...
        with st.expander("📜 Pending Proclamations Review"):
            ...
            pending_procs = [
                p
                for p in proclamations.get("proclamations", [])
            ...
            if pending_procs:
                for i, proc in enumerate(pending_procs):
                    ...
                    st.text(
                        f"Author: {proc.get('author', 'Unknown')} | Time: {proc.get('timestamp', '')[:19]}"
                    )
                    ...
                    with col_approve:
                        if st.button(f"✅ Approve #{i+1}"):
                            ...
                            for p in all_procs:
                                if p.get("content") == proc.get("content") and p.get(
                                    "timestamp"
                                ) == proc.get("timestamp"):
                                    p["status"] = "approved"
                                    break
        ...
        with st.expander("🤝 Add Council Concord"):
            concord_type = st.selectbox(
                "Type of Concord:", ["affirmation", "directive", "resolution"]
            )
