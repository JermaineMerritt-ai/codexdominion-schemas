...
def format_timestamp(timestamp_str):
    ...
    try:
        ...
        return timestamp_str[:16] if len(timestamp_str) > 16 else timestamp_str, "Unknown"
...
def display_heartbeat_section(beats, time_filter):
    ...
    for beat in beats:
        try:
            beat_time = datetime.fromisoformat(beat.get('timestamp', '').replace('Z', '+00:00'))
    ...
    with st.expander(f"📊 Heartbeat History ({len(filtered_beats)} entries)", expanded=True):
...
def display_proclamations_section(proclamations, time_filter, role_filter, type_filter):
    ...
    for proc in proclamations:
        # Time filter
        try:
            proc_time = datetime.fromisoformat(proc.get('timestamp', '').replace('Z', '+00:00'))
    ...
    with st.expander(f"📜 Proclamation Chronicle ({len(filtered_proclamations)} entries)", expanded=True):
...
def display_ledger_section(entries, time_filter, role_filter):
    ...
    with col2:
        ...
        most_common_type = max(set(entry_types), key=entry_types.count) if entry_types else "None"
        st.metric("🎯 Most Common Type", most_common_type)
    ...
    with st.expander(f"📖 Ledger Chronicle ({len(filtered_entries)} entries)", expanded=True):
        for entry in reversed(filtered_entries[-20:]):  # Show last 20
            ...
            proclamation = entry.get('proclamation', entry.get('description', 'Ledger entry'))
...
def codex_bulletin_interface():
    ...
    proclamations = load_json("proclamations.json", {"proclamations": []})["proclamations"]
    ...
    type_filter = st.sidebar.selectbox("🎯 Type Filter (Proclamations)", ["All Types"] + all_types)
    ...
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["💓 Heartbeat", "📜 Proclamations", "📖 Ledger", "🌟 Combined View"])
    
    with tab1:
        display_heartbeat_section(beats, time_filter)
    
    with tab2:
        display_proclamations_section(proclamations, time_filter, role_filter, type_filter)
    ...
    with tab4:
        ...
        for entry in entries:
            combined_entries.append({
                ...
                'type': 'Ledger',
                'role': entry.get('role', 'Unknown'),
                'content': entry.get('proclamation', entry.get('description', 'Chronicle entry')),
                'category': '📖'
            })
        ...
        st.info(f"📊 Showing unified timeline of {len(combined_entries)} total chronicle entries")
