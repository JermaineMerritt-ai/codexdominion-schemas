...
from core.action_ai import generate_drafts, optimize_prompt
...
class SparkStudio:
    ...
    def _auto_select_template(self, content_type: str, prompt: str) -> Dict:
        ...
        if any(
            ...
            for word in ["proclaim", "decree", "empire", "sovereign"]
        ):
            return self.creative_templates["empire_proclamation"]
        elif any(
            word in prompt_lower for word in ["document", "guide", "tutorial", "manual"]
        ):
            return self.creative_templates["technical_documentation"]
        elif any(
            word in prompt_lower for word in ["story", "narrative", "tale", "adventure"]
        ):
            return self.creative_templates["creative_story"]
        elif any(
            word in prompt_lower
            for word in ["enhance", "improve", "optimize", "upgrade"]
        ):
    ...
    def _generate_with_engine(
        self, engine: Dict, prompt: str, template_config: Dict, parameters: Dict
    ) -> Dict[str, Any]:
    ...
    def _create_base_content(
        self, prompt: str, template_config: Dict, parameters: Dict
    ) -> str:
        """Create base content structure"""

        structure = template_config.get(
        ...
        for section in structure:
            section_content = self._generate_section_content(
                section, prompt, tone, style, parameters
            )
    ...
    def _generate_section_content(
        self, section: str, prompt: str, tone: str, style: str, parameters: Dict
    ) -> str:
        ...
        base_template = section_templates.get(
            section,
    ...
    def _enhance_content(
        self, base_content: str, engine: Dict, template_config: Dict
    ) -> str:
    ...
    def get_creative_analytics(self) -> Dict[str, Any]:
        ...
        generated_memories = codex_memory.search_memories(
            tags=["spark_studio", "generated_content"], limit=100
        )
        ...
        for memory in generated_memories:
            ...
            if "content_type" in metadata:
                ...
                analytics["content_types"][content_type] = (
                    analytics["content_types"].get(content_type, 0) + 1
                )

            # Count engines
            if "engine_used" in metadata:
                engine = metadata["engine_used"]
                analytics["engines_used"][engine] = (
                    analytics["engines_used"].get(engine, 0) + 1
                )
    ...
    def optimize_content(
        self, content: str, optimization_type: str = "engagement"
    ) -> Dict[str, Any]:
        ...
        strategy = optimization_strategies.get(
            optimization_type, optimization_strategies["engagement"]
        )
