    ...
    try:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            for i, cell in enumerate(cells):
                ...
                if cell["type"] == "text":
                    f.write(f"## 📝 Text Cell {cell_number}\n\n")
                    f.write(f"{cell['content']}\n\n")
                    
                elif cell["type"] == "code":
                    f.write(f"## 💻 Code Cell {cell_number}\n\n")
                    f.write(f"```python\n{cell['content']}\n```\n\n")
                    
                elif cell["type"] == "prompt":
                    f.write(f"## 🤖 Prompt Cell {cell_number}\n\n")
        ...
        print(f"❌ Error: Could not find file {notebook_file}")
        return False
    except json.JSONDecodeError as e:
        print(f"❌ Error: Invalid JSON format in {notebook_file}: {e}")
        return False
...
def main():
    ...
    print("📚 Codex Tome Formatter - Codex Dominion Suite v1.0")
    print("=" * 60)
    
    if len(sys.argv) < 2:
        # Default behavior - look for notebook.json in current directory
        if os.path.exists("notebook.json"):
            print("🔍 Found notebook.json in current directory")
            ...
            print("\n💡 Or place a 'notebook.json' file in the current directory")
            return
    ...
    if sys.argv[1] == "--batch":
        if len(sys.argv) >= 4:
            ...
            print("   Usage: python codex_tome_formatter.py --batch input_dir output_dir")
...
def create_sample_notebook():
    ...
    print("📄 Created sample_notebook.json")
    return sample_data
