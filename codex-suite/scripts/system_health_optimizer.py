...
def optimize_wsl_docker():
    ...
    run_command('Get-Process "*docker*" -ErrorAction SilentlyContinue | Stop-Process -Force', "Stopping Docker processes")
    time.sleep(2)
...
def optimize_memory_aggressive():
    ...
    # Clear all caches
    run_command("Get-Process | Where-Object {$_.ProcessName -like '*cache*'} | Stop-Process -Force", "Clearing cache processes")
    
    # Optimize Edge browser memory
    run_command('Get-Process "msedge" | Where-Object {$_.WorkingSet -lt 100MB} | Stop-Process -Force', "Optimizing Edge processes")
    
    # Clear system memory cache
    run_command("[System.GC]::Collect(); [System.GC]::WaitForPendingFinalizers()", "Force .NET garbage collection")
...
def optimize_system_services():
    ...
    for service in services_to_optimize:
        run_command(f'Get-Service "{service}" -ErrorAction SilentlyContinue | Set-Service -StartupType Manual', f"Optimizing {service}")
...
def clean_temp_files():
    ...
    for location in temp_locations:
        run_command(f'Remove-Item "{location}\\*" -Recurse -Force -ErrorAction SilentlyContinue', f"Cleaning {location}")
    
    # Clean browser caches
    run_command('Remove-Item "$env:LOCALAPPDATA\\Microsoft\\Edge\\User Data\\Default\\Cache\\*" -Recurse -Force -ErrorAction SilentlyContinue', "Cleaning Edge cache")
...
def create_system_health_report():
    ...
    print(f"💾 Memory: {memory.available/1024**3:.1f}GB available ({(memory.available/memory.total)*100:.1f}%)")
    
    # Disk stats
    disk = psutil.disk_usage('C:\\')
    print(f"💿 Disk C: {disk.free/1024**3:.1f}GB free ({(disk.free/disk.total)*100:.1f}%)")
