...
def publish_html(page_json="love_lab_page.json", output="public/page.html"):
    ...
    for block in data["blocks"]:
        if block["type"] == "Hero":
            ...
            html.append(f"        <a href='#' style='background: rgba(255,255,255,0.2); padding: 10px 20px; text-decoration: none; color: white; border-radius: 5px;'>{c.get('cta', 'Witness')}</a>")
            ...
            html.append(f"        <form action='{c.get('action', '/submit')}' method='POST'>")
            for field in c.get("fields", []):
                html.append(f"            <p><label>{field}: <input type='text' name='{field.lower()}' required></label></p>")
            ...
            html.append(f"        <p>{c.get('text', 'Codex Eternum')} • Generated by Codex Dominion Suite</p>")
            html.append(f"        <p><small>Published: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</small></p>")
