...
from .settings import CODEX_CONFIG, get_data_path
...
class EnhancedMemoryManager:
        ...
        try:
            ...
            if index_path.exists():
                try:
                    ...
                    print(
                        f"✅ Loaded existing FAISS index with {len(self.memory_ids)} memories"
                    )
                except Exception as e:
    ...
    def _generate_embedding(self, text: str) -> np.ndarray:
        ...
        if len(pseudo_embedding) < self.embedding_dimension:
            pseudo_embedding = np.pad(
                pseudo_embedding, (0, self.embedding_dimension - len(pseudo_embedding))
            )
        else:
        ...
        try:
            ...
            data = {
                ...
                "metadata": {
                    ...
                    "version": "2.0.0",
                    "total_memories": 0,
                    "vector_enabled": FAISS_AVAILABLE
                    and self.vector_config.get("faiss_enabled", True),
                },
            }
    ...
    def semantic_search(
        self, query: str, limit: int = 10, similarity_threshold: float = None
    ...
    def search_memories(
        ...
        memories.sort(
            key=lambda x: (x.get("importance", 0), x.get("created", "")), reverse=True
        )
    ...
    def get_similar_memories(
        ...
        return [(mem, score) for mem, score in similar if mem["id"] != memory_id][
            :limit
        ]
    ...
    def get_enhanced_analytics(self) -> Dict[str, Any]:
        ...
        analytics = {
            ...
            "embedding_model": (
                "SentenceTransformers" if self.embedding_model else "Fallback"
            ),
            "types_distribution": {},
        ...
        for memory in memories.values():
            ...
            # Connection count
            analytics["connection_network"]["edges"] += len(
                memory.get("connections", [])
            )
                ...
                clusters.append(
                    {
                        ...
                        "avg_importance": np.mean(
                            [m["importance"] for m in cluster_members]
                        ),
                        "dominant_type": max(
                            set([m["type"] for m in cluster_members]),
