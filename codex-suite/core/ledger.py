        ...
        if atomic:
            ...
            try:
                with open(temp_filepath, "w", encoding="utf-8") as f:
                    json.dump(
                        data, f, indent=2, ensure_ascii=False, separators=(",", ": ")
                    )
...
def append_entry(file, key, entry, validate_entry=True, max_entries=10000):
    ...
    try:
        ...
        if "id" not in entry:
            ...
            existing_ids = [
                e.get("id", 0) for e in existing_entries if isinstance(e.get("id"), int)
            ]
        ...
        if entry_hash in existing_hashes:
            logger.warning(
                f"Duplicate entry detected, skipping: {entry.get('id', 'unknown')}"
            )
            return False
            ...
            for entry in entries:
                ...
                for filter_key, filter_value in filters.items():
                    ...
                    if isinstance(filter_value, dict):
                        if (
                            "equals" in filter_value
                            and entry_value != filter_value["equals"]
                        ):
                            match = False
                            break
                        if (
                            "contains" in filter_value
                            and filter_value["contains"].lower()
                            not in str(entry_value).lower()
                        ):
                            match = False
                            break
                        if (
                            "greater_than" in filter_value
                            and entry_value <= filter_value["greater_than"]
                        ):
                            match = False
                            break
                    else:
...
if __name__ == "__main__":
    ...
    # Test functionality
    test_entry = {"type": "test", "description": "Test entry from streamlined ledger"}
