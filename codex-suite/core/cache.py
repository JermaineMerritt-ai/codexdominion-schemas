...
class CodexCache:
    ...
    def _initialize_redis(self):
        ...
        try:
            ...
            print(
                f"✅ Redis connected: {self.redis_config['host']}:{self.redis_config['port']}"
            )
    ...
    def clear_namespace(self, namespace: str) -> int:
        ...
        try:
            ...
            if self.redis_client:
                ...
                to_delete = [
                    k
                    for k in self.fallback_cache.keys()
                    if k.startswith(f"codex:{namespace}:")
                ]
...
class CachedMemorySystem:
    ...
    def get_cached_search(self, query: str, **kwargs) -> List[Dict]:
        ...
        if hasattr(self.memory, "semantic_search") and kwargs.get("use_semantic", True):
            results = [
                (mem, score)
                for mem, score in self.memory.semantic_search(query, **kwargs)
            ]
...
class PerformanceMonitor:
    ...
    def _generate_recommendations(self, hit_rate: float, avg_time: float) -> List[str]:
        ...
        if avg_time > 0.5:  # 500ms
            recommendations.append(
                "⚡ Search times are high - consider optimizing queries"
            )
