...
def test_tiktok_earnings_system():
    ...
    try:
        ...
        if config_file.exists():
            ...
            print(
                f"   API Token configured: {'Yes' if config.get('tiktok', {}).get('token') not in [None, 'YOUR_TIKTOK_TOKEN_HERE'] else 'No'}"
            )
            print(
                f"   User ID configured: {'Yes' if config.get('tiktok', {}).get('user_id') not in [None, 'YOUR_USER_ID_HERE'] else 'No'}"
            )
            print(
                f"   Auto-archive enabled: {config.get('earnings_settings', {}).get('auto_archive', False)}"
            )
            print(
                f"   Creator Fund tracking: {config.get('earnings_settings', {}).get('track_creator_fund', False)}"
            )
    ...
    try:
        ...
        if enhanced_metrics:
            ...
            print(
                f"   Earnings per 1K views: ${enhanced_metrics.get('earnings_per_1k_views', 0):.4f}"
            )
            print(
                f"   Trend direction: {enhanced_metrics.get('trend_direction', 'unknown')}"
            )
        else:
    ...
    try:
        ...
        if earnings_summary and not earnings_summary.get("error"):
            ...
            print(
                f"   Monthly projection: ${earnings_summary.get('monthly_projection', 0):.2f}"
            )
            print(
                f"   Primary revenue source: {earnings_summary.get('primary_revenue_source', 'Unknown')}"
            )
            print(
                f"   Revenue diversification: {earnings_summary.get('revenue_diversification', 0)} streams"
            )
            ...
            print(
                f"⚠️ Earnings analysis returned: {earnings_summary.get('error', 'Unknown issue')}"
            )
    ...
    try:
        ...
        if isinstance(compat_metrics, dict) and not compat_metrics.get("error"):
            ...
            print(
                f"❌ tiktok_metrics() function failed: {compat_metrics.get('error', 'Unknown')}"
            )
        ...
        if isinstance(earnings_result, dict) and not earnings_result.get("error"):
            ...
            print(
                f"❌ get_tiktok_earnings() function failed: {earnings_result.get('error', 'Unknown')}"
            )
...
def demo_earnings_analysis():
    ...
    try:
        ...
        if sample_earnings.get("error"):
            ...
            print(
                f"   Monthly Projection: ${sample_earnings.get('monthly_projection', 0):.2f}"
            )
        ...
        for stream_name, stream_data in streams.items():
            ...
            print(
                f"   {stream_name.replace('_', ' ').title()}: ${amount:.2f} ({percentage}%)"
            )
