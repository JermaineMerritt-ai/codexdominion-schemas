...
1. **Cloud Run Service** deployed (codex-signals)

## 🚀 Step 1: Get Your Service Hash

1. Go to [Google Cloud Console > Cloud Run](https://console.cloud.google.com/run)
1. Find your `codex-signals` service
1. Note the URL format: `https://codex-signals-ABC123DEF.run.app`
1. Extract the hash: `ABC123DEF` (the part after `codex-signals-`)
...
1. **Cloud Run Logs**: Monitor your service receiving the scheduled requests
1. **Codex Capsules**: Track performance via the capsules API (if registered)
...
1. **signals-daily-scheduler** (automated mode) - Cloud Scheduler version
