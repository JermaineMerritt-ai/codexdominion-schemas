    ...
    if cycle_df:
        ...
        for idx, row in df.iterrows():
            ...
            fig.add_trace(go.Scatter(
                x=[row['initiated'], row.get('actual_completion') or row.get('target_completion')],
                y=[row['name'], row['name']],
                mode='lines+markers',
                name=row['id'],
...
def main():
    ...
    if not OMEGA_SEAL_AVAILABLE:
        st.error("⚠️ Omega Seal system is not properly configured. Please ensure omega_seal.py is available.")
        return
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🔄 Cycle Management", "📚 Sacred Archives", "📊 Metrics", "🔍 Audit Trail"])
