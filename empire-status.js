...
class EmpireStatus {
  ...
  async runDiagnostics() {
    ...
    await this.checkComponent('Dashboard System', () => {
      return (
        fs.existsSync('codex-suite') ||
        fs.existsSync('advanced_intelligence_computation_dashboard.py')
      );
    });
    ...
    await this.checkComponent('Sacred Seals', () => {
      const sealFiles = fs
        .readdirSync('.')
        .filter((file) => file.includes('seal') && file.endsWith('.json'));
      return sealFiles.length > 0;
    });

    // Check proclamations
    await this.checkComponent('Proclamations Archive', () => {
      const proclamationFiles = fs
        .readdirSync('.')
        .filter((file) => file.includes('PROCLAMATION') && file.endsWith('.md'));
  ...
  displayResults() {
    ...
    console.log(
      chalk.bold.cyan(
        `Empire Operational Level: ${percentage}% (${operationalCount}/${totalCount})`
      )
    );
