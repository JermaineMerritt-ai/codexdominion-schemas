    ...
    try:
        ...
        for port, service in ports_to_check.items():
            if f":{port}" in netstat_output and "LISTENING" in netstat_output:
                # Extract PID
                lines = [line for line in netstat_output.split('\n') if f":{port}" in line and "LISTENING" in line]
                if lines:
                    pid = lines[0].split()[-1]
                    print(f"✅ {service}")
                    ...
                    try:
                        ...
                        if 'python.exe' in tasklist_result.stdout:
                            ...
                            print(f"   Process: {tasklist_result.stdout.split()[0] if tasklist_result.stdout.split() else 'Unknown'}")
                    except:
        ...
        print("⚡ QUICK ACTIONS:")
        print("-" * 20)
        print("To start dashboard: python -m streamlit run codex_simple_dashboard.py --server.port 8502")
        print("To start API: python -m uvicorn app.main:app --host 127.0.0.1 --port 8000")
