...
def render_woocommerce_tab():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📤 Sync Products", "📝 Create Custom", "📊 Sync History"]
    )
                ...
                if selected_products:
                    ...
                    with col2:
                        auto_update = st.checkbox(
                            "🔄 Update existing products", value=False
                        )
        ...
        with st.form("create_product"):
            ...
            with col1:
                ...
                product_price = st.number_input(
                    "Price ($)*", min_value=0.01, value=97.00
                )
        ...
        if sync_files:
            ...
            for sync_file in sync_files[:10]:  # Show last 10 syncs
                try:
                    ...
                    with st.expander(
                        f"📅 {sync_time} - ✅{success_count} ❌{failed_count}"
                    ):
                        st.json(sync_data)
...
if __name__ == "__main__":
    # Standalone test
    st.set_page_config(
        page_title="🏪 WooCommerce Integration Test", page_icon="🏪", layout="wide"
    )
