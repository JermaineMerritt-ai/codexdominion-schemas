        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if execution_time > 1.0:
                    st.warning(f"⚠️ Slow operation detected: {op_name} took {execution_time:.2f}s")
...
def display_enhanced_metric(title, value, delta=None, help_text=None):
    ...
    delta_html = f'<div style="color: #32CD32; font-size: 0.9em;">📈 {delta}</div>' if delta else ''
    help_html = f'<div style="color: #ccc; font-size: 0.8em; margin-top: 5px;">{help_text}</div>' if help_text else ''
...
def run_enhanced_revenue_crown():
    ...
    if 'performance_data' in st.session_state:
        total_ops = sum(len(times) for times in st.session_state.performance_data.values())
        st.markdown(f"""
