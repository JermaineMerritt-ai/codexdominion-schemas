...
emergency_command = st.text_input(
    "EMERGENCY COMMAND INPUT:",
    placeholder="Enter emergency command or status request...",
)

if st.button("🚨 EXECUTE EMERGENCY COMMAND", type="primary"):
    if emergency_command:
        st.success(
            f"🚨 EMERGENCY RESPONSE: Command '{emergency_command}' received and processed"
        )
