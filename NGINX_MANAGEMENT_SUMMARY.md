...
1. **`nginx-test-reload.bat`** - Direct batch simulation
1. **`nginx-manager.ps1`** - Full-featured (if fixed)
...
1. **`ionos_systemctl_commands.sh`** - Includes nginx commands
1. **`ionos_codex_deploy.sh`** - Full deployment with nginx
...
1. **`ionos_systemctl_commands.sh`** includes your nginx command as step 5
1. **`IONOS_DEPLOYMENT_GUIDE.md`** documents the complete process
