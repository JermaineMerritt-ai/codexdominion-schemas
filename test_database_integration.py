...
async def test_database_integration():
    ...
    for capsule_slug in capsules_to_test:
        ...
        if result["status"] == "success":
            print(
                f"✅ Success - Run ID: {result['run_id']}, Checksum: {result['checksum'][:8]}..."
            )
        else:
    ...
    print(
        f"Total Successful Runs: {stats['overall']['successful_runs']}/{stats['overall']['total_runs']}"
    )

    print(f"\nPer-Capsule Performance:")
    for capsule_slug, capsule_stats in stats["by_capsule"].items():
        print(
            f"  📋 {capsule_slug}: {capsule_stats['success_rate']:.1f}% ({capsule_stats['total_runs']} runs)"
        )
    ...
    if all_runs:
        ...
        integrity_ok = mock_capsule_db.verify_artifact_integrity(
            test_run["id"], test_run["checksum"]
        )
        print(
            f"Integrity check for run {test_run['id']}: {'✅ PASS' if integrity_ok else '❌ FAIL'}"
        )
