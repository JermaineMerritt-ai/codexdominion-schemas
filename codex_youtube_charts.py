...
class CodexYouTubeCharts:
    ...
    def __init__(self, config_file: str = "youtube_config.json"):
        ...
        self.api_key = self.config.get("youtube", {}).get("api_key") or os.getenv(
            "YOUTUBE_API_KEY"
        )
    ...
    def youtube_metrics(self, channel_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            stats_response = (
                ...
                .list(part="statistics,snippet,brandingSettings", id=target_channel)
                .execute()
            )
            ...
            self.logger.info(
                f"Retrieved metrics for channel: {metrics.get('channel_title')}"
            )
            return metrics
        ...
        try:
            ...
            if recent_videos:
                ...
                total_recent_views = sum(
                    video.get("views", 0) for video in recent_videos
                )
                recent_engagement = (
                    (total_recent_engagement / total_recent_views * 100)
            ...
            return {
                ...
                "channel_score": self._calculate_channel_score(
                    subscribers, avg_views_per_video, recent_engagement
                ),
            }
    ...
    def _calculate_channel_score(
        self, subscribers: int, avg_views: int, engagement: float
    ) -> int:
        """Calculate overall channel performance score (0-100)"""
        try:
            # Scoring algorithm
            subscriber_score = min(
                subscribers / 10000 * 30, 30
            )  # Max 30 points for subscribers
    ...
    def upload_video(
        ...
        try:
            ...
            video_tags = tags or self.config.get("upload_settings", {}).get(
                "auto_tags", []
            )
            ...
            body = {
                "snippet": {
                    ...
                    "tags": video_tags,
                    "categoryId": self.config.get("upload_settings", {}).get(
                        "default_category", "22"
                    ),
                },
    ...
    def _calculate_trends(
        ...
        try:
            ...
            return {
                ...
                "growth_rate": (
                    round((subscriber_growth / previous_subs * 100), 2)
                    if previous_subs > 0
                    else 0
                ),
            }
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            test_response = (
                ...
                .list(part="snippet", q="test", maxResults=1, type="video")
                .execute()
            )
