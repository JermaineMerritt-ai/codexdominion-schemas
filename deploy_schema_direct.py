...
"""
    ...
    try:
        with engine.connect() as connection:
            ...
            result = connection.execute(sqlalchemy.text("SELECT COUNT(*) FROM capsules"))
            count = result.scalar()
            print(f"📈 Total capsules: {count}")
            
            # List all capsules
            result = connection.execute(sqlalchemy.text("SELECT slug, title, kind, schedule FROM capsules ORDER BY created_at"))
            capsules = result.fetchall()
            
            print("\n🎯 Deployed Capsules:")
            for row in capsules:
                slug, title, kind, schedule = row
                schedule_info = f" (scheduled: {schedule})" if schedule else " (on-demand)"
