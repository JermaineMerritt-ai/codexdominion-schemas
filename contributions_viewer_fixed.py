...
def display_contributions_detail(contributions):
    ...
    sorted_contributions = sorted(contributions, key=lambda x: x.get('timestamp', ''), reverse=True)
...
def filter_contributions(contributions):
    ...
    # Status filter
    status_options = ['All'] + list(set([c.get('status', 'Pending') for c in contributions]))
    # Replace empty strings with 'Pending'
    status_options = ['All' if x == 'All' else ('Pending' if x == '' else x) for x in status_options]
