...
def main():
    ...
    constellation_entry = {
        ...
        "total_amount": sum(
            item["amount"] for item in constellation_data["constellation"]
        ),
        "stream_count": len(constellation_data["constellation"]),
        "cycles": list(
            set(item["cycle"] for item in constellation_data["constellation"])
        ),
        "stars": list(
            set(item["star"] for item in constellation_data["constellation"])
        ),
    }
    ...
    for stream_data in constellation_data["constellation"]:
        ...
        stream_result = append_entry_simple(
            "revenue_streams.json", "streams", stream_entry
        )
    ...
    revenue_summary = {
        "total_revenue": constellation_entry["total_amount"],
        "streams": {"Store": 300, "Social": 150, "Website": 200},
        "cycles": {
            "Morning Flame": 300,
            "Twilight Flame": 150,
            "Seasonal Festival": 200,
        },
        "stars": {"Golden Dawn": 300, "Evening Concord": 150, "Festival Radiance": 200},
        "last_updated": datetime.now().isoformat(),
        "period": "2025-11-07",
    }
