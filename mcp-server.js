    ...
    try {
      ...
      if (component === 'all' || component === 'database') {
        try {
          ...
          status.components.database = { status: 'operational', message: 'Database file accessible' };
        } catch {
          status.components.database = { status: 'warning', message: 'Database file not found' };
        }
      }
      ...
      if (component === 'all' || component === 'frontend') {
        try {
          ...
          status.components.frontend = { status: 'operational', message: 'Frontend configuration found' };
        } catch {
          status.components.frontend = { status: 'error', message: 'Frontend not configured' };
        }
      }
      ...
      if (component === 'all' || component === 'backend') {
        try {
          ...
          status.components.backend = { status: 'operational', message: 'Backend application found' };
        } catch {
          status.components.backend = { status: 'error', message: 'Backend application not found' };
        }
      }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Codex Dominion System Status\n\nOverall: ${status.overall.toUpperCase()}\nTimestamp: ${status.timestamp}\n\nComponents:\n${Object.entries(status.components).map(([name, info]) => `- ${name}: ${info.status.toUpperCase()} - ${info.message}`).join('\n')}`,
          },
        ],
      };
    } catch (error) {
