...
class CodexWooCommerceSync:
    ...
    def __init__(self, store_url: str = None, consumer_key: str = None, consumer_secret: str = None):
        ...
        self.consumer_key = consumer_key or config.get("consumer_key") or os.getenv("WC_KEY")
        self.consumer_secret = consumer_secret or config.get("consumer_secret") or os.getenv("WC_SECRET")
        
        # Authentication
        self.auth = (self.consumer_key, self.consumer_secret) if self.consumer_key and self.consumer_secret else None
    ...
    def create_product(self, product_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"✅ Created product: {product_data['name']} (ID: {product['id']})")
                return product
            else:
    ...
    def sync_products(self, products: List[Dict]) -> Dict:
        ...
        print(f"🚀 Starting sync of {len(products)} products to The Merritt Method store...")
        ...
        for i, product in enumerate(products, 1):
            print(f"📦 Syncing product {i}/{len(products)}: {product.get('title', product.get('name', 'Unknown'))}")
            ...
            if created_product:
                ...
                results["failed"] += 1
                results["errors"].append(f"Failed to create {product.get('title', product.get('name', 'Unknown'))}")
        
        # Save sync results
        self.save_sync_results(results)
        
        print(f"📊 Sync complete: {results['success']} successful, {results['failed']} failed")
        return results
    ...
    def transform_product_data(self, product: Dict) -> Dict:
        ...
        return {
            ...
            "regular_price": str(product.get("price", product.get("regular_price", "0"))),
            "description": product.get("description", ""),
            "short_description": product.get("short_description", ""),
            "categories": [{"id": product["category_id"]}] if product.get("category_id") else [],
    ...
    def update_product(self, product_id: int, product_data: Dict) -> Optional[Dict]:
        ...
        try:
            response = self.session.put(f"{self.api_url}/products/{product_id}", json=product_data)
...
def sync_products(products: List[Dict], wc_key: str = None, wc_secret: str = None):
    ...
    sync_client = CodexWooCommerceSync(
        consumer_key=wc_key or WC_KEY,
        consumer_secret=wc_secret or WC_SECRET
    )
...
if __name__ == "__main__":
    ...
    if sync_client.test_connection():
        ...
        print("💡 Please update WC_KEY and WC_SECRET with your actual WooCommerce API credentials")