...
class MCPHealthMonitor:
    ...
    def wait_for_server(self, timeout: int = 60, interval: int = 5) -> bool:
        ...
        while time.time() - start < timeout:
            try:
                ...
                if r.status_code == 200:
                    ...
                    if data.get("status") == "alive":
                        ...
                        logger.info(f"🔥 MCP server is radiant and sovereign! (Response time: {elapsed:.2f}s)")
                        self._log_server_details(data)
                        return True
                    else:
                        logger.warning(f"⚠️ Server responded but status is: {data.get('status', 'unknown')}")
                else:
            ...
            if remaining > 0:
                logger.info(f"⏳ Waiting for MCP server... ({remaining:.0f}s remaining)")
                time.sleep(min(interval, remaining))
            attempt += 1
            
        logger.error(f"❌ MCP server did not respond within {timeout}s - Sacred flame may need rekindling")
        return False
    ...
    def check_status(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                logger.info(f"✅ Status check successful: {data.get('message', 'Unknown')}")
                return data
            else:
    ...
    def check_capabilities(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                logger.info(f"🔧 Capabilities check successful: Protocol v{data.get('protocol_version', 'unknown')}")
                return data
            else:
                logger.warning(f"⚠️ Capabilities check failed with code: {r.status_code}")
                return None
        except Exception as e:
    ...
    def comprehensive_check(self) -> Dict[str, Any]:
        ...
        if results["status"] and results["status"].get("status") == "alive":
            ...
            logger.info("👑 Comprehensive verification: MCP server fully operational and sovereign")
        elif results["status"]:
            results["overall_status"] = "partial"
            logger.warning("⚠️ Comprehensive verification: MCP server partially responsive")
        else:
