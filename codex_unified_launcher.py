...
class CodexUnifiedLauncher:
        ...
        try:
            ...
            if args.stream == "affiliate":
                if not args.order_id:
                    ...
                    return False
                txn_id = self.treasury.ingest_affiliate(
                    args.order_id, args.amount, args.currency or "USD"
                )
                ...
                if not args.symbol:
                    ...
                    return False
                txn_id = self.treasury.ingest_stock(
                    args.symbol, args.amount, args.currency or "USD"
                )
                ...
                if not args.pool_id:
                    ...
                    return False
                txn_id = self.treasury.ingest_amm(
                    args.pool_id, args.amount, args.currency or "USD"
                )
                ...
                txn_id = self.treasury.ingest_consulting(
                    client_id, hours, rate, args.currency or "USD"
                )

            elif args.stream == "development":
                project_id = args.project_id or "default-project"
                txn_id = self.treasury.ingest_development(
                    project_id, args.amount, args.currency or "USD"
                )
    ...
    def cmd_serve(self, args):
        ...
        try:
                        ...
                        jsonify(
                            {
                                ...
                                "dawn_flame": dawn_status.get(
                                    "flame_status", "unknown"
                                ),
                                "version": "1.0.0",
                            }
                        ),
            ...
            print(
                f"📊 Treasury API: http://{args.host}:{args.port}/api/treasury/summary"
            )
    ...
    def cmd_status(self, args):
        ...
        try:
            ...
            if self.treasury:
                ...
                print(
                    f"💰 Treasury (7 days): ${summary['total_revenue']:.2f} ({summary['total_transactions']} txns)"
                )
                print(f"🗄️  Data Source: {summary['source']}")
            else:
            ...
            try:
                status = get_dawn_status()
                print(
                    f"🌅 Dawn Status: {status['flame_status']} ({status['total_dispatches']} total)"
                )
                print(
                    f"   Today: {'Dispatched' if status['dispatched_today'] else 'Pending'}"
                )
            except:
    ...
    def cmd_report(self, args):
        ...
        try:
            ...
            try:
                ...
                print(f"Total Dispatches: {status['total_dispatches']}")
                print(
                    f"Today's Status: {'✅ Complete' if status['dispatched_today'] else '⏳ Pending'}"
                )
                print(f"Flame Status: {status['flame_status']}")
            except:
...
Examples:
  ...
  python codex_unified_launcher.py setup
    ...
    treasury_parser.add_argument(
        "action", choices=["summary", "ingest", "list"], help="Treasury action"
    )
    ...
    dawn_parser.add_argument(
        "action", choices=["dispatch", "status"], help="Dawn action"
    )
