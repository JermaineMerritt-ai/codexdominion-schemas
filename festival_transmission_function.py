...
def festival_transmission(event, context):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"📚 Retrieved {len(existing_data.get('cycles', []))} existing cycles")
        ...
        response_data = {
            **data,
            "summary": {
                ...
                "last_rite": data["cycles"][-1]["rite"] if data["cycles"] else "None yet",
...
def festival_transmission_pubsub(event, context):
    ...
    if 'data' in event:
        ...
        festival_data = {"rite": "Background Festival", "proclamation": "Automated Ceremony"}
