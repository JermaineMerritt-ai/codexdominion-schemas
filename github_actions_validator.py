...
def validate_github_workflows():
    ...
    for workflow_file in workflows_dir.glob("*.yaml"):
        ...
        try:
            ...
            print(
                f"🔍 Workflow content keys: {list(workflow_content.keys()) if workflow_content else 'None'}"
            )

            # Check for common issues
            if not workflow_content or (
                "on" not in workflow_content and True not in workflow_content
            ):
                validation_results["issues_found"].append(
                    f"❌ {workflow_file.name}: Missing 'on' trigger"
                )
            ...
            if "actions/checkout@v3" in workflow_text:
                validation_results["issues_found"].append(
                    f"⚠️ {workflow_file.name}: Using outdated checkout action"
                )
            ...
            validation_results["issues_found"].append(
                f"❌ {workflow_file.name}: YAML syntax error - {str(e)}"
            )
            ...
            if action_dir.is_dir():
                ...
                if action_yml.exists():
                    try:
                        ...
                        validation_results["issues_found"].append(
                            f"❌ Action {action_dir.name}: {str(e)}"
                        )
                else:
                    validation_results["issues_found"].append(
                        f"❌ Action {action_dir.name}: Missing action.yml"
                    )
