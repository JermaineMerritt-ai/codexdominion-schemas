...
class IntegrationTester:
    ...
    def test_core_dependencies(self) -> bool:
        ...
        for module, description in dependencies.items():
            try:
                ...
                self.log_test(
                    f"Dependency: {module}", False, f"Missing - {description}"
                )
                all_passed = False
    ...
    def test_core_modules(self) -> bool:
        ...
        for module, description in modules.items():
            try:
                ...
                self.log_test(
                    f"Core Module: {module}", False, f"Import error - {str(e)}"
                )
                all_passed = False
    ...
    def test_app_modules(self) -> bool:
        ...
        for module, description in modules.items():
            try:
                ...
                self.log_test(
                    f"App Module: {module}", False, f"Import error - {str(e)}"
                )
                all_passed = False
    ...
    def test_file_structure(self) -> bool:
        ...
        required_dirs = ["apps", "core", "modules", "data", "static"]
            ...
            if process.poll() is None:
                ...
                try:
                    ...
                    self.log_test(
                        "FastAPI Server",
                        api_ok,
                        f"Started on port 8001, health: {response.status_code}",
                    )
                ...
                self.log_test(
                    "FastAPI Server", False, f"Failed to start: {stderr[:100]}..."
                )
                return False
    ...
    def test_memory_system(self) -> bool:
        ...
        try:
            ...
            found = any(
                "integration test" in str(mem).lower() for mem in search_results
            )

            if found:
                self.log_test(
                    "Memory System", True, "Store and search operations successful"
                )
                return True
            else:
    ...
    def test_cache_system(self) -> bool:
        ...
        try:
            ...
            test_value = {
                "message": "Integration test cache entry",
                "timestamp": time.time(),
            }
            ...
            if retrieved and retrieved["message"] == test_value["message"]:
                ...
                self.log_test(
                    "Cache System", True, f"Operations successful (backend: {backend})"
                )
                return True
            else:
    ...
    def test_ledger_system(self) -> bool:
        ...
        try:
            ...
            if transaction and transaction.get("type") == "integration_test":
                self.log_test(
                    "Ledger System", True, "Transaction operations successful"
                )
                return True
            else:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any("Core Module" in test for test in failed_tests):
            recommendations.append(
                "🔧 Check core module import paths and __init__.py files"
            )
        ...
        if any("Cache System" in test for test in failed_tests):
            recommendations.append(
                "⚡ Install Redis for better caching: pip install redis"
            )
