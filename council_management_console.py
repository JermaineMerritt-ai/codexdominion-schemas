...
try:
    from ceremonial_council_archival import (
...
def add_council_entry_interactive():
    ...
    entry_types = ["Proclamation", "Affirmation", "Blessing", "Silence", "Inscription", "Declaration"]
    ...
    if add_metadata:
        ...
        if ceremony_type:
            ceremonial_metadata["ceremony_type"] = ceremony_type.upper().replace(' ', '_')
    ...
    # Create the entry
    try:
        entry = append_council_entry(entry_type, author, message, ceremonial_metadata if ceremonial_metadata else None)
...
def search_council_entries():
    ...
    try:
        if choice == "1":
            entry_type = input("Enter entry type (Proclamation, Affirmation, etc.): ").strip()
...
def show_council_statistics():
    ...
    try:
        ...
        for entry in recent_entries:
            print(f"   📅 {entry.get('date')} - {entry.get('type')} by {entry.get('author')}")
