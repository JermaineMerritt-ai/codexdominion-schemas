...
class ChatMessageHooks {
  ...
  suggestsChatInteraction(change, document) {
    ...
    const isQuestion =
      text.includes('?') || text.match(/^(how|what|why|when|where|can|could|should|would)/i);
    const isAtEndOfLine =
      change.range.end.character === document.lineAt(change.range.end.line).text.length;
  ...
  isChatContextFile(document) {
    ...
    return (
      chatFilePatterns.some((pattern) => fileName.includes(pattern)) ||
