...
try:
    from codex_tiktok_earnings import (CodexTikTokEarnings, archive_tiktok,
                                       get_tiktok_earnings, tiktok_metrics)
...
try:
    from codex_pinterest_capsule import (CodexPinterestCapsule,
                                         archive_pinterest,
                                         get_pinterest_capsule_summary,
                                         pinterest_batch_pin,
                                         pinterest_metrics, pinterest_pin)
...
def main():
    ...
    with tabs[0]:  # Dominion Central
        ...
        with col3:
            ...
            ledger = load_ledger()
            st.metric(
                "📜 Ledger Entries", len(ledger) if isinstance(ledger, dict) else 0
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        try:
            ...
            if archive_file.exists():
                ...
                st.info(
                    "No archive file found - run a Dawn Dispatch to create activity history"
                )
        except Exception as e:
    ...
    if DAWN_AVAILABLE:
        ...
        with tabs[tab_index]:
            ...
            with col1:
                ...
                if st.button(
                    "🌅 Execute Dawn Dispatch", key="dawn_execute_btn", type="primary"
                ):
                    with st.spinner("Generating comprehensive system report..."):
                        try:
                            ...
                            if result.get("success"):
                                st.markdown(
                                    '<div class="success-box">✅ Dawn Dispatch completed successfully!</div>',
                                    unsafe_allow_html=True,
                                )
                                ...
                                st.markdown(
                                    f'<div class="error-box">❌ Dawn Dispatch failed: {result.get("error")}</div>',
                                    unsafe_allow_html=True,
                                )
                        except Exception as e:
                            st.markdown(
                ...
                if st.button("📊 Quick System Check", key="dawn_quick_check_btn"):
                    try:
                        ...
                        with col_b:
                            ...
                            st.metric(
                                "📜 Ledger Status", ledger_data.get("status", "Unknown")
                            )
                        ...
                        with col_c:
                            ...
                            st.metric(
                                "🔧 Services Online", f"{online_count}/{len(services)}"
                            )
        ...
        with tabs[youtube_tab_index]:
            ...
            with col1:
                            ...
                            with st.spinner("Fetching YouTube analytics..."):
                                try:
                                    ...
                                    if analytics.get("error"):
                                        ...
                                        st.session_state["youtube_analytics"] = (
                                            analytics
                                        )
                ...
                with col_b:
                    if st.button(
                        "💾 Archive Current",
                        key="youtube_archive_btn",
                        disabled=not st.session_state.get("youtube_analytics"),
                    ):
                        try:
                            if st.session_state.get("youtube_analytics"):
                                success = archive_youtube(
                                    st.session_state["youtube_analytics"]
                                )
                                if success:
                ...
                if st.session_state.get("youtube_analytics"):
                    ...
                    with metric_col1:
                        ...
                        st.metric(
                            "👥 Subscribers",
                            f"{subscribers:,}",
                            delta=growth if growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    with metric_col2:
                        ...
                        st.metric(
                            "👀 Total Views",
                            f"{views:,}",
                            delta=view_growth if view_growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    with metric_col3:
                        ...
                        st.metric(
                            "🎬 Total Videos",
                            f"{videos:,}",
                            delta=video_growth if video_growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    if analytics.get("channel_title"):
                        ...
                        with info_col1:
                            st.markdown(
                                f"**🎯 Channel:** {analytics.get('channel_title', 'Unknown')}"
                            )
                            st.markdown(
                                f"**🌍 Country:** {analytics.get('country', 'Not specified')}"
                            )
                            st.markdown(
                                f"**🔗 Custom URL:** {analytics.get('custom_url', 'Not set')}"
                            )
                    ...
                    if analytics.get("recent_videos"):
                        ...
                        for i, video in enumerate(recent_videos):
                            with st.expander(
                                f"🎥 {video.get('title', 'Unknown Title')[:60]}..."
                            ):
                                ...
                                with video_col3:
                                    st.metric(
                                        "💬 Comments", f"{video.get('comments', 0):,}"
                                    )
        ...
        with tabs[tiktok_tab_index]:
            ...
            with col1:
                ...
                with col_b:
                    if st.button(
                        "💾 Archive Current",
                        key="tiktok_archive_btn",
                        disabled=not st.session_state.get("tiktok_earnings"),
                    ):
                        try:
                            if st.session_state.get("tiktok_earnings"):
                                success = archive_tiktok(
                                    st.session_state["tiktok_earnings"]
                                )
                                if success:
                                    st.success("✅ Archived successfully!")
                                else:
                ...
                if st.session_state.get("tiktok_earnings"):
                    ...
                    with metric_col2:
                        ...
                        st.metric(
                            "👥 Followers",
                            f"{followers:,}",
                            delta=follower_growth if follower_growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    if earnings.get("revenue_streams"):
                        ...
                        primary_source = earnings.get(
                            "primary_revenue_source", "Unknown"
                        )
                        ...
                        with stream_col1:
                            ...
                            st.markdown(
                                f"**🔄 Diversification:** {diversification} revenue streams"
                            )
                            ...
                            for stream_name, stream_data in streams.items():
                                ...
                                st.markdown(
                                    f"• **{stream_name.title()}:** ${amount:.2f} ({percentage}%)"
                                )
                        ...
                        with stream_col2:
                            ...
                            try:
                                ...
                                chart_data = {
                                    ...
                                    "Amount": [
                                        v.get("amount", 0) for v in streams.values()
                                    ],
                                }
                                ...
                                st.info(
                                    "📊 Install plotly for revenue charts: pip install plotly"
                                )
                            except Exception as e:
                    ...
                    if earnings.get("performance_insights"):
                        ...
                        with insight_col2:
                            ...
                            if priority:
                                ...
                                st.markdown(
                                    f"**{priority_emoji} Priority Focus:** {priority.title()}"
                                )

                    # Demo data notice
                    if earnings.get("demo"):
                        st.info(
                            "📊 **Demo Data:** Configure TikTok API credentials for real analytics"
                        )
            ...
            with col2:
                ...
                calc_views = st.number_input(
                    "Views (in thousands)", min_value=0, value=100, step=10
                )
        ...
        with tabs[threads_tab_index]:
            ...
            with col1:
                            ...
                            with st.spinner("Fetching Threads engagement analytics..."):
                                try:
                                    ...
                                    if engagement.get("error"):
                                        ...
                                        # Store in session state for display
                                        st.session_state["threads_engagement"] = (
                                            engagement
                                        )
                ...
                with col_b:
                    if st.button(
                        "💾 Archive Current",
                        key="threads_archive_btn",
                        disabled=not st.session_state.get("threads_engagement"),
                    ):
                        try:
                            if st.session_state.get("threads_engagement"):
                                success = archive_threads(
                                    st.session_state["threads_engagement"]
                                )
                                if success:
                                    st.success("✅ Archived successfully!")
                ...
                if st.session_state.get("threads_engagement"):
                    ...
                    with metric_col1:
                        ...
                        st.metric(
                            "❤️ Total Likes",
                            f"{likes:,}",
                            delta=likes_growth if likes_growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    with metric_col5:
                        ...
                        st.metric(
                            "👀 Reach",
                            f"{reach:,}",
                            delta=reach_growth if reach_growth != 0 else None,
                        )
                    ...
                    if engagement.get("community_type"):
                        ...
                        with community_col1:
                            ...
                            st.markdown(
                                f"**{community_emoji} Community Type:** {community_type.replace('_', ' ').title()}"
                            )

                            engagement_distribution = engagement.get(
                                "engagement_distribution", {}
                            )
                            if engagement_distribution:
                                st.markdown("**📊 Engagement Distribution:**")
                                st.markdown(
                                    f"• ❤️ Likes: {engagement_distribution.get('likes', 0)}%"
                                )
                                st.markdown(
                                    f"• 🔄 Reposts: {engagement_distribution.get('reposts', 0)}%"
                                )
                                st.markdown(
                                    f"• 💬 Replies: {engagement_distribution.get('replies', 0)}%"
                                )

                            quality_score = engagement.get(
                        ...
                        with community_col2:
                            ...
                            try:
                                ...
                                if engagement_distribution and any(
                                ...
                                st.info(
                                    "📊 Install plotly for engagement charts: pip install plotly"
                                )
                            except Exception as e:
                                st.warning(f"Chart unavailable: {str(e)}")
                    ...
                    if engagement.get("engagement_velocity"):
                        ...
                        with trend_col1:
                            ...
                            st.markdown(
                                f"**{velocity_emoji} Velocity:** {velocity.title()}"
                            )

                        with trend_col2:
                            peak_hour = engagement.get(
                                "peak_engagement_hour", "unknown"
                            )
                    ...
                    if engagement.get("demo"):
                        st.info(
                            "📊 **Demo Data:** Configure Meta Graph API credentials for real analytics"
                        )
            ...
            with col2:
                ...
                calc_replies = st.number_input(
                    "Replies", min_value=0, value=100, step=10
                )
                calc_impressions = st.number_input(
                    "Impressions", min_value=1, value=10000, step=500
                )

                if calc_impressions > 0:
                    total_engagement_calc = calc_likes + calc_reposts + calc_replies
                    engagement_rate_calc = (
                        total_engagement_calc / calc_impressions
                    ) * 100
        ...
        with tabs[whatsapp_tab_index]:
            ...
            with col1:
                ...
                with col_a:
                    if st.button(
                        "📊 Get Analytics",
                        key="whatsapp_get_btn",
                        type="primary",
                        disabled=not phone_id,
                    ):
                        if phone_id:
                            with st.spinner("Fetching WhatsApp Business analytics..."):
                                try:
                                    ...
                                    if analytics.get("error"):
                                        ...
                                        # Store in session state for display
                                        st.session_state["whatsapp_analytics"] = (
                                            analytics
                                        )
                ...
                with col_b:
                    if st.button(
                        "💾 Archive Current",
                        key="whatsapp_archive_btn",
                        disabled=not st.session_state.get("whatsapp_analytics"),
                    ):
                        try:
                            if st.session_state.get("whatsapp_analytics"):
                                success = archive_whatsapp(
                                    st.session_state["whatsapp_analytics"]
                                )
                                if success:
                                    st.success("✅ Archived successfully!")
                ...
                if st.session_state.get("whatsapp_analytics"):
                    ...
                    with metric_col2:
                        ...
                        st.metric(
                            "🎯 Leads",
                            f"{leads:,}",
                            delta=lead_growth if lead_growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    with metric_col3:
                        ...
                        st.metric(
                            "💰 Cost (USD)",
                            f"${cost:.2f}",
                            delta=f"${cost_growth:.2f}" if cost_growth != 0 else None,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    with perf_col1:
                        cost_per_conversation = analytics.get(
                            "cost_per_conversation", 0
                        )
                        st.metric(
                            "💵 Cost/Conversation", f"${cost_per_conversation:.3f}"
                        )
                    ...
                    with perf_col3:
                        ...
                        st.metric(
                            "📊 ROI", f"{roi_percentage:.1f}%", delta_color=roi_color
                        )

                    with perf_col4:
                        performance_score = analytics.get(
                            "business_performance_score", 0
                        )
                        st.metric(
                    ...
                    if analytics.get("cost_efficiency_rating"):
                        ...
                        with business_col1:
                            cost_efficiency = analytics.get(
                                "cost_efficiency_rating", "unknown"
                            )
                            cost_emoji = {
                                "excellent": "🟢",
                                "good": "🟡",
                            ...
                            st.markdown(
                                f"**{cost_emoji} Cost Efficiency:** {cost_efficiency.title()}"
                            )

                            performance_trend = analytics.get(
                            ...
                            st.markdown(
                                f"**{trend_emoji} Trend:** {performance_trend.title()}"
                            )

                            delivery_quality = analytics.get(
                            ...
                            delivery_emoji = {
                                ...
                                "good": "🟡",
                                "needs_improvement": "🔴",
                            }.get(delivery_quality, "❓")
                            st.markdown(
                                f"**{delivery_emoji} Delivery Quality:** {delivery_quality.replace('_', ' ').title()}"
                            )

                            engagement_quality = analytics.get(
                            ...
                            st.markdown(
                                f"**{engagement_emoji} Engagement Quality:** {engagement_quality.title()}"
                            )
                        ...
                        with business_col2:
                            ...
                            st.markdown(
                                f"**{roi_emoji} ROI Status:** {roi_status.replace('_', ' ').title()}"
                            )

                            monthly_projection = analytics.get(
                                "monthly_cost_projection", 0
                            )
                            st.markdown(
                                f"**📅 Monthly Cost Projection:** ${monthly_projection:.2f}"
                            )
                            ...
                            messages_per_conversation = analytics.get(
                                "messages_per_conversation", 0
                            )
                            st.markdown(
                                f"**📱 Messages/Conversation:** {messages_per_conversation:.1f}"
                            )
                    ...
                    if analytics.get("total_campaigns", 0) > 0:
                        ...
                        with campaign_col3:
                            total_messages_sent = analytics.get(
                                "total_messages_sent", 0
                            )
                            st.metric("📤 Total Sent", f"{total_messages_sent:,}")

                        campaign_performance = analytics.get(
                        ...
                        perf_emoji = {
                            ...
                            "fair": "🟠",
                            "needs_improvement": "🔴",
                        }.get(campaign_performance, "❓")
                        st.markdown(
                            f"**{perf_emoji} Campaign Rating:** {campaign_performance.replace('_', ' ').title()}"
                        )
                                    ...
                                    with st.spinner("Sending broadcast..."):
                                        try:
                                            ...
                                            if broadcast_result.get("error"):
                                                st.error(
                                                    f"❌ Broadcast failed: {broadcast_result['error']}"
                                                )
                                            else:
                                                ...
                                                with result_col3:
                                                    st.metric(
                                                        "💰 Est. Cost",
                                                        f"${broadcast_result.get('estimated_cost', 0):.2f}",
                                                    )
                    ...
                    if analytics.get("demo"):
                        st.info(
                            "📊 **Demo Data:** Configure WhatsApp Business API credentials for real analytics and broadcasting"
                        )
            ...
            with col2:
                ...
                try:
                    ...
                    if history:
                        ...
                        for entry in history[-5:]:
                            ...
                            st.markdown(
                                f"• {timestamp} - {conversations} conv, ${cost:.2f}"
                            )
                    else:
                ...
                calc_conversations = st.number_input(
                    "Conversations", min_value=0, value=100, step=10
                )
                calc_cost_per_conv = st.number_input(
    ...
    if PINTEREST_AVAILABLE:
        ...
        with tabs[pinterest_tab_index]:
            ...
            with col1:
                ...
                with col_a:
                    if st.button(
                        "📊 Get Analytics", key="pinterest_get_btn", type="primary"
                    ):
                ...
                with col_b:
                    if st.button(
                        "💾 Archive Current",
                        key="pinterest_archive_btn",
                        disabled=not st.session_state.get("pinterest_analytics"),
                    ):
                        try:
                            if st.session_state.get("pinterest_analytics"):
                                success = archive_pinterest(
                                    st.session_state["pinterest_analytics"]
                                )
                                if success:
                                    st.success("✅ Archived successfully!")
                                else:
                ...
                if st.session_state.get("pinterest_analytics"):
                    ...
                    with metric_col3:
                        ...
                        st.metric(
                            "💾 Saves", f"{saves:,}", delta=f"{save_rate:.2f}% Rate"
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    with metric_col4:
                        ...
                        st.metric(
                            "💰 Affiliate Revenue",
                            f"${affiliate_revenue:.2f}",
                            delta=f"${monthly_projection:.2f}/mo",
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                    ...
                    if analytics.get("content_quality"):
                        ...
                        with analysis_col1:
                            ...
                            st.markdown(
                                f"**{quality_emoji} Content Quality:** {content_quality.replace('_', ' ').title()}"
                            )

                            audience_engagement = analytics.get(
                            ...
                            engagement_emoji = {
                                ...
                                "low": "🟠",
                                "very_low": "🔴",
                            }.get(audience_engagement, "❓")
                            st.markdown(
                                f"**{engagement_emoji} Audience Engagement:** {audience_engagement.replace('_', ' ').title()}"
                            )

                            monetization_efficiency = analytics.get(
                                "monetization_efficiency", "unknown"
                            )
                            monetization_emoji = {
                                "excellent": "💰",
                                "good": "🟢",
                                ...
                                "low": "🟠",
                                "very_low": "🔴",
                            }.get(monetization_efficiency, "❓")
                            st.markdown(
                                f"**{monetization_emoji} Monetization:** {monetization_efficiency.replace('_', ' ').title()}"
                            )
                        ...
                        with analysis_col2:
                            ...
                            st.markdown(
                                f"**{trend_emoji} Performance Trend:** {performance_trend.title()}"
                            )
                            ...
                            st.markdown(
                                f"**{revenue_emoji} Revenue Trend:** {revenue_trend.title()}"
                            )
                                ...
                                with st.spinner("Creating Pinterest pin..."):
                                    try:
                                        ...
                                        if pin_result.get("success"):
                                            st.success(
                                                f"✅ {pin_result.get('message', 'Pin created successfully!')}"
                                            )

                                            if pin_result.get("pin_url"):
                                                st.markdown(
                                                    f"🔗 **Pin URL:** {pin_result['pin_url']}"
                                                )
                                        else:
                                            st.error(
                                                f"❌ Pin creation failed: {pin_result.get('error', 'Unknown error')}"
                                            )
                    ...
                    with batch_col1:
                        if st.button(
                            "📌 Create Batch Pins", key="pinterest_batch_pins_btn"
                        ):
                            if batch_pins_text:
                                try:
                                    ...
                                    with st.spinner("Creating batch pins..."):
                                        ...
                                        with result_col1:
                                            st.metric(
                                                "✅ Successful",
                                                batch_result.get("successful_pins", 0),
                                            )

                                        with result_col2:
                                            st.metric(
                                                "❌ Failed",
                                                batch_result.get("failed_pins", 0),
                                            )

                                        with result_col3:
                                            st.metric(
                                                "📊 Total Processed",
                                                batch_result.get("total_processed", 0),
                                            )
                    ...
                    with batch_col2:
                        if st.button(
                            "📋 JSON Template", key="pinterest_json_template_btn"
                        ):
                            template = [
                                {
                    ...
                    if analytics.get("demo"):
                        st.info(
                            "📊 **Demo Data:** Configure Pinterest Business API credentials for real analytics and pin creation"
                        )
            ...
            with col2:
                ...
                try:
                    ...
                    if history:
                        ...
                        for entry in history[-5:]:
                            ...
                            st.markdown(
                                f"• {timestamp} - {impressions:,} imp, ${revenue:.2f}"
                            )
                    else:
                        st.info("No archive history yet")
                ...
                calc_impressions = st.number_input(
                    "Impressions", min_value=0, value=50000, step=1000
                )
                calc_ctr = st.number_input(
                    "CTR (%)", min_value=0.0, value=1.5, step=0.1, format="%.1f"
                )
                calc_conversion = st.number_input(
                    "Conversion Rate (%)",
                    min_value=0.0,
                    value=5.0,
                    step=0.5,
                ...
                calc_commission = st.number_input(
                    "Avg Commission ($)",
                    min_value=0.0,
                    value=15.0,
                    step=1.0,
    ...
    with tabs[status_tab_index]:
        ...
        with col1:
            ...
            services = [
                ...
                {
                    "name": "Dawn Dispatch",
                    "status": "🟢 Ready" if DAWN_AVAILABLE else "🔴 Not Available",
                    "available": DAWN_AVAILABLE,
                },
                {
                    "name": "WooCommerce",
                    "status": (
                        "🟢 Available" if WOOCOMMERCE_AVAILABLE else "🔴 Not Configured"
                    ),
                    "available": WOOCOMMERCE_AVAILABLE,
                },
                {
                    "name": "Twitter",
                    "status": (
                        "🟢 Available" if TWITTER_AVAILABLE else "🔴 Not Configured"
                    ),
                ...
                },
                {
                    "name": "Buffer",
                    "status": (
                        "🟢 Available" if BUFFER_AVAILABLE else "🔴 Not Configured"
                    ),
                ...
                },
                {
                    "name": "YouTube Charts",
                    "status": (
                        "🟢 Available" if YOUTUBE_AVAILABLE else "🔴 Not Configured"
                    ),
                    "available": YOUTUBE_AVAILABLE,
                },
                {
                    "name": "TikTok Earnings",
                    "status": (
                        "🟢 Available" if TIKTOK_AVAILABLE else "🔴 Not Configured"
                    ),
                ...
                },
                {
                    ...
                    "status": (
                        "🟢 Available" if THREADS_AVAILABLE else "🔴 Not Configured"
                    ),
        ...
        with col2:
            ...
            st.metric(
                "📈 System Health", f"{(available_services/total_services)*100:.1f}%"
            )
