...
try:
    from codex_database import db, get_portfolio_summary, get_affiliate_performance, get_top_pools
...
try:
    from codex_error_handler import safe_execute, CodexErrorHandler
...
def load_sample_data():
    ...
    return {
        'portfolios': [
            {
                ...
                'positions': [
                    {'symbol': 'AAPL', 'quantity': 100, 'avg_cost': 150.00, 'last_price': 155.25, 'market_value': 15525.00},
                    {'symbol': 'MSFT', 'quantity': 50, 'avg_cost': 300.00, 'last_price': 310.50, 'market_value': 15525.00},
                    {'symbol': 'GOOGL', 'quantity': 25, 'avg_cost': 2500.00, 'last_price': 2475.00, 'market_value': 61875.00},
                ]
            },
            {
                'id': str(uuid.uuid4()),
                ...
                'positions': [
                    {'symbol': 'SPY', 'quantity': 200, 'avg_cost': 400.00, 'last_price': 405.00, 'market_value': 81000.00},
                    {'symbol': 'BND', 'quantity': 40, 'avg_cost': 100.00, 'last_price': 100.00, 'market_value': 4000.00},
                ]
            }
        ],
        'affiliate_performance': {
            ...
            'programs': [
                {'program': 'TradingView', 'clicks': 1250, 'conversions': 45, 'commission': 2250.00, 'conversion_rate': 3.6},
                {'program': 'Interactive Brokers', 'clicks': 890, 'conversions': 32, 'commission': 1600.00, 'conversion_rate': 3.6},
                {'program': 'Webull', 'clicks': 567, 'conversions': 18, 'commission': 800.00, 'conversion_rate': 3.2},
                {'program': 'TD Ameritrade', 'clicks': 234, 'conversions': 4, 'commission': 200.00, 'conversion_rate': 1.7},
            ]
        },
        'amm_pools': [
            {'pool_symbol': 'USDC/ETH', 'tvl_usd': 125000000.00, 'apr': 8.5, 'risk_tier': 'low'},
            {'pool_symbol': 'DAI/USDC', 'tvl_usd': 85000000.00, 'apr': 4.2, 'risk_tier': 'low'},
            {'pool_symbol': 'WBTC/ETH', 'tvl_usd': 45000000.00, 'apr': 12.8, 'risk_tier': 'medium'},
            {'pool_symbol': 'LINK/ETH', 'tvl_usd': 25000000.00, 'apr': 18.5, 'risk_tier': 'medium'},
            {'pool_symbol': 'UNI/ETH', 'tvl_usd': 15000000.00, 'apr': 35.2, 'risk_tier': 'high'},
        ]
    }
...
def main():
    ...
    with st.sidebar:
        ...
        st.image("https://via.placeholder.com/200x60/2a5298/ffffff?text=CODEX", width=200)
        
        st.markdown("### 🎯 Navigation")
        page = st.selectbox(
            "Select Dashboard",
            ["Portfolio Overview", "Position Management", "Trading Analytics", "Affiliate Performance", "DeFi Pools", "Risk Analysis"]
        )
...
def show_position_management(data):
    ...
    for portfolio in data['portfolios']:
        for pos in portfolio['positions']:
            all_positions.append({
                ...
                'Unrealized P&L': (pos['last_price'] - pos['avg_cost']) * pos['quantity'],
                'Return %': ((pos['last_price'] - pos['avg_cost']) / pos['avg_cost'] * 100)
            })
    ...
    for col in ['Avg Cost', 'Last Price', 'Market Value', 'Unrealized P&L']:
        formatted_positions[col] = formatted_positions[col].apply(lambda x: f"${x:,.2f}")
    formatted_positions['Return %'] = formatted_positions['Return %'].apply(lambda x: f"{x:.2f}%")
    ...
    fig = px.scatter(
        ...
        labels={'Return %': 'Return Percentage (%)', 'Market Value': 'Market Value ($)'}
    )
...
def show_trading_analytics(data):
    ...
    symbols = list(set([pos['symbol'] for portfolio in data['portfolios'] for pos in portfolio['positions']]))
...
def show_affiliate_performance(data):
    ...
    with col3:
        st.metric(
            "✅ Conversions",
            f"{affiliate_data['total_conversions']:,}",
            delta="+15.3%"
        )
    ...
    formatted_programs['commission'] = formatted_programs['commission'].apply(lambda x: f"${x:,.2f}")
    formatted_programs['conversion_rate'] = formatted_programs['conversion_rate'].apply(lambda x: f"{x:.2f}%")
...
def show_defi_pools(data):
    ...
    formatted_pools['tvl_usd'] = formatted_pools['tvl_usd'].apply(lambda x: f"${x/1000000:,.1f}M")
    ...
    st.markdown(formatted_pools.to_html(escape=False, index=False), unsafe_allow_html=True)
...
def show_risk_analysis(data):
    ...
    with col1:
        ...
        risk_df = pd.DataFrame([
            {'Risk Tier': tier.title(), 'Value': value, 'Percentage': value/total_value*100}
    ...
    with col2:
        ...
        risk_metrics = [
            ...
            {'Metric': 'Value at Risk (95%)', 'Value': '$8,750', 'Status': 'Acceptable'},
