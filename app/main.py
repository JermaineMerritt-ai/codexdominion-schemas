...
def affiliate_stats():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching affiliate stats: {str(e)}"
        )
...
def amm_pools():
    ...
    try:
        rows = q(
            "SELECT id, pool_symbol, tvl_usd, apr, risk_tier, updated_at FROM amm_pools ORDER BY tvl_usd DESC LIMIT 20"
        )
        ...
        return {
            ...
            "avg_apr": (
                round(sum(pool["apr"] for pool in pools_data) / len(pools_data), 2)
                if pools_data
                else 0.0
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching AMM pools: {str(e)}"
        )
...
def amm_swap(req: SwapRequest):
    ...
    try:
        ...
        event_payload = {
            ...
            "amount": req.amount,
            "estimated_output": req.amount
            * 0.997,  # Simplified calculation with 0.3% fee
            "slippage": 0.3,
            "timestamp": datetime.datetime.now().isoformat(),
        }
        ...
        q(
            insert_sql,
            (req.pool_id, json.dumps(event_payload), tx_hash, datetime.datetime.now()),
            fetch=False,
        )
...
def analytics_summary():
    ...
    try:
        ...
        # Recent picks
        picks_sql = (
            "SELECT COUNT(*) as recent_picks FROM daily_picks WHERE created_at > %s"
        )
        week_ago = datetime.datetime.now() - datetime.timedelta(days=7)
        recent_picks = (
            q(picks_sql, (week_ago,))[0][0] if q(picks_sql, (week_ago,)) else 0
        )
        ...
        total_commission = (
            q(affiliate_sql, (month_ago,))[0][0]
            if q(affiliate_sql, (month_ago,))
            else 0.0
        )
        ...
        return {
            ...
            "affiliate": {
                "commission_last_30_days": (
                    float(total_commission) if total_commission else 0.0
                ),
...
def portfolio_performance(portfolio_id: str, days: int = 30):
    ...
    return {
        ...
        "summary": {
            ...
            "total_return": round(
                (
                    performance_data[-1]["portfolio_value"]
                    / performance_data[0]["portfolio_value"]
                    - 1
                )
                * 100,
                2,
