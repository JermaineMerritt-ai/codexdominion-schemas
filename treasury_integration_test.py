...
def archive_transaction(stream, amount, currency="USD", source="", status="pending"):
    ...
    txn_id = treasury.archive_transaction(
        stream=stream, amount=amount, currency=currency, source=source, status=status
    )
...
def treasury_report():
    ...
    if recent_txns:
        ...
        for txn in recent_txns:
            print(
                f"   {txn['id']}: {txn['stream']} ${txn['amount']:.2f} - {txn['status']}"
            )
...
def main():
    ...
    try:
        ...
        direct_id = archive_transaction(
            "consulting", 750.00, "USD", "direct:client", "confirmed"
        )
