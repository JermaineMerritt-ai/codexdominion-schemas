...
class CodexThreadsEngagement:
    ...
    def __init__(self, config_file: str = "threads_config.json"):
        ...
        self.meta_token = self.config.get("threads", {}).get("meta_token") or os.getenv("META_TOKEN")
        self.profile_id = self.config.get("threads", {}).get("profile_id") or os.getenv("THREADS_PROFILE_ID")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "reposts": self._extract_metric_value(data, "shares"),  # Shares = Reposts
                ...
                self.logger.warning(f"Meta Graph API returned status {response.status_code}")
                return self._generate_demo_metrics()
    ...
    def _enhance_metrics(self, base_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_engagement_per_thread = total_engagement / threads_count if threads_count > 0 else 0
    ...
    def _calculate_performance_score(self, metrics: Dict[str, Any]) -> int:
        ...
        try:
            ...
            engagement_score = min((likes + reposts + replies) / 1000 * 40, 40)  # Max 40 points
            reach_score = min(reach / 10000 * 30, 30)                           # Max 30 points
            interaction_quality = min((replies + reposts) / likes * 100 if likes > 0 else 0, 30)  # Max 30 points
    ...
    def _calculate_growth_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "growth_rate": round((likes_growth / previous_likes * 100), 2) if previous_likes > 0 else 0
            }
    ...
    def get_engagement_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            engagement_analysis = self._analyze_engagement_trends(history, current_metrics)
    ...
    def _analyze_engagement_trends(self, history: List[Dict[str, Any]], current: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze engagement trends and patterns"""
        try:
            if len(history) < 2:
                return {"engagement_velocity": "stable", "peak_engagement_time": "unknown"}
            
            # Calculate engagement velocity
            recent_engagements = [entry.get("total_engagement", 0) for entry in history[-14:]]  # Last 14 days
            ...
            peak_hour = max(avg_by_hour, key=avg_by_hour.get) if avg_by_hour else "unknown"
            
            return {
                "engagement_velocity": velocity,
                "peak_engagement_hour": f"{peak_hour}:00" if peak_hour != "unknown" else "unknown",
                "avg_daily_engagement": sum(recent_engagements) / len(recent_engagements) if recent_engagements else 0
            }
    ...
    def _generate_content_insights(self, current: Dict[str, Any], history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if engagement_rate > 5:
                insights.append("🔥 Excellent engagement rate - content resonates strongly")
            elif engagement_rate > 2:
                insights.append("📈 Good engagement - consider posting frequency optimization")
            else:
                insights.append("⚠️ Low engagement - review content strategy")
                recommendations.append("Experiment with trending topics and interactive content")
            
            # Community interaction analysis
            reply_ratio = current.get("reply_to_like_ratio", 0)
            if reply_ratio > 0.3:
                insights.append("💬 Strong conversational engagement - community is highly interactive")
            elif reply_ratio > 0.1:
                insights.append("🗣️ Moderate conversation - encourage more discussion")
            else:
                insights.append("📢 Broadcast style - consider adding conversation starters")
                recommendations.append("Ask questions and create discussion prompts")
            
            # Reach analysis
            reach_rate = current.get("reach_rate", 0)
            if reach_rate > 80:
                insights.append("🎯 Excellent reach efficiency - content is highly discoverable")
            elif reach_rate > 50:
                insights.append("👀 Good reach - content finding its audience")
            else:
                insights.append("📊 Reach optimization needed")
                recommendations.append("Use relevant hashtags and post at optimal times")
