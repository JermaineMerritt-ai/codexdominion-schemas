...
class CodexCeremonialInscriber:
    ...
    def inscribe_ceremony(self, kind: CeremonyKind, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🕯️ Ceremonial {kind} inscribed successfully: {entry['cycle_id']}"
            )
    ...
    def _fallback_local_inscription(
        self, kind: CeremonyKind, message: str
    ) -> Dict[str, Any]:
    ...
    def get_ceremonial_inscriptions(
        self, kind: CeremonyKind = None, limit: int = 10
    ) -> list:
        ...
        try:
            ...
            ceremonial_cycles = [
                c
                for c in cycles
                if c.get("ceremony_type", "").startswith("ceremonial_")
            ]

            # Further filter by kind if specified
            if kind:
                ceremonial_cycles = [
                    c for c in ceremonial_cycles if c.get("kind") == kind
                ]
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        print("Examples:")
        print(
            "  python codex_ceremonial_inscriber.py proclamation 'The eternal flame burns bright'"
        )
        print(
            "  python codex_ceremonial_inscriber.py silence 'In memory of all who came before'"
        )
        print(
            "  python codex_ceremonial_inscriber.py blessing 'May wisdom guide our path'"
        )
        sys.exit(1)
    ...
    if kind not in ["proclamation", "silence", "blessing"]:
        print(
            f"Error: Invalid kind '{kind}'. Must be one of: proclamation, silence, blessing"
        )
        sys.exit(1)
