    ...
    def __init__(self):
        ...
        self.services = {
            ...
            'main_dashboard': {
                ...
                'command': [sys.executable, '-m', 'streamlit', 'run', 'dashboard/app.py', '--server.port', '8502', '--server.address', '127.0.0.1'],
                'port': 8502,
                'url': 'http://127.0.0.1:8502'
            },
            'monetization_crown': {
                'name': 'Enhanced Monetization Crown',
                'command': [sys.executable, '-m', 'streamlit', 'run', 'monetization_crown.py', '--server.port', '8503', '--server.address', '127.0.0.1'],
                'port': 8503,
                'url': 'http://127.0.0.1:8503'
            },
            'portfolio_dashboard': {
                'name': 'Portfolio Management Dashboard',
                'command': [sys.executable, '-m', 'streamlit', 'run', 'codex_portfolio_dashboard.py', '--server.port', '8501', '--server.address', '127.0.0.1'],
                'port': 8501,
    ...
    def monitor_services(self):
        ...
        try:
            while True:
                ...
                for service_name, running_service in list(self.running_services.items()):
                    process = running_service['process']
                    if process.poll() is not None:
                        logger.error(f"❌ {running_service['config']['name']} has stopped unexpectedly")
