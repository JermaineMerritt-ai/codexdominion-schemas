        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "healthy":
                    self.log_test("Health Check", True, f"Engine status: {data.get('engine_status')}")
                else:
    ...
    def test_daily_signals(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test("Daily Signals", True, f"{picks_count} picks in {execution_time}ms")
            else:
                self.log_test("Daily Signals", False, f"Status code: {response.status_code}")
    ...
    def run_all_tests(self):
        ...
        for i in range(max_retries):
            try:
                ...
                if i == max_retries - 1:
                    print("❌ API service not available. Please start the service with:")
