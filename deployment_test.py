...
def main():
    ...
    print("-" * 20)
    commands_to_test = [("docker", "Docker Engine"), ("gcloud", "Google Cloud CLI")]
    ...
    if not gcloud_ok:
        print(
            "   📝 Configure: gcloud auth login && gcloud config set project YOUR_PROJECT_ID"
        )
    ...
    if all_passed and gcloud_ok:
        ...
        print(
            f"   gcloud run deploy codex-backend --image gcr.io/{project_id}/codex-backend \\"
        )
