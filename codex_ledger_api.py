...
class TradingSignal(BaseModel):
    ...
    confidence: Optional[float] = Field(default=0.0, ge=0, le=100, description="Confidence score 0-100")

class AMMPool(BaseModel):
    asset_pair: str = Field(..., max_length=50, description="Asset pair e.g., USDC/ETH")
    strategy_id: Optional[int] = Field(default=None, description="Associated strategy ID")
    weight: float = Field(..., ge=0, le=100, description="Pool weight percentage")
    cap: float = Field(..., gt=0, description="Pool capacity limit")
    state: str = Field(default="active", description="Pool state: active, paused, closed")
...
async def health_check():
    ...
    return {
        ...
        "database": "connected" if get_database_connection()["connected"] else "disconnected"
    }
...
async def write_ledger(entry: LedgerEntry):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to write ledger entry: {str(e)}")
    ...
    try:
        ...
        if currency:
            treasury_data["balances"] = {
                k: v for k, v in treasury_data["balances"].items() 
                if k == currency.upper()
            }
            treasury_data["recent_transactions"] = [
                t for t in treasury_data["recent_transactions"] 
        ...
        raise HTTPException(status_code=500, detail=f"Failed to read treasury: {str(e)}")
...
async def write_treasury(txn: TreasuryTransaction):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to write treasury transaction: {str(e)}")
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "average_apr": sum(p["apr"] for p in filtered_pools) / len(filtered_pools) if filtered_pools else 0,
                "active_pools": len([p for p in filtered_pools if p["state"] == "active"])
            },
...
async def write_pool(pool: AMMPool):
    ...
    try:
        ...
        if pool.weight < 0 or pool.weight > 100:
            raise HTTPException(status_code=400, detail="Pool weight must be between 0 and 100")
