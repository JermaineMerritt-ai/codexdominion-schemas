...
def display_sovereignty_dashboard():
    ...
    print(
        f"   Database Runs: {db_stats['total_runs']} (Success: {db_stats['overall_success_rate']}%)"
    )
    ...
    for run in recent_runs:
        timestamp = (
            run["executed_at"][:19]
            if len(run["executed_at"]) > 19
            else run["executed_at"]
        )
        status_icon = "✅" if run["status"] == "success" else "❌"
        actor_display = run["actor"][:12] if len(run["actor"]) > 12 else run["actor"]

        print(
            f"   {status_icon} {run['capsule_slug']:<20} {actor_display:<12} {timestamp}"
        )
