...
def main():
    ...
    with tabs[0]:  # Dominion Central
        ...
        for scroll in scrolls:
            st.markdown(
                f'<div class="proclamation-box">{scroll}</div>', unsafe_allow_html=True
            )
        ...
        if woocommerce_service:
                ...
                if st.button("🔄 Sync Products"):
                    with st.spinner("Syncing products..."):
                        try:
                            ...
                            st.markdown(
                                f'<div class="success-box">✅ Sync completed: {result["message"]}</div>',
                                unsafe_allow_html=True,
                            )
                        except Exception as e:
                            st.markdown(
                                f'<div class="error-box">❌ Sync failed: {str(e)}</div>',
                                unsafe_allow_html=True,
                            )
            ...
            with col2:
                ...
                try:
                    if woocommerce_service.test_connection():
                        st.markdown(
                            '<div class="success-box">🟢 Store Online</div>',
                            unsafe_allow_html=True,
                        )
                        ...
                        if products:
                            ...
                            for product in products:
                                st.markdown(
                                    f"**{product.get('name', 'Unknown')}** - ${product.get('price', '0')}"
                                )
                    else:
                        st.markdown(
                            '<div class="error-box">🔴 Store Offline</div>',
                            unsafe_allow_html=True,
                        )
                except Exception as e:
                    st.markdown(
                        f'<div class="error-box">❌ Connection Error: {str(e)}</div>',
                        unsafe_allow_html=True,
                    )
        else:
    ...
    with tabs[5]:  # Proclamation System
        ...
        proc_tabs = st.tabs(
            ["🐦 Twitter", "📱 Multi-Platform (Buffer)", "📊 Analytics"]
        )
        ...
        with proc_tabs[0]:  # Twitter Tab
            if twitter_service:
                ...
                with col1:
                    ...
                    create_thread = st.checkbox(
                        "Create as Thread", value=False, key="twitter_thread"
                    )
                        ...
                        if twitter_text:
                            try:
                                with st.spinner("Sending to Twitter..."):
                                    if create_thread:
                                        result = twitter_service.create_thread(
                                            [twitter_text],
                                            twitter_type,
                                            twitter_hashtags,
                                        )
                                    else:
                                        result = twitter_service.send_proclamation(
                                            twitter_text, twitter_type, twitter_hashtags
                                        )

                                    if result.get("success"):
                                        st.success(
                                            f"✅ Twitter post sent! ID: {result.get('tweet_id')}"
                                        )
                                    else:
            ...
            if buffer_service:
                ...
                with col1:
                    ...
                    available_platforms = list(
                        set([p.get("service", "unknown") for p in profiles])
                    )
                    ...
                    # Scheduling option
                    schedule_post = st.checkbox(
                        "📅 Schedule for Later", key="schedule_buffer"
                    )
                    schedule_time = None
                    ...
                    if schedule_post:
                        ...
                        with col_time:
                            schedule_time_input = st.time_input(
                                "Time", key="schedule_time"
                            )

                        schedule_time = datetime.combine(
                            schedule_date, schedule_time_input
                        )
                            ...
                            try:
                                with st.spinner("Sending to multiple platforms..."):
                                    if selected_platforms:
                                        result = (
                                            buffer_service.send_to_specific_platforms(
                                                text=buffer_text,
                                        ...
                                        if schedule_time:
                                            result = (
                                                buffer_service.schedule_proclamation(
                                                    text=buffer_text,
                                            ...
                                            result = (
                                                buffer_service.broadcast_sovereignty(
                                                    buffer_text
                                                )
                                            )
                                    ...
                                    if result.get("success"):
                                        ...
                                        st.success(
                                            f"✅ Sent to {platforms_text}{schedule_text}!"
                                        )
                                    else:
                                        st.error(f"❌ Failed: {result.get('error')}")
                            except Exception as e:
                ...
                with col2:
                    ...
                    try:
                        ...
                        if analytics:
                            st.metric(
                                "📱 Connected Profiles",
                                analytics.get("total_profiles", 0),
                            )
                            st.metric(
                                "📝 Recent Posts", analytics.get("recent_posts", 0)
                            )
                            st.metric(
                                "⏰ Pending Posts", analytics.get("pending_posts", 0)
                            )

                            st.markdown("**🎯 Platforms:**")
                            for platform, count in analytics.get(
        ...
        with proc_tabs[2]:  # Analytics Tab
            ...
            with col1:
                ...
                if twitter_service:
                    try:
                        ...
                        twitter_success = len(
                            [h for h in twitter_history if h.get("success", True)]
                        )
                        ...
                        st.metric(
                            "Success Rate",
                            (
                                f"{(twitter_success/twitter_count*100):.1f}%"
                                if twitter_count > 0
                                else "0%"
                            ),
                        )
            ...
            with col2:
                ...
                if buffer_service:
                    try:
                        ...
                        st.metric(
                            "Connected Platforms",
                            buffer_analytics.get("total_profiles", 0),
                        )
                    except:
            ...
            for activity in all_activities[:10]:
                timestamp = (
                    datetime.fromisoformat(activity["timestamp"]).strftime(
                        "%m/%d %H:%M"
                    )
                ...
                content = (
                    activity.get("text", "")[:50] + "..."
                    if len(activity.get("text", "")) > 50
                    else activity.get("text", "")
                )
    ...
    with tabs[7]:  # Strategic Command
        ...
        for campaign in campaigns:
            ...
            st.markdown(
                f"Status: {campaign['status']} | Progress: {campaign['progress']}%"
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if dawn_service:
                ...
                if st.button("🌅 Generate Dawn Dispatch", type="primary"):
                    with st.spinner("Generating comprehensive system report..."):
                        try:
                            ...
                            if result.get("success"):
                                st.markdown(
                                    '<div class="success-box">✅ Dawn Dispatch generated successfully!</div>',
                                    unsafe_allow_html=True,
                                )
                                ...
                                with metrics_col1:
                                    st.metric(
                                        "💰 Store Revenue",
                                        store_metrics.get("revenue", "N/A"),
                                    )
                                    st.metric(
                                        "📦 Orders", store_metrics.get("orders", 0)
                                    )
                                ...
                                with metrics_col2:
                                    ...
                                    st.metric(
                                        "🐦 Twitter",
                                        social_metrics.get("twitter_posts", 0),
                                    )

                                with metrics_col3:
                                    health_score = system_status.get("health_score", 0)
                                    st.metric(
                                        "⚡ System Health", f"{health_score:.1f}%"
                                    )
                                    st.metric(
                                        "🔧 Services Online",
                                        f"{sum(1 for v in system_status.get('services', {}).values() if v)}/{len(system_status.get('services', {}))}",
                                    )
                ...
                if st.button("📖 View Recent Archives"):
                    try:
                        ...
                        if archive_file.exists():
                            ...
                            if archives:
                                st.markdown(
                                    f"**Found {len(archives)} archived reports**"
                                )
            ...
            with col2:
                ...
                try:
                    ...
                    auto_proclaim = config.get("dispatch_settings", {}).get(
                        "auto_proclaim", False
                    )
                    archive_enabled = config.get("dispatch_settings", {}).get(
                        "archive_enabled", True
                    )

                    st.markdown(f"• Auto Proclaim: {'✅' if auto_proclaim else '❌'}")
                    st.markdown(
                        f"• Archive Enabled: {'✅' if archive_enabled else '❌'}"
                    )
                    st.markdown(
                        f"• Daily Schedule: {config.get('dispatch_settings', {}).get('daily_schedule', 'Not set')}"
                    )
                ...
                st.info(
                    "💡 To enable automatic daily dispatches, configure the scheduler in your deployment environment."
                )
        else:
        ...
        with col1:
            ...
            services = [
                ...
                },
                {
                    ...
                    "status": "🟢 Connected" if twitter_service else "🔴 Offline",
                    "uptime": "98.7%",
                },
                {
                    "name": "Buffer Multi-Platform",
                    "status": (
                        "🟢 Connected"
                        if buffer_service and buffer_service.test_connection()
                        else "🔴 Offline"
                    ),
                    "uptime": "98.2%",
                },
                {
                    ...
                    "status": "🟢 Active" if woocommerce_service else "🔴 Disconnected",
                    "uptime": "97.2%",
                },
                {
