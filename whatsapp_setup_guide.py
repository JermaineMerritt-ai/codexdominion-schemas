...
class WhatsAppSetupGuide:
    ...
    def test_whatsapp_api(self, token: str = None, phone_id: str = None):
        ...
        try:
            ...
            response = requests.get(f"https://graph.facebook.com/v19.0/{phone_id}", 
                                  headers=headers, timeout=10)
            
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Phone API Connection successful!")
                print(f"   Phone Number: {data.get('display_phone_number', 'Not available')}")
                print(f"   Verified Name: {data.get('verified_name', 'Not available')}")
                print(f"   Quality Rating: {data.get('quality_rating', 'Not available')}")
            else:
            ...
            if capabilities_response.status_code == 200:
                ...
                print(f"⚠️  Template access limited: {capabilities_response.status_code}")
