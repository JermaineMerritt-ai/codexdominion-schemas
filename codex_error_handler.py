...
file_handler = logging.FileHandler(
    LOG_DIR / f"codex_system_{datetime.now().strftime('%Y%m%d')}.log"
)
...
error_handler = logging.FileHandler(
    LOG_DIR / f"codex_errors_{datetime.now().strftime('%Y%m%d')}.log"
)
...
class CodexErrorHandler:
    ...
    def __init__(self):
        self.error_log = (
            LOG_DIR / f"error_summary_{datetime.now().strftime('%Y%m%d')}.json"
        )
        self.error_counts = {}
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                logger.warning(
                    f"[{module_name}] {func.__name__} failed, returning default: {default_return}"
                )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        ...
                        logger.error(
                            f"[{platform_name}] API call failed after {max_retries} attempts"
                        )
                        return None
                    else:
                        # Retry
                        logger.warning(
                            f"[{platform_name}] API call attempt {attempt + 1} failed, retrying..."
                        )
                        import time
...
class HealthMonitor:
    ...
    def _check_platform_modules(self) -> dict:
        ...
        modules = [
            "codex_youtube_charts",
            "codex_tiktok_earnings",
            "codex_threads_engagement",
            "codex_whatsapp_business",
            "codex_pinterest_capsule",
            "codex_affiliate_command",
        ]
    ...
    def _check_archive_system(self) -> dict:
        ...
        archive_files = [
            ...
            "completed_archives.jsonl",
            "ledger_youtube.jsonl",
            "ledger_tiktok.jsonl",
            "ledger_threads.jsonl",
            "ledger_whatsapp.jsonl",
            "ledger_pinterest.jsonl",
            "ledger_affiliate.jsonl",
        ]
        ...
        for archive_file in archive_files:
            try:
                if (
                    Path(archive_file).exists()
                    and Path(archive_file).stat().st_size > 0
                ):
                    healthy_archives += 1
            except Exception:
                pass
