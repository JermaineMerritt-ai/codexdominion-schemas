...
tab_summary, tab_contrib, tab_council = st.tabs(["Summary", "Add Contributions", "Council Oversight"])
...
with tab_summary:
    ...
    cols = st.columns(4)
    with cols[0]: st.write(f"🛡 Custodians: {len(ac.get('custodians', []))}")
    with cols[1]: st.write(f"🌱 Heirs: {len(ac.get('heirs', []))}")
    with cols[2]: st.write(f"🌍 Customers: {len(ac.get('customers', []))}")
    with cols[3]: st.write(f"🏛 Councils: {len(ac.get('councils', []))}")
