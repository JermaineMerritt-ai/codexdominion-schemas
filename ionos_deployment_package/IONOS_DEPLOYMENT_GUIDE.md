...
1. **Port not accessible**: Check firewall with `sudo ufw status`
1. **Domain not loading**: Verify DNS settings and nginx config
1. **Permission errors**: Ensure files are owned by `codex` user
