...
def display_ebook_preview(ebook_data):
    ...
    with col1:
        ...
        st.write(f"**Created:** {datetime.fromisoformat(metadata.get('created', '2024-01-01')).strftime('%B %d, %Y')}")
...
def main():
    ...
    if not ebooks:
        st.warning("No ebooks found. Generate some ebooks first using the Codex Ebook Generator!")
    ...
    if selected_ebook:
        ...
        display_mode = st.sidebar.radio(
            "Display Mode:",
            ["Preview", "Read HTML", "Read Markdown", "View JSON"]
        )
