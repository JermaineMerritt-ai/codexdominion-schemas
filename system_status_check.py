...
def check_system_status():
    ...
    try:
        ...
        print(f"✅ Ledger loads successfully - {len(ledger_data.get('entries', []))} entries")
        status["passed"] += 1
    ...
    try:
        ...
        if const_path.exists():
            ...
            print(f"✅ Constellations load successfully - {len(const_data.get('constellations', []))} constellations")
            status["passed"] += 1
        else:
    ...
    success_rate = (status["passed"] / status["total_checks"]) * 100 if status["total_checks"] > 0 else 0
    ...
    status_report = {
        ...
        "system_status": "OPERATIONAL" if status["failed"] == 0 else f"{status['failed']} ISSUES",
        "issues": status["issues"],
        "active_services": [
            {"name": "Revenue Crown Constellation", "url": "http://localhost:8066", "status": "running"},
            {"name": "Unified Codex Dashboard", "url": "http://localhost:8099", "status": "running"}
        ]
    }
