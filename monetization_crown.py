...
try:
    from codex_error_handler import (
        safe_execute, safe_file_operation, safe_api_call, 
        log_info, log_error, log_warning, health_monitor,
        get_system_status
    )
    ERROR_HANDLING_AVAILABLE = True
    log_info("Enhanced Monetization Crown starting with full error handling", "monetization_crown")
except ImportError:
    ERROR_HANDLING_AVAILABLE = False
    # Fallback functions
    def log_info(msg, module="system"): pass
    def log_error(msg, module="system"): st.error(f"[{module}] {msg}")
    def log_warning(msg, module="system"): st.warning(f"[{module}] {msg}")
...
youtube_functions = safe_import('codex_youtube_charts', ['youtube_metrics', 'archive_youtube'])
youtube_metrics = youtube_functions.get('youtube_metrics') if youtube_functions else None
archive_youtube = youtube_functions.get('archive_youtube') if youtube_functions else None

# Import TikTok functions
tiktok_functions = safe_import('codex_tiktok_earnings', ['tiktok_metrics', 'archive_tiktok'])
...
threads_functions = safe_import('codex_threads_engagement', ['threads_metrics', 'archive_threads'])
threads_metrics = threads_functions.get('threads_metrics') if threads_functions else None
archive_threads = threads_functions.get('archive_threads') if threads_functions else None

# Import WhatsApp functions
whatsapp_functions = safe_import('codex_whatsapp_business', ['whatsapp_metrics', 'whatsapp_broadcast', 'archive_whatsapp'])
whatsapp_metrics = whatsapp_functions.get('whatsapp_metrics') if whatsapp_functions else None
whatsapp_broadcast = whatsapp_functions.get('whatsapp_broadcast') if whatsapp_functions else None
archive_whatsapp = whatsapp_functions.get('archive_whatsapp') if whatsapp_functions else None

# Import Pinterest functions
pinterest_functions = safe_import('codex_pinterest_capsule', ['pinterest_metrics', 'pinterest_pin', 'archive_pinterest'])
pinterest_metrics = pinterest_functions.get('pinterest_metrics') if pinterest_functions else None
pinterest_pin = pinterest_functions.get('pinterest_pin') if pinterest_functions else None
archive_pinterest = pinterest_functions.get('archive_pinterest') if pinterest_functions else None

# Import Dawn Monetization Scroll functions
dawn_functions = safe_import('dawn_monetization_scroll', ['dawn_monetization_scroll', 'get_monetization_history'])
dawn_monetization_scroll_func = dawn_functions.get('dawn_monetization_scroll') if dawn_functions else None
get_monetization_history = dawn_functions.get('get_monetization_history') if dawn_functions else None

# Import Affiliate Command functions
affiliate_functions = safe_import('codex_affiliate_command', ['fetch_affiliate_metrics', 'archive_affiliate'])
fetch_affiliate_metrics = affiliate_functions.get('fetch_affiliate_metrics') if affiliate_functions else None
archive_affiliate = affiliate_functions.get('archive_affiliate') if affiliate_functions else None
...
def main():
    ...
    # Main header
    st.markdown('<h1 class="main-header">👑 Monetization Crown 🔥</h1>', unsafe_allow_html=True)
    st.markdown("### 🚀 **Multi-Platform Creator Economy Command Center**")
    
    # Platform availability status
    platforms_available = []
    if youtube_metrics: platforms_available.append("📺 YouTube")
    if tiktok_metrics: platforms_available.append("🎵 TikTok")
    if threads_metrics: platforms_available.append("🧵 Threads")
    if whatsapp_metrics: platforms_available.append("💬 WhatsApp")
    if pinterest_metrics: platforms_available.append("📌 Pinterest")
    if fetch_affiliate_metrics: platforms_available.append("💎 Affiliate")
    ...
    if st.session_state.get('dawn_scroll'):
        ...
        with scroll_col3:
            ...
            st.metric("👑 Top Platform", top_platform.title() if top_platform != 'None' else 'None')
        
        with scroll_col4:
            performance_tier = summary.get('performance_tier', 'startup')
            tier_emoji = {"elite": "👑", "advanced": "⭐", "growing": "📈", "emerging": "🌱", "startup": "🚀"}.get(performance_tier, "🔥")
        ...
        if total_revenue >= 500:
            st.success("🔥 **ADVANCED TIER ACHIEVED** - Excellent monetization performance!")
    ...
    tab_youtube, tab_tiktok, tab_threads, tab_whatsapp, tab_pinterest, tab_affiliate, tab_portfolio = st.tabs([
        "📺 YouTube Charts", "🎵 TikTok Earnings", "🧵 Threads Engagement", "💬 WhatsApp Business", "📌 Pinterest Capsule", "💎 Affiliate Command", "📈 Portfolio Manager"
    ])
    ...
    with tab_youtube:
        ...
        if youtube_metrics:
            ...
            if st.button("📊 Fetch YouTube Metrics", key="youtube_fetch", type="primary"):
                if ch_id:
                        ...
                        try:
                            ...
                            if m.get("error"):
                                ...
                                if len(m.keys()) > 3:
                                    ...
                                    extra_keys = [k for k in m.keys() if k not in ['subscribers', 'views', 'videos', 'error']]
                                ...
                                if archive_youtube:
                                    ...
                                    if archive_success:
                                        st.success("✅ YouTube metrics archived successfully!")
                        ...
                        try:
                            ...
                            if m.get("error"):
                                ...
                                for row in range(rows_needed):
                                    ...
                                    end_idx = min(start_idx + cols_per_row, len(metric_keys))
                                    ...
                                    for i, key in enumerate(metric_keys[start_idx:end_idx]):
                                        with cols[i]:
                                            ...
                                            if isinstance(value, (int, float)):
                                                if "revenue" in key.lower() or "earning" in key.lower():
                                ...
                                if archive_tiktok:
                                    ...
                                    if archive_success:
                                        st.success("✅ TikTok metrics archived successfully!")
    ...
    with tab_threads:
        ...
        if threads_metrics:
            ...
            with col1:
                if st.button("🧵 Fetch Threads Metrics", key="threads_fetch", type="primary"):
                    with st.spinner("Fetching Threads engagement..."):
                        try:
                            ...
                            if m.get("error"):
                                ...
                                metric_keys = [k for k in m.keys() if k != 'error']
                                cols_per_row = 3
                                rows_needed = (len(metric_keys) + cols_per_row - 1) // cols_per_row
                                        ...
                                        with cols[i]:
                                            ...
                                            if isinstance(value, (int, float)):
                                                if key.endswith("_rate") or key.endswith("_score"):
                                                    st.metric(display_key, f"{value:.1f}{'%' if '_rate' in key else ''}")
                                                else:
                                                    st.metric(display_key, f"{value:,}")
                                            else:
                                                st.metric(display_key, str(value))
                                ...
                                if archive_threads:
                                    ...
                                    if archive_success:
                                        st.success("✅ Threads metrics archived successfully!")
    ...
    with tab_whatsapp:
        ...
        if whatsapp_metrics and whatsapp_broadcast:
                        ...
                        with st.spinner("Dispatching broadcast..."):
                            try:
                                ...
                                if result.get("error"):
                                    ...
                                    st.markdown('<div class="success-message">✅ Broadcast dispatched successfully!</div>', unsafe_allow_html=True)
                                    
                                    # Show results if available
                                    if result.get('successful_sends') is not None:
                                        result_col1, result_col2, result_col3 = st.columns(3)
                                        with result_col1:
                                            st.metric("✅ Successful", result.get('successful_sends', 0))
                                        with result_col2:
                                            st.metric("❌ Failed", result.get('failed_sends', 0))
                                        with result_col3:
                                            st.metric("💰 Est. Cost", f"${result.get('estimated_cost', 0):.2f}")
                            except Exception as e:
            ...
            if st.button("📈 Fetch WhatsApp Metrics", key="whatsapp_fetch", type="primary"):
                    ...
                    try:
                        ...
                        if m.get("error"):
                            ...
                            for row in range(rows_needed):
                                ...
                                for i, key in enumerate(metric_keys[start_idx:end_idx]):
                                    with cols[i]:
                                        ...
                                        display_key = key.replace("_", " ").title()
                                        if isinstance(value, (int, float)):
                                            if "cost" in key.lower() or "revenue" in key.lower():
                                                st.metric(display_key, f"${value:.2f}")
                            ...
                            if archive_whatsapp:
                                ...
                                if archive_success:
                                    st.success("✅ WhatsApp metrics archived successfully!")
    ...
    with tab_pinterest:
        ...
        if pinterest_metrics and pinterest_pin:
            ...
            if st.button("📌 Fetch Pinterest Metrics", key="pinterest_fetch", type="primary"):
                with st.spinner("Fetching Pinterest analytics..."):
                    try:
                        ...
                        if m.get("error"):
                            ...
                            # Display all metrics dynamically
                            metric_keys = [k for k in m.keys() if k not in ['error', 'demo', 'last_updated']]
                            cols_per_row = 3
                            rows_needed = (len(metric_keys) + cols_per_row - 1) // cols_per_row
                                    ...
                                    with cols[i]:
                                        ...
                                        if isinstance(value, (int, float)):
                                            if "revenue" in key.lower() or "cost" in key.lower():
                                                st.metric(display_key, f"${value:.2f}")
                                            elif key.endswith("_rate") or key.endswith("_score") or "percentage" in key.lower():
                                                st.metric(display_key, f"{value:.1f}{'%' if '_rate' in key or 'percentage' in key else ''}")
                                            else:
                                                st.metric(display_key, f"{value:,}")
                                        else:
                                            st.metric(display_key, str(value))
                            ...
                            if archive_pinterest:
                                ...
                                if archive_success:
                                    st.success("✅ Pinterest metrics archived successfully!")
                    ...
                    with st.spinner("Publishing pin..."):
                        try:
                            ...
                            if result.get("success"):
                                st.markdown('<div class="success-message">✅ Pin published successfully!</div>', unsafe_allow_html=True)
                                if result.get('pin_url'):
                                    st.info(f"🔗 **Pin URL:** {result['pin_url']}")
                            else:
                                st.error(f"❌ Pin publication failed: {result.get('error', 'Unknown error')}")
                        except Exception as e:
    ...
    with tab_affiliate:
        ...
        if fetch_affiliate_metrics:
            ...
            with col1:
                ...
                if st.button("💎 Fetch Affiliate Metrics", key="affiliate_fetch", type="primary"):
                    with st.spinner("Fetching affiliate analytics..."):
                        try:
                            ...
                            if m.get("error"):
                                ...
                                with adv_col3:
                                    profitability_tier = m.get('profitability_tier', 'startup')
                                    tier_emoji = {"elite": "👑", "professional": "⭐", "advanced": "📈", "developing": "🌱", "emerging": "🚀", "startup": "💡"}.get(profitability_tier, "🔥")
                                    st.metric(f"{tier_emoji} Tier", profitability_tier.title())
                                ...
                                if m.get('conversion_quality'):
                                    ...
                                    with analysis_col1:
                                        conversion_quality = m.get('conversion_quality', 'unknown')
                                        quality_emoji = {"exceptional": "🌟", "excellent": "🟢", "good": "🟡", "fair": "🟠", "needs_improvement": "🔴"}.get(conversion_quality, "❓")
                                        st.markdown(f"**{quality_emoji} Conversion Quality:** {conversion_quality.replace('_', ' ').title()}")
                                        
                                        traffic_quality = m.get('traffic_quality', 'unknown')
                                        traffic_emoji = {"premium": "💎", "high": "🟢", "good": "🟡", "average": "🟠", "low": "🔴"}.get(traffic_quality, "❓")
                                        st.markdown(f"**{traffic_emoji} Traffic Quality:** {traffic_quality.title()}")
                                        
                                        optimization_potential = m.get('optimization_potential', 'unknown')
                                        opt_emoji = {"optimized": "✅", "low": "🟢", "moderate": "🟡", "high": "🔴"}.get(optimization_potential, "❓")
                                        st.markdown(f"**{opt_emoji} Optimization Potential:** {optimization_potential.title()}")
                                    
                                    with analysis_col2:
                                        growth_trend = m.get('growth_trend', 'unknown')
                                        growth_emoji = {"accelerating": "🚀", "growing": "📈", "stable": "📊", "declining": "📉", "concerning": "🆘"}.get(growth_trend, "❓")
                                        st.markdown(f"**{growth_emoji} Growth Trend:** {growth_trend.title()}")
                                        
                                        revenue_consistency = m.get('revenue_consistency', 'unknown')
                                        consistency_emoji = {"very_consistent": "🎯", "consistent": "🟢", "moderate": "🟡", "volatile": "🔴"}.get(revenue_consistency, "❓")
                                        st.markdown(f"**{consistency_emoji} Revenue Consistency:** {revenue_consistency.replace('_', ' ').title()}")
                                        
                                        active_networks = m.get('active_networks', 0)
                                        diversification = m.get('network_diversification', 0)
                                        st.markdown(f"**🌐 Network Diversification:** {active_networks} networks ({diversification:.1f}%)")
                                ...
                                if m.get('top_network'):
                                    ...
                                    with network_col2:
                                        top_commission = m.get('top_network_commission', 0)
                                        st.metric("💰 Top Network Revenue", f"${top_commission:.2f}")
                                ...
                                if archive_affiliate:
                                    ...
                                    if archive_success:
                                        st.success("✅ Affiliate metrics archived successfully!")
            ...
            with col2:
                ...
                try:
                    ...
                    if history:
                        ...
                        for entry in history[-5:]:
                            ...
                            st.markdown(f"• {timestamp} - {conversions} conv, ${commission:.2f}")
                    else:
                ...
                calc_clicks = st.number_input("Expected Clicks", min_value=0, value=1000, step=100)
                calc_conv_rate = st.number_input("Conversion Rate (%)", min_value=0.0, value=2.5, step=0.1, format="%.1f")
                calc_avg_commission = st.number_input("Avg Commission ($)", min_value=0.0, value=25.0, step=1.0, format="%.2f")
    ...
    with tab_portfolio:
        ...
        try:
            from codex_database import get_portfolio_summary, get_affiliate_performance, get_top_pools
        ...
        if portfolio_available:
            ...
            with col1:
                ...
                portfolio_data = {
                    ...
                    'positions': [
                        {'symbol': 'AAPL', 'quantity': 100, 'market_value': 15525.00, 'unrealized_pnl': 525.00},
                        {'symbol': 'MSFT', 'quantity': 50, 'market_value': 15525.00, 'unrealized_pnl': 525.00},
                        {'symbol': 'GOOGL', 'quantity': 25, 'market_value': 61875.00, 'unrealized_pnl': -625.00}
                    ]
                }
                ...
                positions_df = pd.DataFrame([
                    {
                        ...
                        'P&L': f"${pos['unrealized_pnl']:,.2f}" if pos['unrealized_pnl'] >= 0 else f"-${abs(pos['unrealized_pnl']):,.2f}"
                    }
            ...
            with col2:
                ...
                market_data = [
                    {"Index": "S&P 500", "Value": "4,185.47", "Change": "+0.8%", "Status": "🟢"},
                    {"Index": "NASDAQ", "Value": "12,870.00", "Change": "+1.2%", "Status": "🟢"},
                    {"Index": "DOW", "Value": "33,745.40", "Change": "+0.3%", "Status": "🟢"}
                ]
                
                for market in market_data:
                    st.markdown(f"**{market['Index']}**: {market['Value']} ({market['Change']}) {market['Status']}")
