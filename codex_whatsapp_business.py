...
class CodexWhatsAppBusiness:
    ...
    def __init__(self, config_file: str = "whatsapp_config.json"):
        ...
        self.whatsapp_token = self.config.get("whatsapp", {}).get("token") or os.getenv(
            "WHATSAPP_TOKEN"
        )
        self.phone_id = self.config.get("whatsapp", {}).get("phone_id") or os.getenv(
            "WHATSAPP_PHONE_ID"
        )
        self.business_id = self.config.get("whatsapp", {}).get(
            "business_id"
    ...
    def _fetch_whatsapp_analytics(self, phone_id: str) -> Dict[str, Any]:
        ...
        try:
            if (
                not self.whatsapp_token
                or self.whatsapp_token == "YOUR_WHATSAPP_TOKEN_HERE"
            ):
                return self._generate_demo_metrics()
            ...
            response = requests.get(
                analytics_url, headers=headers, params=params, timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "messages_delivered": self._extract_metric_value(
                ...
                self.logger.warning(
                    f"WhatsApp Business API returned status {response.status_code}"
                )
                return self._generate_demo_metrics()
    ...
    def _generate_demo_metrics(self) -> Dict[str, Any]:
        ...
        messages_delivered = int(
            messages_sent * (0.85 + random.random() * 0.1)
        )  # 85-95% delivery rate
        messages_read = int(
            messages_delivered * (0.65 + random.random() * 0.2)
        )  # 65-85% read rate
        cost = base_cost + random.uniform(-15.0, 25.0)
        leads = int(
            conversations * (0.10 + random.random() * 0.10)
    ...
    def _enhance_metrics(self, base_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            engagement_score = (
                delivery_rate * 0.4 + read_rate * 0.6
            ) * 100  # Weighted engagement
            ...
            roi_percentage = (
                ((estimated_revenue - cost) / cost * 100) if cost > 0 else 0
            )
    ...
    def _calculate_growth_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "growth_rate": (
                    round((conversation_growth / previous_conversations * 100), 2)
    ...
    def whatsapp_broadcast(
        self, message: str, recipients: List[str], template_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            batch_size = self.config.get("broadcast_settings", {}).get(
                "max_recipients_per_batch", 100
            )
            batch_delay = self.config.get("broadcast_settings", {}).get(
                "batch_delay_seconds", 1
            )
            ...
            broadcast_results["estimated_cost"] = self._calculate_broadcast_cost(
                broadcast_results["successful_sends"]
            )
            broadcast_results["success_rate"] = (
                broadcast_results["successful_sends"] / len(recipients)
            ) * 100
    ...
    def _send_batch(
        ...
        results = {"successful": 0, "failed": 0, "errors": [], "message_ids": []}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    results["errors"].append(
                        f"Failed to send to {recipient}: {response.text}"
                    )
    ...
    def _simulate_broadcast(
        self, message: str, recipients: List[str]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_cost": round(
                successful_sends * 0.025, 2
            ),  # Estimated $0.025 per message
            "message_ids": [f"demo_msg_{i}" for i in range(successful_sends)],
            "errors": (
                [f"Demo error {i}" for i in range(failed_sends)]
                if failed_sends > 0
                else []
            ),
            "demo": True,
    ...
    def get_business_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            business_insights = self._analyze_business_performance(
    ...
    def _analyze_business_performance(
        ...
        try:
            ...
            recent_conversations = [
                entry.get("conversations", 0) for entry in history[-14:]
            ]
            ...
            return {
                ...
                "delivery_quality": (
                    ...
                    else "good" if delivery_rate > 0.8 else "needs_improvement"
                ),
                "engagement_quality": (
                    "high"
                    if read_rate > 0.7
                    else "medium" if read_rate > 0.5 else "low"
                ),
            }
    ...
    def _analyze_cost_efficiency(
        ...
        try:
            ...
            daily_costs = [
                entry.get("messaging_cost_usd", 0) for entry in history[-30:]
            ]
            if daily_costs:
            ...
            return {
                ...
                "roi_status": (
                    "profitable"
                    if roi_percentage > 0
                    else "breaking_even" if roi_percentage == 0 else "loss"
                ),
            }
    ...
    def _analyze_campaign_performance(
        ...
        try:
            ...
            campaigns = [
                entry for entry in history if entry.get("type") == "broadcast_campaign"
            ]
            ...
            success_rates = [
                c.get("success_rate", 0) for c in campaigns if c.get("success_rate")
            ]
            avg_success_rate = (
            ...
            return {
                ...
                "last_campaign_date": (
                    campaigns[-1].get("ts", "")[:10] if campaigns else ""
                ),
            }
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.graph_base}/{self.phone_id}", headers=headers, timeout=5
            )
...
def whatsapp_broadcast(
    message: str, recipients: List[str], template_name: Optional[str] = None
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    broadcast_result = whatsapp_system.whatsapp_broadcast(
        "Test message", demo_recipients
    )
