...
cols = st.columns(4)
with cols[0]:
    st.write(f"🛡 Custodians: {len(ac['custodians'])}")
with cols[1]:
    st.write(f"🌱 Heirs: {len(ac['heirs'])}")
with cols[2]:
    st.write(f"🌍 Customers: {len(ac['customers'])}")
with cols[3]:
    st.write(f"🏛 Councils: {len(ac['councils'])}")
...
if data.get("contributions"):
    ...
    for c in data["contributions"]:
        st.write(
            f"• {c['id']} — {c['kind']} by {c['name']} ({c['role']}) - {c['timestamp']}"
        )
        if c.get("text"):
            st.write(
                f"  *\"{c['text'][:100]}{'...' if len(c['text']) > 100 else ''}\"*"
            )
