...
class CodexDatabase:
    ...
    async def get_portfolio_performance(self, portfolio_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "return_percentage": (
                float((total_unrealized_pnl / total_cost * 100))
                if total_cost > 0
                else 0.0
            ),
            "positions": [
                {
                    ...
                    "weight": (
                        float(pos.market_value / total_market_value * 100)
    ...
    async def record_affiliate_metrics(
        self, program: str, clicks: int, conversions: int, commission: Decimal
    ) -> AffiliateMetrics:
