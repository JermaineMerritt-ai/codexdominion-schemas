...
def dawn_monetization_scroll():
    ...
    def calculate_platform_revenue(platform_data, platform_name):
        ...
        if platform_name == "youtube":
            ...
            revenue = latest_entry.get("estimated_revenue", 0) - latest_entry.get(
    ...
    def generate_monetization_insights(platform_revenues):
        ...
        active_platforms = sum(
            1 for p in platform_revenues.values() if p["status"] == "active"
        )
        avg_performance = (
            sum(p["performance_score"] for p in platform_revenues.values())
            / len(platform_revenues)
        ...
        top_platform = (
            max(platform_revenues.items(), key=lambda x: x[1]["revenue"])
            if platform_revenues
            else (None, {"revenue": 0})
        )
        ...
        growth_potential = (
            ...
            else "moderate" if avg_performance > 50 else "developing"
        )
        ...
        return {
            ...
            "diversification_score": round(
                (active_platforms / 6) * 100, 1
            ),  # Updated for 6 platforms including affiliate
        }
    ...
    try:
        ...
        monetization_report = {
            ...
            "sovereignty_status": (
                "active" if insights["total_revenue"] > 0 else "building"
            ),
        }
        ...
        logger.info(
            f"🔥 Dawn Monetization Scroll completed - Total Revenue: ${insights['total_revenue']:.2f}"
        )
...
def calculate_revenue_growth(current_revenue, historical_data):
    ...
    try:
        previous_revenue = historical_data[1]["monetization"]["insights"][
            "total_revenue"
        ]
