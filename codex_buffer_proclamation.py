...
class CodexBufferProclaimer:
    ...
    def _load_profiles(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/profiles.json", params={"access_token": self.token}
            )
    ...
    def format_proclamation(
        self, text: str, proclamation_type: str = "sovereignty"
    ) -> str:
    ...
    def send_proclamation(
        ...
        try:
            ...
            if not profile_ids:
                return {"success": False, "error": "No Buffer profiles available"}
            ...
            if media_urls:
                update_data["media"] = {
                    "link": media_urls[0]
                }  # Buffer supports one media item

            # Send the update
            response = requests.post(
                f"{self.base_url}/updates/create.json", data=update_data
            )
            ...
            if response.status_code == 200:
                ...
                error_msg = (
                    f"Buffer API error: {response.status_code} - {response.text}"
                )
    ...
    def broadcast_sovereignty(
        self, text: str, exclude_platforms: List[str] = None
    ) -> Dict[str, Any]:
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics = {
                ...
                "recent_posts": len(
                    [h for h in self.proclamation_history if h.get("success", False)]
                ),
            ...
            for profile in self.profiles:
                ...
                analytics["profiles_by_platform"][service] = (
                    analytics["profiles_by_platform"].get(service, 0) + 1
                )
    ...
    def get_proclamation_history(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        return (
            self.proclamation_history[-limit:] if limit else self.proclamation_history
        )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/user.json", params={"access_token": self.token}
            )
...
def proclaim_social(text: str, profile_ids: List[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        result = buffer_client.send_proclamation(
            text=text, profile_ids=profile_ids, proclamation_type="sovereignty"
        )
