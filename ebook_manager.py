...
class CodexEbookManager:
    ...
    def create_ebook_from_template(self, template_type: str, **kwargs) -> Optional[Dict[str, str]]:
...
def run_ebook_management_dashboard():
    ...
    st.set_page_config(
        page_title="Codex Ebook Manager",
        page_icon="📚",
        layout="wide"
    )
...
def show_library_page(manager):
    ...
    if not ebooks:
        st.info("No ebooks found. Create your first ebook using the 'Create New' section!")
        return
...
def show_create_page(manager):
    ...
    if creation_type == "📜 From Proclamations":
        ...
        with st.form("comprehensive_form"):
            title = st.text_input("Ebook Title", value="Codex Dominion: Complete System Guide")
        ...
        with st.form("custom_form"):
            ...
            num_chapters = st.number_input("Number of Chapters", min_value=1, max_value=20, value=3)
            
            chapters = []
            for i in range(num_chapters):
                st.write(f"**Chapter {i+1}:**")
                chapter_title = st.text_input(f"Chapter {i+1} Title", value=f"Chapter {i+1}")
                chapter_content = st.text_area(f"Chapter {i+1} Content", height=150, key=f"chapter_{i}")
