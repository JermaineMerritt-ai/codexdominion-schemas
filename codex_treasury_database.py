        ...
        if not db_password:
            try:
                ...
                result = subprocess.run(
                    ['gcloud', 'secrets', 'versions', 'access', 'latest', '--secret=codex-db-pass'],
...
class CodexTreasury:
    ...
    def _init_connection_pool(self) -> None:
        ...
        try:
            ...
            if os.environ.get('INSTANCE_CONNECTION_NAME') or os.environ.get('DATABASE_URL'):
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    result = cur.fetchone()
                    if result:
                        env_type = "Cloud SQL" if os.environ.get('INSTANCE_CONNECTION_NAME') else "Local PostgreSQL"
    ...
    def _store_in_postgres(self, txn_data: Dict[str, Any]) -> None:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                ...
                """, {
                    **txn_data,
                    "metadata": json.dumps(txn_data["metadata"])
                })
    ...
    def _store_in_ledger(self, txn_data: Dict[str, Any]) -> None:
        ...
        summary["total_revenue"] = summary.get("total_revenue", 0.0) + txn_data["amount"]
    ...
    def ingest_affiliate(self, order_id: str, amount: float, currency: str = "USD", **kwargs) -> str:
    ...
    def ingest_stock(self, symbol: str, amount: float, currency: str = "USD", **kwargs) -> str:
    ...
    def ingest_amm(self, pool_id: str, amount: float, currency: str = "USD", **kwargs) -> str:
        ...
        return self.archive_transaction(
            ...
            metadata={
                "pool_id": pool_id,
                "transaction_type": "amm_swap",
                **kwargs
            }
        )
    
    def ingest_consulting(self, client_id: str, hours: float, rate: float, currency: str = "USD", **kwargs) -> str:
    ...
    def ingest_development(self, project_id: str, amount: float, currency: str = "USD", **kwargs) -> str:
    ...
    def _get_ledger_summary(self, days: int) -> Dict[str, Any]:
        ...
        recent_txns = [
            ...
            if datetime.datetime.fromisoformat(txn["created_at"].replace("Z", "")) > cutoff
        ]
        ...
        for txn in recent_txns:
            ...
            if not streams[stream]["last_transaction"] or txn["created_at"] > streams[stream]["last_transaction"]:
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    # Create indexes
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_transactions_stream ON transactions(stream);")
...
def main():
    ...
    affiliate_txn = treasury.ingest_affiliate("A123", 49.99, metadata={"platform": "WooCommerce"})
    print(f"✅ Affiliate transaction: {affiliate_txn}")
    
    # Stock trading
    stock_txn = treasury.ingest_stock("MSFT", 1000.00, metadata={"shares": 10, "price_per_share": 100.00})
    print(f"✅ Stock transaction: {stock_txn}")
    
    # AMM pool
    amm_txn = treasury.ingest_amm("pool-eth-usdc", 500.00, metadata={"pool_type": "liquidity", "apr": 12.5})
    print(f"✅ AMM transaction: {amm_txn}")
    
    # Consulting
    consulting_txn = treasury.ingest_consulting("client-001", 8.0, 150.00, metadata={"project": "AI Development"})
