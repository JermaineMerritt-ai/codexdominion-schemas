...
class CodexAffiliateCommand:
    ...
    def __init__(self, config_file: str = "affiliate_config.json"):
        ...
        self.api_key = self.config.get('affiliate_api_key', os.getenv('AFFILIATE_API_KEY', ''))
    ...
    def _process_affiliate_analytics(self, data: Dict) -> Dict[str, Any]:
        ...
        top_network = max(network_breakdown.items(), key=lambda x: x[1].get('commission', 0)) if network_breakdown else (None, {})
        ...
        return {
            ...
            'optimization_potential': self._assess_optimization_potential(conversion_rate, earnings_per_click),
            
            # Growth metrics
            'monthly_projection': round(commission_usd * 30 / self.config.get('analytics_days', 30), 2),
            ...
            'network_diversification': self._calculate_diversification_score(network_breakdown),
            
            # Quality metrics
            'conversion_quality': self._assess_conversion_quality(conversion_rate),
            'revenue_consistency': self._assess_revenue_consistency(data.get('daily_breakdown', [])),
    ...
    def _calculate_affiliate_performance_score(self, conversion_rate: float, 
                                            epc: float, commission: float) -> float:
        ...
        conversion_score = min(conversion_rate / 3.0 * 100, 100)  # 3% is excellent conversion rate
    ...
    def _analyze_affiliate_growth(self, data: Dict) -> Dict[str, Any]:
        ...
        previous_commission = sum(day.get('commission', 0) for day in previous_week) if previous_week else recent_commission
        
        if previous_commission == 0:
            growth_rate = 0
            trend = 'new'
        else:
            growth_rate = ((recent_commission - previous_commission) / previous_commission) * 100
    ...
    def _calculate_diversification_score(self, network_breakdown: Dict) -> float:
        ...
        total_commission = sum(net.get('commission', 0) for net in network_breakdown.values())
        if total_commission == 0:
            return 0.0
        
        # Calculate Herfindahl index for diversification
        shares = [(net.get('commission', 0) / total_commission) ** 2 for net in network_breakdown.values()]
    ...
    def _assess_revenue_consistency(self, daily_data: List[Dict]) -> str:
        ...
        variance = sum((c - avg_commission) ** 2 for c in daily_commissions) / len(daily_commissions)
    ...
    def _get_demo_affiliate_metrics(self) -> Dict[str, Any]:
        ...
        base_conversions = random.randint(int(base_clicks * 0.01), int(base_clicks * 0.05))
        ...
        average_commission = base_commission / base_conversions if base_conversions > 0 else 0
        ...
        return {
            ...
            'profitability_tier': random.choice(['advanced', 'professional', 'developing']),
            ...
            'top_network': random.choice(['ShareASale', 'Commission Junction', 'ClickBank', 'Amazon Associates']),
            ...
            'revenue_consistency': random.choice(['consistent', 'very_consistent', 'moderate']),
