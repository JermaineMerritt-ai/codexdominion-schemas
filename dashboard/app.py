...
def make_request(method, endpoint, data=None, timeout=10):
    ...
    try:
        ...
        return None, "Connection Error: API server not available. Please ensure the API is running on port 8000."
...
def main():
    ...
    with tab_portfolio:
        ...
        with col1:
            ...
            pid = st.text_input("Portfolio ID", placeholder="Enter portfolio ID (e.g., portfolio-123)")
                ...
                if pid:
                    ...
                    if data and 'positions' in data:
                        ...
                        with col_d:
                            ...
                            st.metric("Unrealized P&L", f"${pnl:,.2f}", delta=f"{summary.get('portfolio_return_percent', 0):.1f}%")
                        ...
                        if positions:
                            ...
                            for p in positions:
                                with st.container():
                                    col_sym, col_qty, col_price, col_value, col_pnl = st.columns([2, 2, 2, 2, 2])
                                    ...
                                    with col_price:
                                        ...
                                        price_change = last_price - avg_cost if last_price and avg_cost else 0
                                    ...
                                    with col_pnl:
                                        ...
                                        st.metric("P&L", f"${unrealized_pnl:,.2f}", delta=f"{return_pct:.1f}%")
        ...
        with col2:
            ...
            if pid:
                perf_data, perf_error = make_request("GET", f"/portfolio/{pid}/performance?days=30")
                ...
                if perf_data and 'performance' in perf_data:
                    ...
                    if not df.empty:
                        ...
                        with col_start:
                            st.metric("Starting Value", f"${summary.get('starting_value', 0):,.2f}")
                        with col_end:
                            st.metric("Current Value", f"${summary.get('ending_value', 0):,.2f}")
                        with col_return:
                ...
                if uid and symbols_input:
                    ...
                    if symbols:
                        ...
                        if result:
                            ...
                            with col_b:
                                st.metric("Symbols Analyzed", result.get('symbols_analyzed', 0))
                            with col_c:
                                expected_return = result.get('performance', {}).get('expected_return', 0)
    ...
    with tab_ai:
        ...
        st.info("🎯 AI Builder proposes allocations by risk tier and detects missing exposures.")
        
        col1, col2 = st.columns([1, 1])
        
        with col1:
            st.subheader("🎚️ Risk Tier Selection")
            tier = st.selectbox("Select Risk Tier", ["conservative", "moderate", "aggressive"])
            ...
            # Display as chart
            df_allocation = pd.DataFrame(list(allocation.items()), columns=['Asset Class', 'Weight'])
            df_allocation['Weight_Percent'] = df_allocation['Weight'] * 100
            
            fig = px.pie(df_allocation, values='Weight', names='Asset Class', 
        ...
        with col2:
            ...
            risk_profiles = {
                "conservative": {"volatility": "Low", "expected_return": "4-6%", "drawdown_risk": "Minimal"},
                "moderate": {"volatility": "Medium", "expected_return": "6-10%", "drawdown_risk": "Moderate"},
                "aggressive": {"volatility": "High", "expected_return": "10-15%", "drawdown_risk": "Significant"}
            }
        ...
        st.caption("⚠️ **Disclaimer**: Educational guidance only; consult a professional for personal investment advice.")
    ...
    with tab_affiliate:
        ...
        with col2:
            ...
            st.info("📊 Performance trends will be displayed here when historical data is available.")
            
            # Commission calculator
            st.subheader("💰 Commission Calculator")
            
            estimated_clicks = st.number_input("Estimated Monthly Clicks", min_value=0, value=1000, step=100)
            conversion_rate_input = st.number_input("Conversion Rate (%)", min_value=0.0, max_value=100.0, value=3.5, step=0.1)
            commission_per_conversion = st.number_input("Commission per Conversion ($)", min_value=0.0, value=50.0, step=5.0)
            ...
            if estimated_clicks > 0:
                ...
                st.metric("Estimated Monthly Commission", f"${estimated_commission:,.2f}")
            ...
            if st.button("🔍 Load Top Pools", type="primary"):
                ...
                if pools_data and 'pools' in pools_data:
                    ...
                    with col_b:
                        ...
                        st.metric("Total TVL", f"${total_tvl/1e6:.1f}M" if total_tvl > 1e6 else f"${total_tvl:,.0f}")
                    with col_c:
                    ...
                    for pool in pools[:5]:  # Show top 5
                        with st.container():
                            col_pool, col_tvl, col_apr, col_risk = st.columns([3, 2, 2, 2])
                            ...
                            with col_tvl:
                                ...
                                st.metric("TVL", f"${tvl/1e6:.1f}M" if tvl > 1e6 else f"${tvl:,.0f}")
                ...
                if pool_id and from_asset and to_asset and amount > 0:
                    ...
                    if result:
                        ...
                        if tx:
                            ...
                            st.info(f"Confirmation: {tx.get('estimated_confirmation', 'Unknown')}")
                        
                        if swap_details:
                            col_input, col_output = st.columns(2)
                            
                            with col_input:
                                st.metric("Input", f"{swap_details.get('input_amount', 0)} {swap_details.get('from_asset', '')}")
                            
                            with col_output:
                                estimated_output = swap_details.get('estimated_output', 0)
                                st.metric("Estimated Output", f"{estimated_output:.6f} {swap_details.get('to_asset', '')}")
                            
                            st.caption(f"Fee: {swap_details.get('fee_percent', 0)}% | Slippage: {swap_details.get('slippage_tolerance', 0)}%")
