...
3. When prompted, use: `codex_user` as username
4. Copy and paste the migration_scroll.sql content
...
2. **capsule_runs** - Complete execution tracking
...
2. **v_recent_executions** - Recent execution history (last 7 days)

### Functions:
1. **record_capsule_execution()** - Main function for your record_capsule_run() integration
2. **get_capsule_stats()** - Performance analytics
...
2. Update your record_capsule_run() function to use the PostgreSQL function
3. Enable smart archiver to use Cloud Storage
4. Test full end-to-end capsule execution with database tracking
