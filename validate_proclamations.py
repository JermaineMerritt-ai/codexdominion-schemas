    ...
    try:
        ...
        if not result["metadata_accurate"]:
            result["issues"].append(
                f"Metadata count ({metadata_count}) doesn't match actual count ({len(proclamations)})"
            )
        ...
        if result["pydantic_valid"]:
            result["issues"].append(
                f"All {valid_proclamations} proclamations are valid"
            )
        else:
            result["issues"].append(
                f"Only {valid_proclamations}/{len(proclamations)} proclamations are valid"
            )
...
def run_comprehensive_validation():
    ...
    for file_path in files_to_validate:
        ...
        print(
            f"   📈 Metadata Accurate: {'✅' if result['metadata_accurate'] else '❌'}"
        )
        ...
        if all(
            [
                ...
                result["metadata_accurate"],
                result["pydantic_valid"],
            ]
        ):
            valid_files += 1
