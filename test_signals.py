...
def test_signals_engine():
    ...
    market = [
        ...
        MarketSnapshot(symbol="XYZ", price=12.0, vol_30d=0.65, trend_20d=-0.10, liquidity_rank=250),
        MarketSnapshot(symbol="AAPL", price=195.3, vol_30d=0.28, trend_20d=0.15, liquidity_rank=3),
        MarketSnapshot(symbol="VOLATILE", price=50.0, vol_30d=0.80, trend_20d=0.60, liquidity_rank=400),
    ]
...
def test_mock_data_feed():
    ...
    for snapshot in snapshots:
        ...
        print(f"{snapshot.symbol}: ${snapshot.price:.2f}, vol={snapshot.vol_30d:.2f}, "
...
def test_tier_classification():
    ...
    test_cases = [
        ...
        MarketSnapshot(symbol="ALPHA_STOCK", price=100.0, vol_30d=0.20, trend_20d=0.50, liquidity_rank=10),
        
        # Beta tier: neutral conditions
        MarketSnapshot(symbol="BETA_STOCK", price=100.0, vol_30d=0.30, trend_20d=0.10, liquidity_rank=25),
        
        # Gamma tier: elevated volatility
        MarketSnapshot(symbol="GAMMA_STOCK", price=100.0, vol_30d=0.45, trend_20d=0.20, liquidity_rank=30),
        
        # Delta tier: high volatility
        MarketSnapshot(symbol="DELTA_STOCK", price=100.0, vol_30d=0.70, trend_20d=0.15, liquidity_rank=50),
    ]
    ...
    for snapshot in test_cases:
        ...
        print(f"\n{snapshot.symbol}:")
        print(f"  Tier: {tier}")
        print(f"  Vol: {snapshot.vol_30d:.2f}, Trend: {snapshot.trend_20d:.2f}, Liquidity: {snapshot.liquidity_rank}")
...
def test_risk_management():
    ...
    # High risk scenario
    high_risk_market = [
        MarketSnapshot(symbol="RISKY1", price=100.0, vol_30d=0.80, trend_20d=-0.30, liquidity_rank=200),
        MarketSnapshot(symbol="RISKY2", price=50.0, vol_30d=0.90, trend_20d=-0.50, liquidity_rank=300),
        MarketSnapshot(symbol="STABLE", price=200.0, vol_30d=0.15, trend_20d=0.40, liquidity_rank=5),
    ]
    ...
    if delta_count > 0:
        print(f"✅ Risk management working: {delta_count} symbols classified as Delta (high risk)")
