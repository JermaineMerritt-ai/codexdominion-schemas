...
var NavLink = function (_a) {
    var href = _a.href, icon = _a.icon, text = _a.text, gradient = _a.gradient, _b = _a.isHighlighted, isHighlighted = _b === void 0 ? false : _b;
    return (<link_1.default href={href} className="group">
    <div className={"\n      flex items-center px-4 py-2 rounded-lg transition-all duration-300 transform hover:scale-105\n      ".concat(isHighlighted
            ? "bg-gradient-to-r ".concat(gradient, " border border-yellow-400/50 shadow-lg")
            : 'bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40', "\n    ")}>
      <span className="text-lg mr-2">{icon}</span>
      <span className={"text-sm font-medium ".concat(isHighlighted ? 'text-yellow-100' : 'text-gray-300 group-hover:text-white')}>
        {text}
      </span>
    </div>
  </link_1.default>);
};
var CodexNavigation = function (_a) {
    ...
    return (<nav className="bg-gradient-to-r from-purple-900/50 via-blue-900/50 to-indigo-900/50 backdrop-blur-md border-b border-white/20">
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          {/* Logo */}
          <link_1.default href="/dashboard-selector" className="flex items-center group">
          ...
          <div className="hidden lg:flex items-center space-x-2">
            {navigationLinks.map(function (link, index) { return (<NavLink key={index} href={link.href} icon={link.icon} text={link.text} gradient={link.gradient} isHighlighted={link.isHighlighted}/>); })}
          </div>

          {/* Mobile Menu Button */}
          <div className="lg:hidden">
            <button className="p-2 rounded-lg bg-white/10 border border-white/20 text-white hover:bg-white/20 transition-colors" aria-label="Open mobile navigation menu" title="Open mobile navigation menu">
...
exports.default = CodexNavigation;
