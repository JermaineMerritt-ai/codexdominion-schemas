              ...
              <div className="bg-black bg-opacity-20 rounded-xl border border-purple-800 p-6">
                ...
                <div className="grid gap-4">
                  {artifacts.map((artifact) => (
                    <div key={artifact.id} className="bg-gray-800 bg-opacity-50 rounded-lg p-4 border border-gray-700 hover:border-purple-500 transition-colors">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          <div className="text-2xl">
                            {artifact.type === 'json' ? '📄' : artifact.type === 'markdown' ? '📝' : '⚙️'}
                          </div>
                          <div>
                            <h3 className="font-medium text-white">{artifact.name}</h3>
                            <p className="text-sm text-gray-400">
                              {artifact.size} • Modified {new Date(artifact.lastModified).toLocaleString()}
                            </p>
                    ...
                    <div key={index} className="bg-gray-800 bg-opacity-50 rounded-lg p-6 border border-gray-700">
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-white">{new Date(crown.lastAudit).toLocaleString()}</span>
                        </div>
                        <div>
                          <p className="text-gray-400 mb-2">Dependencies:</p>
                          <div className="flex flex-wrap gap-1">
                            {crown.dependencies.map((dep, i) => (
                              <span key={i} className="px-2 py-1 bg-purple-800 text-purple-200 rounded text-xs">
                                {dep}
                              </span>
                            ))}
                          </div>
                      ...
                      <button className="p-4 bg-gray-800 border border-gray-600 rounded-lg text-left hover:border-purple-500 transition-colors">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-400">Detect and process system events</p>
                          </div>
                        </div>
                      </button>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-800 bg-opacity-30 rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      {[
                        { time: '14:30', event: 'Festival ceremony inscribed', type: 'ceremony', user: 'system' },
                        { time: '13:15', event: 'Infrastructure crown updated', type: 'maintenance', user: 'custodian_01' },
                        { time: '12:00', event: 'Artifact backup completed', type: 'backup', user: 'automated' },
                        { time: '11:45', event: 'New heir registered', type: 'registration', user: 'heir_dashboard' }
                      ].map((log, i) => (
                        <div key={i} className="flex items-center justify-between p-3 bg-gray-700 bg-opacity-50 rounded">
                          <div className="flex items-center">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className={`w-2 h-2 rounded-full ${
                              log.type === 'ceremony' ? 'bg-purple-500' :
                              log.type === 'maintenance' ? 'bg-orange-500' :
                              log.type === 'backup' ? 'bg-green-500' : 'bg-blue-500'
                            }`}></span>
                          </div>
                        </div>
                      ))}
