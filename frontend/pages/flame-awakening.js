...
exports.default = FlameAwakeningPage;
var react_1 = require("react");
var link_1 = require("next/link");
var CodexNavigation_1 = require("../components/CodexNavigation");
// Flame Awakening Component
var FlameAwakening = function () {
...
var CosmicFlow = function () {
    var particles = Array.from({ length: 20 }, function (_, i) {
        var left = Math.random() * 100;
        var top = Math.random() * 100;
        var delay = Math.random() * 5;
        var duration = 3 + Math.random() * 4;
        return (<div key={i} className={"absolute w-1 h-1 bg-white rounded-full opacity-60 cosmic-particle-".concat(i)}/>);
    });
    var particleStyles = Array.from({ length: 20 }, function (_, i) {
        var left = Math.random() * 100;
        var top = Math.random() * 100;
        var delay = Math.random() * 5;
        var duration = 3 + Math.random() * 4;
        return "\n      .cosmic-particle-".concat(i, " {\n        left: ").concat(left, "%;\n        top: ").concat(top, "%;\n        animation-delay: ").concat(delay, "s;\n        animation-duration: ").concat(duration, "s;\n      }\n    ");
    }).join('');
    return (<div className="relative w-full h-64 overflow-hidden rounded-2xl border border-gold-400/30">
...
var SacredVerse = function (_a) {
    var line = _a.line, icon = _a.icon, delay = _a.delay;
    return (<div className={"bg-gradient-to-r from-purple-800/20 via-blue-800/20 to-indigo-800/20 backdrop-blur-sm rounded-xl p-6 border border-gold-400/30 shadow-lg animate-fade-in sacred-verse-delay"}>
      <div className="flex items-center gap-4">
        <div className="text-4xl animate-pulse">{icon}</div>
        <p className="text-xl text-gray-200 font-medium italic">{line}</p>
      </div>
      <style jsx>{"\n        .sacred-verse-delay {\n          animation-delay: ".concat(delay, ";\n        }\n      ")}</style>
    </div>);
};
function FlameAwakeningPage() {
    var sacredVerses = [
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-indigo-900 text-white">
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="space-y-8 mb-16 max-w-4xl mx-auto">
          {sacredVerses.map(function (verse, index) { return (<SacredVerse key={index} line={verse.line} icon={verse.icon} delay={verse.delay}/>); })}
        </div>
