...
const CosmicParticle: React.FC<ParticleProps> = ({ delay, duration, x, y }) => (
  <div
    className={`absolute w-1 h-1 bg-gradient-to-r from-gold-400 to-amber-300 rounded-full opacity-70 cosmic-particle-${delay}-${duration}-${x}-${y}`}
  />
);
...
const EternalFlame: React.FC = () => {
  ...
  return (
    <div className="relative mx-auto mb-8 w-32 h-32">
      {/* Outer Ring - Cosmos */}
      <div
        className={`absolute inset-0 rounded-full border-4 border-purple-500/30 eternal-flame-spin ${isVisible ? 'animate-spin' : ''}`}
      >
      ...
      <div
        className={`absolute inset-4 rounded-full border-3 border-amber-500/50 eternal-flame-spin-reverse ${isVisible ? 'animate-spin' : ''}`}
      >
...
const VerseLine: React.FC<VerseLineProps> = ({ text, delay, icon }) => {
  ...
  return (
    ...
    >
      <span className={`text-3xl mr-4 animate-pulse verse-delay-${delay}`}>{icon}</span>
...
const SovereignSeal: React.FC = () => {
  ...
  return (
    <div className="relative mx-auto w-24 h-24 mb-8">
      <div
        className={`absolute inset-0 border-4 border-gold-400 rounded-full bg-gradient-to-br from-gold-500/20 to-amber-600/20 sovereign-seal-rotate`}
      >
...
const CovenantEternal: React.FC = () => {
  ...
  const particleStyles = particles
    .map(
      (p) => `
    .cosmic-particle-${p.delay}-${p.duration}-${p.x}-${p.y} {
      left: ${p.x}%;
      top: ${p.y}%;
      animation: pulse ${p.duration}s infinite ${p.delay}s ease-in-out, float ${p.duration * 2}s infinite ${p.delay}s ease-in-out alternate;
    }
  `
    )
    .join('');
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="The eternal covenant binding flame to silence, hymn to transmission"
        />
