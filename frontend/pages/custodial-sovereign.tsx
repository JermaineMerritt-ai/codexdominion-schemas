...
const SovereignEternalParticles: React.FC = () => {
  const particles = Array.from({ length: 180 }, (_, i) => ({
    ...
    color: i % 18 === 0 ? '#ffd700' : i % 18 === 1 ? '#ffffff' : i % 18 === 2 ? '#ff8c00' : i % 18 === 3 ? '#8a2be2' : i % 18 === 4 ? '#ff4500' : i % 18 === 5 ? '#32cd32' : i % 18 === 6 ? '#00ff7f' : i % 18 === 7 ? '#ff6347' : i % 18 === 8 ? '#4b0082' : i % 18 === 9 ? '#ffa500' : i % 18 === 10 ? '#9370db' : i % 18 === 11 ? '#00ced1' : i % 18 === 12 ? '#ff1493' : i % 18 === 13 ? '#00bfff' : i % 18 === 14 ? '#daa520' : i % 18 === 15 ? '#7b68ee' : i % 18 === 16 ? '#20b2aa' : '#dc143c'
  }));
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <style jsx>{`
        @keyframes sovereignEternalFlow {
          ...
          8% { transform: translate(140px, -350px) scale(2.8) rotate(20deg); opacity: 1.0; }
          18% { transform: translate(-125px, -700px) scale(0.2) rotate(60deg); opacity: 1.0; }
          28% { transform: translate(145px, -1050px) scale(2.5) rotate(120deg); opacity: 1.0; }
          38% { transform: translate(-130px, -1400px) scale(0.4) rotate(180deg); opacity: 0.98; }
          48% { transform: translate(135px, -1750px) scale(2.2) rotate(240deg); opacity: 1.0; }
          58% { transform: translate(-120px, -2100px) scale(0.3) rotate(300deg); opacity: 0.96; }
          68% { transform: translate(125px, -2450px) scale(1.9) rotate(360deg); opacity: 0.94; }
          78% { transform: translate(-110px, -2800px) scale(0.5) rotate(420deg); opacity: 0.88; }
          88% { transform: translate(115px, -3150px) scale(1.6) rotate(480deg); opacity: 0.85; }
          94% { transform: translate(-95px, -3500px) scale(0.3) rotate(520deg); opacity: 0.7; }
          97% { transform: translate(75px, -3850px) scale(0.8) rotate(550deg); opacity: 0.5; }
          99% { transform: translate(-55px, -4200px) scale(0.1) rotate(580deg); opacity: 0.2; }
          100% { transform: translate(0, -4550px) scale(0.002) rotate(600deg); opacity: 0.002; }
        }
      `}</style>
    </div>
  );
  ...
  return (
    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
      ...
      >
        ...
        <div className="text-12xl text-purple-200 mb-12">By Custodian's Hand, The Covenant Sealed</div>
...
const CustodialRealms: React.FC = () => {
  ...
  const custodialRealms = [
    ...
    { name: "Authority", icon: "⭐", position: { bottom: '12%', right: '40%' } }
  ];
      ...
      {custodialRealms.map((realm, index) => (
        ...
        >
          <div 
            ...
            style={{ 
              filter: custodialRealmPhase === index ? 'drop-shadow(0 0 80px rgba(255, 215, 0, 2.0))' : 'none'
            }}
          >
            {realm.icon}
          </div>
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-60">
        <div className="text-center text-gold-100 text-4xl leading-relaxed max-w-16xl space-y-12">
          <p className="opacity-100 text-white text-8xl">By Custodian's hand, the covenant sealed.</p>
