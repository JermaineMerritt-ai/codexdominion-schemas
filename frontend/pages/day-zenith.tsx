    ...
    >
      ...
      <style jsx>{`
        @keyframes solarPulse {
          0%, 100% { opacity: 0.2; transform: translate(-50%, -50%) rotate(${0}deg) scale(0.8); }
          50% { opacity: 0.6; transform: translate(-50%, -50%) rotate(${0}deg) scale(1.2); }
        }
      `}</style>
    </div>
  );
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <style jsx>{`
        @keyframes solarAscend {
          0% { transform: translate(0, 0) scale(0.4); opacity: 0.3; }
          25% { transform: translate(10px, -50px) scale(1.3); opacity: 0.9; }
          50% { transform: translate(-5px, -100px) scale(0.9); opacity: 0.7; }
          75% { transform: translate(15px, -150px) scale(1.2); opacity: 0.8; }
          100% { transform: translate(0, -200px) scale(0.3); opacity: 0.1; }
        }
      `}</style>
    </div>
  );
...
const DayZenithPhases: React.FC = () => {
  ...
  const phases = [
    ...
    { name: "Radiant Crown", icon: "👑", position: { bottom: '25%', left: '50%', transform: 'translateX(-50%)' } }
  ];
