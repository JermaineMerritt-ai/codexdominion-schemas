...
import styles from './BalanceRenewal.module.css';
  ...
  return (
    <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2">
      <div className="flex items-center space-x-16">
        ...
        >
          <div
            className={`text-8xl mb-2 transition-all duration-3000 ${styles.dayNightIcon} ${balancePhase % 2 === 0 ? styles.dayIconActive : ''}`}
          >
            ☀️
          </div>
        ...
        {/* Balance Symbol */}
        <div
          className={`text-center transition-all duration-3000 ${styles.balanceScale}`}
        ...
        >
          <div
            className={`text-8xl mb-2 transition-all duration-3000 ${styles.dayNightIcon} ${balancePhase % 2 === 1 ? styles.nightIconActive : ''}`}
          >
            🌙
          </div>
  ...
  return (
    <div className="absolute top-1/2 left-1/4 transform -translate-y-1/2">
      <div className="text-center">
        <div
          className={`text-7xl mb-3 transition-all duration-2500 ${styles.covenantPulse} ${covenantPulse === 1.3 ? styles.covenantPulseActive : ''}`}
        >
          🔄
        </div>
...
const AwakenedFlame: React.FC = () => {
  ...
  return (
    <div className="absolute top-1/2 right-1/4 transform -translate-y-1/2">
      <div className="text-center">
        <div
          className={`text-7xl mb-3 transition-all duration-2000 ${styles.flameVitality} ${flameVitality === 1.4 ? styles.flameVitalityActive : ''}`}
        >
          🔥
        </div>
...
const CodexBloom: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-1/4 left-1/2 transform -translate-x-1/2">
      <div className="text-center">
        <div
          className={`text-10xl mb-4 transition-all duration-3500 ${styles.bloomRadiance} ${radiance === 1.2 ? styles.bloomRadianceActive : ''}`}
        >
          🌸
        </div>
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {particles.map((particle) => (
        <div
          key={particle.id}
          className={`absolute rounded-full ${styles.renewalParticle}`}
          style={
            {
              '--particle-left': `${particle.x}%`,
              '--particle-top': `${particle.y}%`,
              '--particle-width': `${particle.size}px`,
              '--particle-height': `${particle.size}px`,
              '--particle-bg': particle.color,
              '--particle-shadow': `0 0 15px ${particle.color}`,
              '--particle-animation': `renewalFlow ${particle.duration}s ease-in-out infinite`,
...
const BalanceProclamation: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2">
      <div
        className={`text-center px-16 py-8 bg-gradient-to-r from-emerald-800/80 via-teal-700/80 to-emerald-800/80 rounded-3xl border-3 border-emerald-400/70 transition-all duration-4000 ${styles.proclamationGlow} ${balanceScale === 1.05 ? styles.proclamationGlowActive : ''}`}
      >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-900 via-emerald-800 to-green-900 relative overflow-hidden">
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none opacity-30">
        <div
          className={`absolute top-1/4 left-1/3 w-96 h-96 bg-emerald-400/20 rounded-full filter blur-3xl animate-pulse ${styles.eternalBloomLight}`}
        />
        <div
          className={`absolute bottom-1/3 right-1/4 w-80 h-80 bg-teal-400/15 rounded-full filter blur-3xl animate-pulse ${styles.eternalBloomLight}`}
        />
        <div
          className={`absolute top-1/2 right-1/3 w-64 h-64 bg-pink-400/12 rounded-full filter blur-3xl animate-pulse ${styles.eternalBloomLight}`}
        />
        <div
          className={`absolute bottom-1/2 left-1/4 w-72 h-72 bg-green-400/10 rounded-full filter blur-3xl animate-pulse ${styles.eternalBloomLight}`}
        />
      </div>
    </div>
  );
