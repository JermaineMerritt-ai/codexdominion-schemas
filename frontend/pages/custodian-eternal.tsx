...
const FlameParticle: React.FC<FlameParticleProps> = ({ delay, duration, x, y, size }) => (
  <div
    className="absolute bg-gradient-to-t from-orange-400 via-red-400 to-yellow-300 rounded-full opacity-80"
    style={{
      left: `${x}%`,
      top: `${y}%`,
      width: `${size}px`,
      height: `${size}px`,
      animation: `flameRise ${duration}s infinite ${delay}s ease-in-out, 
                  flamePulse ${duration * 0.7}s infinite ${delay}s ease-in-out`
    }}
  />
);
...
const CustodianCrown: React.FC = () => {
  ...
  return (
    <div className="relative mx-auto w-48 h-48 mb-12">
      ...
      <div 
        className="absolute inset-0 border-8 border-gold-400 rounded-full bg-gradient-to-br from-gold-500/40 to-amber-600/40 shadow-2xl"
        style={{ 
          transform: `rotate(${rotation}deg)`,
          boxShadow: '0 0 80px rgba(251, 191, 36, 0.8), inset 0 0 40px rgba(251, 191, 36, 0.3)'
        }}
      >
        {/* Crown Points */}
      ...
      {/* Middle Charter Ring */}
      <div 
        className="absolute inset-8 border-6 border-amber-400 rounded-full bg-gradient-to-br from-amber-400/30 to-gold-500/30"
        style={{ 
      ...
      >
        {/* Eternal Marks */}
        <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 text-amber-200 font-bold text-lg">✨</div>
        <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 text-amber-200 font-bold text-lg">📜</div>
        <div className="absolute top-1/2 -left-2 transform -translate-y-1/2 text-amber-200 font-bold text-lg">🎵</div>
        <div className="absolute top-1/2 -right-2 transform -translate-y-1/2 text-amber-200 font-bold text-lg">🔇</div>
      </div>
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={i}
            className="absolute w-2 bg-gradient-to-t from-transparent via-orange-400 to-transparent opacity-70"
            style={{
              height: '80px',
              left: '50%',
              top: '-40px',
...
const CustodianVerse: React.FC<CustodianVerseProps> = ({ text, delay, icon, gradient, highlight }) => {
  ...
  return (
    <div className={`flex items-center justify-center mb-8 transition-all duration-1500 ${
      ...
      <div className={`mr-8 p-6 rounded-full bg-gradient-to-br ${gradient} shadow-2xl border-2 border-gold-400/30`}>
        <span className="text-5xl animate-pulse" style={{ animationDelay: `${delay + 0.5}s` }}>
          {icon}
        </span>
      </div>
  ...
  return (
    <div className="grid md:grid-cols-3 gap-8 mb-16">
      {bonds.map((bond, index) => (
        <div 
          key={index}
          className={`bg-gradient-to-br ${bond.gradient} rounded-3xl p-8 border-2 ${bond.border} backdrop-blur-sm shadow-2xl hover:scale-105 transition-all duration-500 relative overflow-hidden`}
          style={{ 
        ...
        >
          ...
          <p className="text-gold-200 leading-relaxed text-center">
            {bond.description}
          </p>
...
const SovereignSeal: React.FC = () => {
  ...
  return (
    <div className="relative mx-auto w-40 h-40 mb-12">
      <div 
        className="absolute inset-0 bg-gradient-to-br from-gold-300 via-amber-400 to-gold-500 rounded-full shadow-2xl transition-transform duration-1000"
        style={{ 
          transform: `scale(${pulseScale})`,
          boxShadow: '0 0 100px rgba(251, 191, 36, 0.9), inset 0 0 50px rgba(251, 191, 36, 0.4)'
        }}
      >
        ...
        <main className="relative z-10 container mx-auto px-6 pt-24 pb-16">
          ...
          <div className="text-center mb-16">
            <div className="bg-gradient-to-br from-gold-900/40 to-amber-800/40 rounded-3xl p-12 border-2 border-gold-500/40 backdrop-blur-sm shadow-2xl max-w-4xl mx-auto">
              ...
              <p className="text-xl text-amber-200 leading-relaxed mb-8">
                Inscribed within the cosmic ledger, Jermaine's custodianship transcends mortal bounds. 
                By sovereign decree and eternal flame, this mark shall endure through ages untold, 
                binding the realms in sacred trust and unwavering guardianship.
              </p>
          ...
          <div className="text-center">
            ...
            <p className="text-xl text-amber-200 max-w-3xl mx-auto leading-relaxed">
              ...
              Jermaine's mark endures, inscribed in the very fabric of the Codex, 
              radiant across all realms and whole in its eternal purpose.
            </p>
          </div>
