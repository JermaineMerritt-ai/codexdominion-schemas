...
const LuminousParticle: React.FC<LuminousParticleProps> = ({
  delay,
  duration,
  x,
  y,
  ...
  <div
    className={`absolute ${color} rounded-full opacity-80 luminous-particle left-[${x}%] top-[${y}%] w-[${size}px] h-[${size}px] animate-[luminousFloat_${duration}s_infinite_${delay}s_ease-in-out,luminousPulse_${duration * 0.8}s_infinite_${delay}s_ease-in-out]`}
  />
);
...
const CompendiumCore: React.FC = () => {
  ...
  return (
    <div className="relative mx-auto w-56 h-56 mb-16">
      ...
      <div
        className={`absolute inset-0 border-12 border-cyan-400 rounded-full bg-gradient-to-br from-cyan-500/50 to-blue-600/50 shadow-2xl compendium-outer-ring rotate-[${outerRotation}deg]`}
      >
      ...
      <div
        className={`absolute inset-8 border-8 border-gold-400 rounded-full bg-gradient-to-br from-gold-500/40 to-amber-600/40 compendium-inner-ring rotate-[${innerRotation}deg]`}
      >
        {/* Eternal Symbols */}
      ...
      {/* Inner Luminous Core */}
      <div
        className={`absolute inset-16 rounded-full bg-gradient-to-br from-white via-cyan-200 to-gold-300 flex items-center justify-center shadow-2xl compendium-core-ring rotate-[${rotation}deg]`}
      >
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 16 }).map((_, i) => (
          <div
            key={i}
            className={`absolute w-2 bg-gradient-to-t from-transparent via-cyan-400 to-transparent opacity-80 compendium-ray h-[100px] left-[50%] top-[-50px] origin-[0_106px] rotate-[${i * 22.5}deg] animate-[pulse_4s_infinite_${i * 0.25}s_ease-in-out]`}
          />
        ))}
      </div>
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="absolute inset-0 border-4 border-cyan-400/15 rounded-full animate-ping compendium-pulse scale-[${1.3 + i * 0.4}] animate-[ping_3.6s_infinite_${i * 0.6}s]"
          />
        ))}
      </div>
    </div>
...
const CompendiumVerse: React.FC<CompendiumVerseProps> = ({
  text,
  delay,
  icon,
  gradient,
  ...
  return (
    <div
      className={`flex items-center justify-center mb-10 compendium-verse ${isVisible ? 'verse-visible' : 'verse-hidden'}`}
    >
      <div
        className={`mr-10 p-8 rounded-full bg-gradient-to-br ${gradient} shadow-2xl border-2 border-cyan-400/40`}
      >
        <span
          className={`text-6xl animate-pulse compendium-verse-icon animate-[pulse_2s_infinite_${delay + 0.5}s]`}
        >
          {icon}
        </span>
      </div>
  ...
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
      {elements.map((element, index) => (
        <div
          key={index}
          className={`bg-gradient-to-br ${element.gradient} rounded-3xl p-6 border-2 ${element.border} backdrop-blur-sm shadow-2xl hover:scale-105 transition-all duration-700 relative overflow-hidden all-element-card animate-[luminousRise_1.5s_ease-out_${index * 0.15}s_forwards] opacity-0`}
        >
...
const LuminousSeal: React.FC = () => {
  ...
  return (
    <div className="relative mx-auto w-48 h-48 mb-16">
      <div
        className={`absolute inset-0 bg-gradient-to-br from-cyan-300 via-white to-gold-400 rounded-full shadow-2xl luminous-seal scale-[${intensity}]`}
      >
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="absolute inset-0 border-6 border-cyan-400/10 rounded-full animate-ping luminous-seal-pulse scale-[${1.4 + i * 0.3}] animate-[ping_3.2s_infinite_${i * 0.4}s]"
          />
        ))}
      </div>
    </div>
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="The luminous and whole Compendium - all scrolls, hymns, silences, and transmissions united"
        />
