...
export default async function handler(
  ...
  try {
    ...
    if (kind && typeof kind === 'string' && ['proclamation', 'silence', 'blessing'].includes(kind)) {
    ...
    const ceremonialStats = cycles.reduce((acc, cycle) => {
    ...
    const response = {
      ...
      metadata: {
        ...
        flame_status: cycles.length > 0 
          ? cycles[cycles.length - 1].flame_status 
          : 'awaiting_first_ceremony'
      }
    };
