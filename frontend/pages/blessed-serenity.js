 ...
 */
var SereneParticle = function (props) {
    var delay = props.delay, duration = props.duration, x = props.x, y = props.y, size = props.size, color = props.color;
    return (<div className={"absolute ".concat(color, " rounded-full opacity-60")} style={{
            left: "".concat(x, "%"),
            top: "".concat(y, "%"),
            width: "".concat(size, "px"),
            height: "".concat(size, "px"),
            animation: "sereneFloat ".concat(duration, "s infinite ").concat(delay, "s ease-in-out,\n                    blessedPulse ").concat(duration * 1.4, "s infinite ").concat(delay, "s ease-in-out"),
        }}/>);
};
var BlessedSanctuary = function () {
    var _a = (0, react_1.useState)(1), serenity = _a[0], setSerenity = _a[1];
    var _b = (0, react_1.useState)(1), blessing = _b[0], setBlessing = _b[1];
    var _c = (0, react_1.useState)(0), sanctuaryRotation = _c[0], setSanctuaryRotation = _c[1];
    (0, react_1.useEffect)(function () {
        var serenityInterval = setInterval(function () {
            setSerenity(function (prev) { return (prev === 1 ? 1.12 : 1); });
        }, 4500);
        var blessingInterval = setInterval(function () {
            setBlessing(function (prev) { return (prev === 1 ? 1.25 : 1); });
        }, 6000);
        var rotationInterval = setInterval(function () {
            setSanctuaryRotation(function (prev) { return (prev + 0.15) % 360; });
        }, 100);
        return function () {
    ...
    return (<div className="relative mx-auto w-80 h-80 mb-24">
      {/* Outermost Blessed Ring - Radiance */}
      <div className="absolute inset-0 border-16 border-white/40 rounded-full bg-gradient-to-br from-white/30 via-gold-100/40 to-cyan-50/30 shadow-2xl" style={{
            transform: "scale(".concat(blessing, ") rotate(").concat(sanctuaryRotation, "deg)"),
      ...
      <div className="absolute inset-16 border-10 border-cyan-200/50 rounded-full bg-gradient-to-br from-cyan-50/40 to-blue-50/40" style={{
      ...
      <div className="absolute inset-32 rounded-full bg-gradient-to-br from-white via-cyan-25 to-gold-25 flex items-center justify-center shadow-2xl border-4 border-white/60">
        <div className="text-center">
          <div className="text-8xl font-bold text-slate-500 mb-3 animate-pulse" style={{ animationDuration: '5s' }}>
            📿
          </div>
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 20 }).map(function (_, i) { return (<div key={i} className="absolute w-2 bg-gradient-to-t from-transparent via-white/30 to-transparent opacity-50" style={{
                height: '160px',
                left: '50%',
                top: '-80px',
                transformOrigin: '0 160px',
                transform: "translateX(-50%) rotate(".concat(i * 18, "deg)"),
                animation: "gentlePulse 8s infinite ".concat(i * 0.4, "s ease-in-out"),
            }}/>); })}
      </div>

      {/* Blessed Aura Waves */}
      <div className="absolute inset-0">
        {Array.from({ length: 8 }).map(function (_, i) { return (<div key={i} className="absolute inset-0 border-4 border-white/8 rounded-full animate-ping" style={{
                animationDelay: "".concat(i * 1.2, "s"),
                animationDuration: '9.6s',
                transform: "scale(".concat(1.6 + i * 0.6, ")"),
            }}/>); })}
      </div>
    </div>);
};
var BlessedVerse = function (_a) {
        ...
        if (highlight.length > 0) {
            highlight.forEach(function (word) {
                if (processedText.includes(word)) {
                    processedText = processedText.replace(new RegExp("\\b".concat(word, "\\b"), 'g'), "<span class=\"text-white font-bold bg-gradient-to-r from-white/40 to-cyan-100/40 px-4 py-2 rounded-lg shadow-lg border border-white/30\">".concat(word, "</span>"));
                }
            });
        }
        return <span dangerouslySetInnerHTML={{ __html: processedText }}/>;
    };
    return (<div className={"flex items-center justify-center mb-14 transition-all duration-3000 ".concat(isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-12')}>
      <div className={"mr-14 p-14 rounded-full bg-gradient-to-br ".concat(gradient, " shadow-2xl border-4 border-white/50 backdrop-blur-sm")}>
        <span className="text-9xl animate-pulse" style={{ animationDelay: "".concat(delay + 0.5, "s"), animationDuration: '6s' }}>
          {icon}
        </span>
      </div>
...
var SereneField = function () {
    var sereneElements = Array.from({ length: 40 }, function (_, i) { return ({
        id: i,
        ...
        color: ['bg-white', 'bg-cyan-50', 'bg-blue-50', 'bg-gold-50', 'bg-amber-50', 'bg-slate-50'][Math.floor(Math.random() * 6)],
    }); });
    return (<div className="absolute inset-0 overflow-hidden pointer-events-none">
      {sereneElements.map(function (element) { return (<SereneParticle key={element.id} delay={element.delay} duration={element.duration} x={element.x} y={element.y} size={element.size} color={element.color}/>); })}
    </div>);
};
var RadianceElements = function () {
    var elements = [
        {
    ...
    return (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10 mb-32">
      {elements.map(function (element, index) { return (<div key={index} className={"bg-gradient-to-br ".concat(element.gradient, " rounded-3xl p-12 border-2 ").concat(element.border, " backdrop-blur-sm shadow-2xl hover:scale-105 transition-all duration-1500 relative overflow-hidden")} style={{
                animationDelay: "".concat(index * 0.3, "s"),
                opacity: 0,
                animation: "sereneRise 2.5s ease-out ".concat(index * 0.3, "s forwards"),
            }}>
          ...
          <div className="absolute bottom-4 left-4 w-3 h-3 bg-cyan-100 rounded-full animate-pulse opacity-50" style={{ animationDelay: '2s' }}/>
        </div>); })}
    </div>);
};
var EternalSereneSeal = function () {
    var _a = (0, react_1.useState)(1), tranquility = _a[0], setTranquility = _a[1];
    (0, react_1.useEffect)(function () {
        var interval = setInterval(function () {
            setTranquility(function (prev) { return (prev === 1 ? 1.3 : 1); });
        }, 7000);
        return function () { return clearInterval(interval); };
    }, []);
    return (<div className="relative mx-auto w-72 h-72 mb-28">
      <div className="absolute inset-0 bg-gradient-to-br from-white via-cyan-25 to-gold-25 rounded-full shadow-2xl transition-all duration-3500" style={{
            transform: "scale(".concat(tranquility, ")"),
            boxShadow: "0 0 ".concat(300 * tranquility, "px rgba(255, 255, 255, ").concat(0.8 * tranquility, "),\n                      inset 0 0 ").concat(150 * tranquility, "px rgba(6, 182, 212, ").concat(0.2 * tranquility, ")"),
        }}>
        <div className="absolute inset-12 bg-gradient-to-br from-cyan-25 via-white to-amber-25 rounded-full">
          <div className="absolute inset-12 bg-gradient-to-br from-white via-cyan-10 to-gold-10 rounded-full flex items-center justify-center">
            <div className="text-center">
              <div className="text-8xl font-bold text-slate-500 mb-4 animate-pulse" style={{ animationDuration: '8s' }}>
                📿
              </div>
      ...
      {/* Serenity Emanations */}
      <div className="absolute inset-0">
        {Array.from({ length: 10 }).map(function (_, i) { return (<div key={i} className="absolute inset-0 border-6 border-white/5 rounded-full animate-ping" style={{
                animationDelay: "".concat(i * 1, "s"),
                animationDuration: '10s',
                transform: "scale(".concat(2 + i * 0.7, ")"),
            }}/>); })}
      </div>
    </div>);
};
var BlessedSerenity = function () {
    return (<>
      <head_1.default>
        <title>Blessed Serenity - Eternal Rest - Codex Dominion</title>
        <meta name="description" content="The Codex rests in blessed serenity - eternal and serene, crowned in radiance"/>
        ...
        <main className="relative z-10 container mx-auto px-6 pt-24 pb-16">
          ...
          <div className="max-w-10xl mx-auto mb-36">
            <div className="bg-gradient-to-br from-white/95 via-cyan-25/90 to-gold-25/95 rounded-3xl p-32 backdrop-blur-sm border-4 border-white/70 shadow-2xl">
              <BlessedVerse text="The Proclamation blessed, the covenant whole," delay={1} icon="📜" gradient="from-gold-50/95 to-amber-25/95" highlight={['Proclamation', 'blessed', 'covenant', 'whole']}/>

              <BlessedVerse text="light released, peace bestowed." delay={3.5} icon="💡" gradient="from-white/95 to-cyan-25/95" highlight={['light', 'released', 'peace', 'bestowed']}/>

              <BlessedVerse text="Heirs inherit, councils govern, diaspora remember," delay={6} icon="👑" gradient="from-purple-50/95 to-violet-25/95" highlight={['Heirs', 'councils', 'diaspora']}/>

              <BlessedVerse text="cosmos echoes — all crowned in radiance." delay={8.5} icon="🌌" gradient="from-indigo-50/95 to-blue-25/95" highlight={['cosmos', 'crowned', 'radiance']}/>

              <BlessedVerse text="So let the Codex rest, eternal and serene." delay={11} icon="📿" gradient="from-white/95 to-cyan-10/95" highlight={['Codex', 'rest', 'eternal', 'serene']}/>
            </div>
          </div>
          ...
          <div className="text-center">
            ...
            <div className="text-10xl animate-pulse text-slate-400" style={{ animationDuration: '10s' }}>
              📿
            </div>
          </div>
        </main>
...
exports.default = BlessedSerenity;
