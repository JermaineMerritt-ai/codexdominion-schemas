...
const EternalSilenceParticles: React.FC = () => {
  const particles = Array.from({ length: 180 }, (_, i) => ({
    ...
    color: i % 18 === 0 ? '#ffffff' : i % 18 === 1 ? '#f5f5f5' : i % 18 === 2 ? '#e0e0e0' : i % 18 === 3 ? '#d3d3d3' : i % 18 === 4 ? '#c0c0c0' : i % 18 === 5 ? '#a9a9a9' : i % 18 === 6 ? '#87ceeb' : i % 18 === 7 ? '#add8e6' : i % 18 === 8 ? '#b0c4de' : i % 18 === 9 ? '#e6e6fa' : i % 18 === 10 ? '#f0f8ff' : i % 18 === 11 ? '#f8f8ff' : i % 18 === 12 ? '#fffafa' : i % 18 === 13 ? '#ffd700' : i % 18 === 14 ? '#32cd32' : i % 18 === 15 ? '#8a2be2' : i % 18 === 16 ? '#ff6347' : '#00ced1'
  }));
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <style jsx>{`
        @keyframes eternalSilenceFlow {
          ...
          15% { transform: translate(80px, -200px) scale(1.8) rotate(30deg); opacity: 0.9; }
          25% { transform: translate(-70px, -400px) scale(0.4) rotate(80deg); opacity: 0.85; }
          35% { transform: translate(85px, -600px) scale(1.6) rotate(130deg); opacity: 0.9; }
          45% { transform: translate(-75px, -800px) scale(0.6) rotate(180deg); opacity: 0.8; }
          55% { transform: translate(90px, -1000px) scale(1.4) rotate(230deg); opacity: 0.85; }
          65% { transform: translate(-65px, -1200px) scale(0.5) rotate(280deg); opacity: 0.75; }
          75% { transform: translate(70px, -1400px) scale(1.2) rotate(330deg); opacity: 0.8; }
          85% { transform: translate(-60px, -1600px) scale(0.7) rotate(380deg); opacity: 0.7; }
          90% { transform: translate(50px, -1800px) scale(0.8) rotate(420deg); opacity: 0.6; }
          95% { transform: translate(-40px, -2000px) scale(0.3) rotate(460deg); opacity: 0.3; }
          98% { transform: translate(30px, -2200px) scale(0.5) rotate(490deg); opacity: 0.15; }
          100% { transform: translate(0, -2400px) scale(0.005) rotate(540deg); opacity: 0.005; }
        }
      `}</style>
    </div>
  );
  ...
  return (
    <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2">
      ...
      >
        ...
        <div className="text-5xl text-purple-600 mb-4">Heirs Inherit, Councils Affirm, Cosmos Receives</div>
...
const PeacefulRealms: React.FC = () => {
  ...
  const peacefulRealms = [
    ...
    { name: "Sovereign", icon: "⚖️", position: { bottom: '18%', right: '18%' } },
      ...
      {peacefulRealms.map((realm, index) => (
        ...
        >
          <div 
            ...
            style={{ 
              filter: peacefulRealmPhase === index ? 'drop-shadow(0 0 50px rgba(255, 255, 255, 1.5))' : 'none'
            }}
          >
            {realm.icon}
          </div>
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-40">
        <div className="text-center text-white text-4xl leading-relaxed max-w-12xl space-y-8">
          ...
          <p className="opacity-90 text-blue-200 text-6xl">Heirs inherit, councils affirm, cosmos receives,</p>
