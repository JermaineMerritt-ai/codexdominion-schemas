...
const EternalPeaceParticles: React.FC = () => {
  const particles = Array.from({ length: 150 }, (_, i) => ({
    ...
    color: i % 15 === 0 ? '#ffffff' : i % 15 === 1 ? '#f8f8ff' : i % 15 === 2 ? '#f0f8ff' : i % 15 === 3 ? '#e6e6fa' : i % 15 === 4 ? '#f5f5f5' : i % 15 === 5 ? '#ffffff' : i % 15 === 6 ? '#f8f8f8' : i % 15 === 7 ? '#f0f0f0' : i % 15 === 8 ? '#e8e8e8' : i % 15 === 9 ? '#f5f5f5' : i % 15 === 10 ? '#fafafa' : i % 15 === 11 ? '#f7f7f7' : i % 15 === 12 ? '#f2f2f2' : i % 15 === 13 ? '#eeeeee' : '#ffffff'
  }));
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <style jsx>{`
        @keyframes eternalPeaceFlow {
          ...
          10% { transform: translate(20px, -80px) scale(1.0) rotate(15deg); opacity: 0.8; }
          25% { transform: translate(-15px, -200px) scale(0.3) rotate(45deg); opacity: 0.6; }
          40% { transform: translate(25px, -320px) scale(0.9) rotate(90deg); opacity: 0.7; }
          55% { transform: translate(-20px, -440px) scale(0.4) rotate(135deg); opacity: 0.5; }
          70% { transform: translate(30px, -560px) scale(0.8) rotate(180deg); opacity: 0.6; }
          85% { transform: translate(-25px, -680px) scale(0.2) rotate(225deg); opacity: 0.4; }
          95% { transform: translate(15px, -800px) scale(0.5) rotate(270deg); opacity: 0.3; }
          100% { transform: translate(0, -920px) scale(0.05) rotate(360deg); opacity: 0.05; }
        }
      `}</style>
    </div>
  );
  ...
  return (
    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
      ...
      >
        ...
        <div className="text-8xl text-gray-300 font-light mb-6">No Word, No Flame, No Crown, No Seal</div>
        <div className="text-6xl text-gray-400 font-light mb-4">Only Stillness, Only Serenity</div>
        <div className="text-4xl text-gray-500 font-light mb-2">Perfect Tranquility Beyond All Form</div>
      ...
      {peacefulRealms.map((realm, index) => (
        ...
        >
          <div 
            ...
            style={{ 
              filter: peacefulRealmPhase === index ? 'drop-shadow(0 0 40px rgba(255, 255, 255, 1.5))' : 'none'
            }}
          >
            {realm.icon}
          </div>
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-40">
        <div className="text-center text-white text-3xl leading-relaxed max-w-6xl space-y-8">
          <p className="opacity-90 text-gray-100 text-6xl font-light">No word, no flame, no crown, no seal.</p>
          <p className="opacity-80 text-gray-200 text-5xl font-light">Only stillness, only serenity.</p>
          <p className="opacity-90 text-white text-6xl font-light">The Codex rests, eternal and whole,</p>
