...
def get_comprehensive_metrics():
    ...
    try:
        ...
        aff_programs = int(
            aff_result[0][1] if aff_result and len(aff_result[0]) > 1 else 0
        )
        aff_conversion_rate = float(
            aff_result[0][2] if aff_result and len(aff_result[0]) > 2 else 0
        )
        ...
        total_tvl = float(
            pools_result[0][1] if pools_result and len(pools_result[0]) > 1 else 0
        )
        avg_apr = float(
            pools_result[0][2] if pools_result and len(pools_result[0]) > 2 else 0
        )
        ...
        unique_users = int(
            picks_result[0][1] if picks_result and len(picks_result[0]) > 1 else 0
        )
        ...
        portfolio_count = int(
            portfolio_result[0][0] if portfolio_result and portfolio_result[0] else 0
        )
        total_positions = int(
            portfolio_result[0][1]
            if portfolio_result and len(portfolio_result[0]) > 1
            else 0
        )
        total_portfolio_value = float(
            portfolio_result[0][2]
            if portfolio_result and len(portfolio_result[0]) > 2
            else 0
        )
        ...
        amm_events = int(
            events_result[0][0] if events_result and events_result[0] else 0
        )
        ...
        return {
            ...
            "trading": {"daily_picks_count": picks_count, "unique_users": unique_users},
            "portfolios": {
...
def calculate_performance_scores(metrics):
    ...
    try:
        ...
        trading_score = min(
            100,
            (metrics["trading"]["daily_picks_count"] * 2)
            + (metrics["trading"]["unique_users"] * 5),
        )

        # Portfolio growth score
        portfolio_score = min(
            100,
            (metrics["portfolios"]["total_portfolios"] * 10)
            + (metrics["portfolios"]["total_positions"] * 2),
        )

        # DeFi engagement score
        defi_score = min(
            100,
            (metrics["amm"]["pool_count"] * 3)
            + (metrics["amm"]["events_yesterday"] * 5),
        )

        # Revenue score
        revenue_score = min(
            100, metrics["affiliate"]["total_commission_usd"] / 100 * 10
        )  # $100 = 10 points

        # Overall composite score
        composite_score = round(
            (trading_score + portfolio_score + defi_score + revenue_score) / 4, 1
        )
...
def dawn_scroll():
    ...
    try:
        ...
        logger.info(
            f"📈 Composite Performance Score: {performance_scores['composite_score']}/100"
        )
        logger.info(f"💰 Revenue: ${metrics['affiliate']['total_commission_usd']:,.2f}")
        logger.info(
            f"🏦 Portfolio Value: ${metrics['portfolios']['total_value_usd']:,.2f}"
        )
...
def generate_weekly_report():
    ...
    try:
        ...
        total_revenue = sum(
            s.get("summary", {}).get("total_revenue_usd", 0) for s in recent_scrolls
        )
        avg_performance = sum(
            s.get("summary", {}).get("composite_performance", 0) for s in recent_scrolls
        ) / len(recent_scrolls)
        total_traders = sum(
            s.get("summary", {}).get("active_traders", 0) for s in recent_scrolls
        )
        ...
        weekly_report = {
            ...
            "trends": {
                "revenue_trend": (
                    ...
                    and recent_scrolls[0].get("summary", {}).get("total_revenue_usd", 0)
                    > recent_scrolls[1].get("summary", {}).get("total_revenue_usd", 0)
                    else "down"
                ),
                "performance_trend": (
                    "up"
                    if len(recent_scrolls) >= 2
                    and recent_scrolls[0]
                    .get("summary", {})
                    .get("composite_performance", 0)
                    > recent_scrolls[1]
                    .get("summary", {})
                    .get("composite_performance", 0)
                    else "down"
