...
class CapsuleDatabase:
    def __init__(self):
        self.db_url = os.getenv(
            "DATABASE_URL", "postgresql://localhost:5432/codex_dominion"
        )
    ...
    def record_run(self, run_data: Dict) -> Dict:
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT id FROM capsules WHERE slug = %s",
                    (run_data["capsule_slug"],),
                )
    ...
    def get_capsule_performance(self) -> List[Dict]:
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM capsule_performance ORDER BY success_rate DESC"
                )
                return [dict(row) for row in cursor.fetchall()]
