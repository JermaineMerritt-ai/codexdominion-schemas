...
class SystemFilesStatusReport:
    ...
    def print_status_summary(self, report):
        ...
        critical_created = sum(1 for f in report["critical_files_created"].values() if f["exists"])
        
        total_config = len(report["configuration_files_created"]) 
        config_created = sum(1 for f in report["configuration_files_created"].values() if f["exists"])
        
        total_data = len(report["data_files_created"])
        data_created = sum(1 for f in report["data_files_created"].values() if f["exists"])
        
        print(f"🚨 Critical Files: {critical_created}/{total_critical} ({'100%' if critical_created == total_critical else 'NEEDS ATTENTION'})")
        print(f"⚙️ Config Files: {config_created}/{total_config} ({'100%' if config_created == total_config else 'PARTIAL'})")
        print(f"🗂️ Data Files: {data_created}/{total_data} ({'100%' if data_created == total_data else 'PARTIAL'})")
