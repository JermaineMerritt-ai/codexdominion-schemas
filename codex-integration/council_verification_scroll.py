...
def validate_entry(entry, required_keys, index, filename):
    ...
    if missing:
        silence(f"{filename} – Entry {index} is incomplete. Missing: {', '.join(missing)}")
        return False
...
def main():
    ...
    if all_passed:
        ...
        silence("Some scrolls are incomplete. The Council must inscribe corrections before transmission.")
