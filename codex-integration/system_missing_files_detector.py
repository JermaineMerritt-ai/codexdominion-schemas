...
class SystemMissingFilesDetector:
    ...
    def analyze_python_imports(self) -> Dict:
                ...
                for from_module, import_items in import_lines:
                    if from_module:
                        ...
                        if from_module.startswith('.'):
                            relative_path = self.resolve_relative_import(py_file, from_module)
        ...
        if python_analysis["broken_relative_imports"]:
            print(f"❌ Found {len(python_analysis['broken_relative_imports'])} broken relative imports")
    ...
    def analyze_yaml_references(self) -> Dict:
        ...
        yaml_files = list(self.workspace_root.rglob("*.yaml")) + list(self.workspace_root.rglob("*.yml"))
        ...
        for yaml_file in yaml_files:
            ...
            try:
                ...
                self.find_file_references_in_dict(yaml_content, yaml_file, yaml_analysis)
        ...
        if yaml_analysis["missing_referenced_files"]:
            print(f"❌ Found {len(yaml_analysis['missing_referenced_files'])} missing referenced files")
    ...
    def analyze_json_references(self) -> Dict:
        ...
        for json_file in json_files:
            ...
            try:
                ...
                self.find_file_references_in_dict(json_content, json_file, json_analysis)
        ...
        if json_analysis["required_data_files"]:
            print(f"⚠️ Missing {len(json_analysis['required_data_files'])} expected data files")
    ...
    def analyze_system_expected_files(self) -> Dict:
        ...
        print(f"⚙️ Checked {len(system_analysis['expected_files'])} expected system files")
        print(f"⚠️ Missing {len(system_analysis['missing_system_files'])} system files")
        if system_analysis["critical_files_missing"]:
            print(f"❌ Missing {len(system_analysis['critical_files_missing'])} CRITICAL files")
    ...
    def analyze_github_actions_deps(self) -> Dict:
        ...
        if github_workflows_dir.exists():
            workflow_files = list(github_workflows_dir.glob("*.yaml")) + list(github_workflows_dir.glob("*.yml"))
            ...
            if capsule_dir.exists():
                for capsule_file in capsule_dir.rglob("*capsule*.yaml"):
                    ...
                    try:
                        ...
                        if "lineage" in capsule_data and "parent_capsules" in capsule_data["lineage"]:
                            for parent in capsule_data["lineage"]["parent_capsules"]:
                                # This would check if parent capsule exists
                                pass
                        
                    except Exception as e:
                        print(f"⚠️ Error analyzing capsule {capsule_file.name}: {str(e)}")
    ...
    def analyze_infrastructure_files(self) -> Dict:
        ...
        print(f"🏗️ Checked {infra_analysis['config_files_checked']} infrastructure configs")
            ...
            for key, value in data.items():
                if isinstance(value, str):
                    ...
                    if self.looks_like_file_path(value):
                        referenced_path = self.resolve_path_reference(value, source_file)
