...
class SystemRepairEngine:
    ...
    def validate_yaml_files(self) -> Dict:
        ...
        print(f"📄 YAML validation: {yaml_validation['valid_files']} valid, {yaml_validation['invalid_files']} invalid")
    ...
    def validate_json_files(self) -> Dict:
        ...
        print(f"🗂️ JSON validation: {json_validation['valid_files']} valid, {json_validation['invalid_files']} invalid")
    ...
    def validate_python_files(self) -> Dict:
        ...
        print(f"🐍 Python validation: {python_validation['valid_files']} valid, {python_validation['invalid_files']} invalid")
                ...
                for capsule_path in capsule_dir.rglob("*.yaml"):
                    if "capsule" in capsule_path.name:
                        ...
                        try:
                            ...
                            if "capsule" in capsule_data and "name" in capsule_data["capsule"]:
                                capsule_validation["capsules_valid"] += 1
                                print(f"✅ Valid capsule: {capsule_data['capsule']['name']}")
                            else:
                                capsule_validation["errors"].append(f"Invalid capsule structure: {capsule_path.name}")
                                
                        except Exception as e:
                            capsule_validation["errors"].append(f"{capsule_path.name}: {str(e)}")
        
        print(f"💊 Capsule validation: {capsule_validation['capsules_valid']}/{capsule_validation['capsules_found']} valid")
    ...
    def check_dependencies(self) -> Dict:
        ...
        dependency_check = {"dependencies_checked": 0, "dependencies_installed": 0, "missing": []}
