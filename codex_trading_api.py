...
try:
    from codex_database import (AffiliateMetrics, AmmEvent, AmmPool,
                                CodexDatabase, DailyPick, Portfolio, Position,
                                db)
...
async def create_portfolio(
    portfolio: PortfolioCreate, current_user: dict = Depends(verify_token)
):
    ...
    try:
        ...
        new_portfolio = await db.create_portfolio(
            owner_id=portfolio.owner_id, risk_tier=portfolio.risk_tier
        )
...
async def create_position(
    position: PositionCreate, current_user: dict = Depends(verify_token)
):
...
async def get_portfolio_positions(
    portfolio_id: str, current_user: dict = Depends(verify_token)
):
...
async def update_position(
    position_id: str, update: PositionUpdate, current_user: dict = Depends(verify_token)
):
...
async def create_daily_pick(
    pick: DailyPickCreate, current_user: dict = Depends(verify_token)
):
...
async def record_affiliate_metrics(
    metrics: AffiliateMetricsCreate, current_user: dict = Depends(verify_token)
):
...
async def create_amm_pool(
    pool: AmmPoolCreate, current_user: dict = Depends(verify_token)
):
...
async def create_amm_event(
    event: AmmEventCreate, current_user: dict = Depends(verify_token)
):
    """Record an AMM event"""
    try:
        if not DATABASE_AVAILABLE:
...
# Analytics endpoints
@app.get(
    "/api/analytics/portfolio-performance/{portfolio_id}",
    response_model=dict,
    tags=["Analytics"],
)
...
async def get_market_data(symbol: str, current_user: dict = Depends(verify_token)):
