...
class CosmicSSLManager:
    ...
    def check_existing_certificates(self):
        ...
        try:
            result = subprocess.run(
                ["sudo", "certbot", "certificates"], capture_output=True, text=True
            )
    ...
    def obtain_certificates(self, interactive=True):
        ...
        print(
            f"🔒 Obtaining SSL certificates for {self.domain} and {self.www_domain}..."
        )
    ...
    def check_timer_status(self):
        ...
        try:
            ...
            if result.returncode == 0 and "active" in result.stdout:
                ...
                print(
                    f"📅 Next renewal check: {timer_result.stdout.split('\n')[1] if timer_result.stdout else 'Unknown'}"
                )
                return True
            else:
