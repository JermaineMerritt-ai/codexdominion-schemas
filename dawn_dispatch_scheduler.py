...
class DawnDispatchScheduler:
    ...
    def _log_execution(self, status: str, timestamp: str, error: str = None):
        ...
        try:
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(
                    f"{datetime.datetime.now().isoformat()} - {status} - {error if error else 'OK'}\n"
                )
    ...
    def setup_schedule(self):
        ...
        try:
            ...
            schedule_time = self.config.get("dispatch_settings", {}).get(
        ...
        try:
            ...
            while self.running:
                ...
                if next_run:
                    ...
                    print(
                        f"\r⏰ Next dispatch in: {int(hours):02d}h {int(minutes):02d}m",
                        end="",
                        flush=True,
                    )
    ...
    def show_schedule_info(self):
        ...
        try:
            ...
            print(
                f"📚 Archive: {'Enabled' if config.get('archive_enabled', True) else 'Disabled'}"
            )
            print(
                f"📢 Auto Proclaim: {'Enabled' if config.get('auto_proclaim', False) else 'Disabled'}"
            )
            ...
            if jobs:
                ...
                for i, job in enumerate(jobs, 1):
                    ...
                    if next_run:
                        print(
                            f"   {i}. Next run: {next_run.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
            else:
...
def main():
    ...
    parser.add_argument(
        "--now", action="store_true", help="Execute dawn dispatch immediately"
    )
