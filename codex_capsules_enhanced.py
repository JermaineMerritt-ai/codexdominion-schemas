...
class CodexCapsulesWithArchiving:
    ...
    async def execute_capsule(self, capsule_slug: str, actor: str = "system") -> Dict[str, Any]:
        ...
        try:
            ...
            if archive_type == 'snapshot':
                ...
                archive_uri = archive_bulletin(
                    execution_result['bulletin_content'], 
                    bulletin_type, 
                    capsule_slug
                )
            elif archive_type == 'analysis_report' and 'report_data' in execution_result:
                analysis_type = execution_result.get('analysis_type', 'general')
                archive_uri = archive_analysis_report(
                    execution_result['report_data'],
                    analysis_type,
                    capsule_slug
                )
            ...
            try:
                artifact_count = len(execution_result.get("artifacts", [])) if isinstance(execution_result, dict) else 1
                festival_cycle = self.festival_keeper.proclaim_capsule_completion(
                    capsule_slug, 
                    artifact_count
                )
                print(f"🎭 Festival cycle recorded: {festival_cycle.get('cycle_id', 'unknown')}")
    ...
    async def _signals_daily_capsule(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "market_data": {
                ...
                "sector_rotation": random.choice(["tech_to_energy", "energy_to_finance", "stable"])
            },
    ...
    async def _treasury_audit_capsule(self) -> Dict[str, Any]:
        ...
        audit_data = {
            ...
            "risk_assessment": {
                ...
                "concentration_risk": random.choice(["minimal", "moderate", "elevated"]),
...
"""
    ...
    async def _education_matrix_capsule(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_competency": sum(education_data["competency_matrix"].values()) / len(education_data["competency_matrix"])
        }
    ...
    def get_capsule_status(self) -> Dict[str, Any]:
        ...
        for slug, capsule in self.active_capsules.items():
            ...
            db_capsule_info = db_stats["by_capsule"].get(slug, {
                "total_runs": 0,
                "successful_runs": 0,
                "success_rate": 0,
                "last_execution": "none"
            ...
            status["capsules"][slug] = {
                ...
                "last_artifact": recent_artifacts[0]["timestamp"] if recent_artifacts else "none",
                "archive_type": capsule.get("archive_type", "snapshot"),
...
async def main():
    ...
    if command == "status":
        ...
        for slug, info in status["capsules"].items():
            ...
            print(f"   Database Runs: {info['database_runs']} ({info['success_rate']} success)")
        ...
        print("Invalid command. Use 'status', 'execute <capsule>', 'list <capsule>', 'runs [capsule]', or 'stats'")
