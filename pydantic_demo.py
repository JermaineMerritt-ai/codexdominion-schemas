...
def demonstrate_pydantic_models():
    ...
    for trans in transactions:
        source_name = (
            trans.source if isinstance(trans.source, str) else trans.source.value
        )
    ...
    for trans in transactions:
        ...
        star = ConstellationStar(
            ...
            cycles=Cycle(
                total=trans.amount,
                morning=trans.amount * 0.4,
                twilight=trans.amount * 0.6,
            ),
        )
        stars.append(star)
    ...
    for trans in transactions:
        entry = data_manager.create_ledger_entry(
            source=trans.source, cycle="Q4 2025", amount=trans.amount
        )
    ...
    for entry in ledger_entries:
        source_name = (
            entry.source if isinstance(entry.source, str) else entry.source.value
        )
    ...
    for approval in approvals:
        source_name = (
            approval.source
            if isinstance(approval.source, str)
            else approval.source.value
        )
        status_name = (
            approval.status
            if isinstance(approval.status, str)
