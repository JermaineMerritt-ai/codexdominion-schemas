# Codex Dominion Go-Live Ceremony (PowerShell)
$ErrorActionPreference = 'Stop'

Write-Host "🔥 Codex Dominion Go-Live Ceremony Initiated 🔥"

# Step 1: Install Argo Rollouts
Write-Host "Installing Argo Rollouts..."
helm repo add argo https://argoproj.github.io/argo-helm
helm repo update
kubectl create namespace codex 2>$null
...
Write-Host "Applying Alertmanager ceremonial routes..."
kubectl apply -f alertmanager/config.yaml -n monitoring

# Step 5: Apply ServiceMonitors + Dashboard
Write-Host "Applying ServiceMonitors and Codex Dominion Dashboard..."
kubectl apply -f k8s/monitoring.yaml -n monitoring
kubectl apply -f grafana/codex-dominion-dashboard.json -n monitoring

Write-Host "✅ Codex Dominion Guardianship System is now live!"
Write-Host "GitHub dispatches → GKE vessels → Argo guides → Prometheus watches → Grafana mirrors → Alertmanager heralds → Twilio speaks → Copilot narrates → Super Action AI commands."
