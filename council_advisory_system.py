...
class DigitalEmpireCouncils:
    ...
    def __init__(self):
        ...
        self.total_advisors = sum(
            len(council["members"]) for council in self.councils.values()
        )
...
def main():
    ...
    with col1:
        ...
        if st.button("🏛️ Convene Council Session") and query and selected_council:
            with st.spinner(f"Convening {selected_council} for strategic advisory..."):
                session = asyncio.run(
                    st.session_state.council_system.convene_council_session(
                        query, selected_council
                    )
                )
        ...
        if st.button("📊 Generate Empire Strategic Briefing"):
            with st.spinner("Generating comprehensive strategic briefing..."):
                ...
                st.write(
                    f"• **Jermaine Integration**: {coordination['jermaine_integration']}"
                )
                st.write(f"• **Avatar Oversight**: {coordination['avatar_oversight']}")
                st.write(
                    f"• **Precision Execution**: {coordination['dot300_precision']}"
                )
    ...
    with col2:
        ...
        for (
            council_name,
            council_info,
        ) in st.session_state.council_system.councils.items():
