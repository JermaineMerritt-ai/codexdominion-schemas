...
class WooCommerceSync:
    ...
    def update_product(self, product_id: int, product_data: Dict) -> Dict:
        ...
        return self.wc_request(
            f"products/{product_id}", method="PUT", data=product_data
        )
    ...
    def sync_products(self, products_json: List[Dict], batch_size: int = 10) -> Dict:
        ...
        for i, product in enumerate(products_json):
            try:
                ...
                if product.get("sku"):
                    try:
                        existing_products = self.wc_request(
                            f"products?sku={product['sku']}"
                        )
                ...
                print(
                    f"❌ Error syncing product {product.get('name', 'Unknown')}: {str(e)}"
                )
