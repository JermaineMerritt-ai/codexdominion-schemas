...
def display_pending_contributions(data):
    ...
    for item in pending:
        ...
        with col1:
            if st.button(f"✅ Affirm {contribution_id}", key=f"affirm_{contribution_id}"):
        ...
        with col2:
            if st.button(f"🤫 Silence {contribution_id}", key=f"silence_{contribution_id}"):
        ...
        with col3:
            st.markdown(f"<small>*Council authority: Affirm to bless, Silence to remove from continuum*</small>", unsafe_allow_html=True)
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["⚖️ Pending Review", "📚 Decision History", "📊 Analytics"])
